				   ...
				   int idx, u32 value)
{
	...
	os_strlcpy(iwr.ifr_ifrn.ifrn_name, drv->ifname, IFNAMSIZ);
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case SIOCGIWAP:
			wpa_printf(MSG_DEBUG, "Wireless event: new AP: "
				   MACSTR,
				   MAC2STR((u8 *) ap_addr.sa_data));
			if (is_zero_ether_addr(
				    (const u8 *) ap_addr.sa_data) ||
			    os_memcmp(ap_addr.sa_data,
					      ...
					      u8 *buf, size_t len)
{
	...
	if (drv->operstate == 1 &&
		...
		netlink_send_oper_ifla_wpa(drv->netlink, drv->ifindex,
			       -1, IF_OPER_UP);
#endif	
...
void * wpa_driver_wext_init(void *ctx, const char *ifname)
{
	...
	drv->netlink = netlink_init_wpa(cfg);
	if (drv->netlink == NULL) {
		os_free(cfg);
		goto err2;
	}
...
err3:
	...
	netlink_deinit_wpa(drv->netlink);
err2:
...
void wpa_driver_wext_deinit(void *priv)
{
	...
	netlink_send_oper_ifla_wpa(drv->netlink, drv->ifindex, 0, IF_OPER_UP);
	netlink_deinit_wpa(drv->netlink);
	//rfkill_deinit(drv->rfkill);
				    ...
				    size_t *len)
{
	...
	for (;;) {
		...
		ret = ioctl(drv->ioctl_sock, SIOCGIWSCAN, &iwr);
		if (ret == 0)
			break;

		if (ret == -E2BIG && res_buf_len < 65535) {
			os_free(res_buf);
			res_buf = NULL;
			res_buf_len *= 2;
...
int wpa_driver_wext_set_operstate(void *priv, int state)
{
	...
	drv->operstate = state;
	connectOperstate = state;
	return netlink_send_oper_ifla_wpa(drv->netlink, drv->ifindex, -1,
