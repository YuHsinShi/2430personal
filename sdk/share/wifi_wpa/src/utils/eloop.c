...
void eloop_run(void)
{
	...
	while ((!eloop.terminate &&
		...
		if (timeout) {
			...
			if(ioctl(ITP_DEVICE_WIFI, ITP_IOCTL_WIFI_MODE, NULL) == 1){ //sta mode
				if(tv.sec<0 || tv.sec>3){
					tv.sec  = 3;
					tv.usec = 0;
				}
			}
		...
		if (timeout) {
			...
			if(ioctl(ITP_DEVICE_WIFI, ITP_IOCTL_WIFI_MODE, NULL) == 1){ //sta mode
				time_diff = now.sec - timeout->time.sec;
	
				if (time_diff < 0){
					now.sec  = timeout->time.sec;
					now.usec = timeout->time.usec;
				}
			}
