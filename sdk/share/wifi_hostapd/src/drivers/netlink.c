...
int netlink_receive_hostapd(void)
{
	char buf[8192];
	int left;
...
struct netlink_data * netlink_init_hostapd(struct netlink_config *cfg)
{
	...
	if (netlink->sock < 0) {
		...
		netlink_deinit_hostapd(netlink);
		return NULL;
	}
	...
	eloop_register_read_sock(netlink->sock, netlink_receive_hostapd, netlink,
				 NULL);
...
void netlink_deinit_hostapd(struct netlink_data *netlink)
{
	if (netlink == NULL)
		return;
	if (netlink->sock >= 0) {
...
int netlink_send_oper_ifla_hostapd(struct netlink_data *netlink, int ifindex,
