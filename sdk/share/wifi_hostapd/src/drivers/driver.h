 ...
 * from the driver.
 */
void hostapd_supplicant_event(void *ctx, enum wpa_event_type event,
				   ...
				   size_t ielen, int reassoc)
{
	...
	hostapd_supplicant_event(ctx, EVENT_ASSOC, &event);
}
...
static inline void drv_event_disassoc(void *ctx, const u8 *addr)
{
	...
	hostapd_supplicant_event(ctx, EVENT_DISASSOC, &event);
}
				      ...
				      size_t data_len)
{
	...
	hostapd_supplicant_event(ctx, EVENT_EAPOL_RX, &event);
}
