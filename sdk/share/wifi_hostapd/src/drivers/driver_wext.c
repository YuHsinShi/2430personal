					      ...
					      u8 *buf, size_t len)
{
	...
	if (drv->operstate == 1 &&
		...
		netlink_send_oper_ifla_hostapd(drv->netlink, drv->ifindex,
			       -1, IF_OPER_UP);
#endif	
...
err3:
	...
	netlink_deinit_hostapd(drv->netlink);
err2:
...
void wpa_driver_wext_deinit(void *priv)
{
	...
	netlink_send_oper_ifla_hostapd(drv->netlink, drv->ifindex, 0, IF_OPER_UP);
	netlink_deinit_hostapd(drv->netlink);
	//rfkill_deinit(drv->rfkill);
...
int wpa_driver_wext_set_operstate(void *priv, int state)
{
	...
	drv->operstate = state;
	return netlink_send_oper_ifla_hostapd(drv->netlink, drv->ifindex, -1,
