...
static int rsn_preauth_iface_add(struct hostapd_data *hapd, const char *ifname)
{
	...
	piface->l2 = l2_packet_init_hostapd(piface->ifname, NULL, ETH_P_PREAUTH,
...
void rsn_preauth_iface_deinit(struct hostapd_data *hapd)
{
	...
	while (piface) {
		prev = piface;
		piface = piface->next;
		l2_packet_deinit_hostapd(prev->l2);
		os_free(prev->ifname);
		os_free(prev);
	}
}
		      ...
		      u8 *buf, size_t len)
{
	...
	if (l2_packet_send_hostapd(piface->l2, sta->addr, ETH_P_PREAUTH, (u8 *) ethhdr,
			   sizeof(*ethhdr) + len) < 0) {
