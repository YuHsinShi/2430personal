...
extern char* hostapdConf;

static char softAPssid[32];
static char softAPpsk[64];
static int bestchannel = 10;
static int hiddenssid = 0;
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
	...
	int pairwise;
	size_t i;
	memset(softAPpsk,0,64);
	memset(softAPssid,0,32);

	f = fopen(fname, "r");
	if (f == NULL) {
		wpa_printf(MSG_ERROR, "Could not open configuration file '%s' "
			   "for reading.", fname);
		wpa_printf(MSG_ERROR, "Do ugResetFactory() and reboot!");
		...
		*pos = '\0';
		...
		if (os_strcmp(buf, "interface") == 0) {
			...
			for (j = 0; hostapd_drivers[j]; j++) {
				printf("hostapd_drivers %s\n",hostapd_drivers[j]->name);
			...
			if (bss->ssid.ssid_len > HOSTAPD_MAX_SSID_LEN ||
				...
				if(mpStatus == true)
				{
                    os_memcpy(&bss->ssid.ssid[bss->ssid.ssid_len],macString,strlen(macString));
					bss->ssid.ssid_len = bss->ssid.ssid_len + strlen(macString);
				}

				bss->ssid.ssid[bss->ssid.ssid_len] = '\0';
				bss->ssid.ssid_set = 1;
				strcpy(softAPssid,bss->ssid.ssid);
			}
		...
		} else if (os_strcmp(buf, "radius_acct_interim_interval") == 0)
		{
			...
			if (len < 8 || len > 63) {
				...
				strcpy(softAPpsk,bss->ssid.wpa_passphrase);
			}
            ...
            conf->channel = bestchannel;
            tempchannel = conf->channel;
			#else
			...
			hiddenssid = bss->ignore_broadcast_ssid;
...
{
    strcpy(ssid,softAPssid);
	if(strlen(softAPpsk) == 0)
		psk = NULL;
	else
		strcpy(psk,softAPpsk);
}

void hostapd_set_bestchannel(int channel)
{
 	bestchannel = channel;
}

void hostapd_set_hiddenssid(int hidden)
{
 	hiddenssid = hidden;
}
...
int hostapd_config_write(char* ssid, char* psk)
{
	...
	//char* name = "B:/hostapd.conf"; 
		
	wpa_printf(MSG_DEBUG, "Writing configuration file '%s'", hostapdConf);

	f = fopen(hostapdConf, "w");
	if (f == NULL) {
		wpa_printf(MSG_DEBUG, "Failed to open '%s' for writing", hostapdConf);
		return -1;
	}
    printf("[hostapd]%s() L#%ld: Write data to conf %s\r\n", __FUNCTION__, __LINE__, hostapdConf);
	...
	{
        fprintf(f, "wpa=0\n");
		fprintf(f, "#wpa_passphrase=87654321\n");
	}
    fprintf(f, "#bridge=br0\n");
    fprintf(f, "#eap_server=1\n");
    //fprintf(f, "wps_state=0\n");
    fprintf(f, "#uuid=12345678-9abc-def0-1234-56789abcdef0\n");
    fprintf(f, "#device_name=RTL8192CU\n");
    fprintf(f, "#manufacturer=Realtek\n");
    fprintf(f, "#model_name=RTW_SOFTAP\n");
    fprintf(f, "#model_number=WLAN_CU\n");
    fprintf(f, "#serial_number=12345\n");
    fprintf(f, "#device_type=6-0050F204-1\n");
	fprintf(f, "#os_version=01020300\n");
	fprintf(f, "#config_methods=label display push_button keypad\n");
	fprintf(f, "driver=rtl871xdrv\n");
	fprintf(f, "beacon_int=500\n");
	fprintf(f, "hw_mode=g\n");
	fprintf(f, "#ieee80211n=1\n");
	fprintf(f, "#wme_enabled=1\n");
	fprintf(f, "#ht_capab=[SHORT-GI-20][SHORT-GI-40][HT40+]\n");
    if(psk)
    {
		fprintf(f, "wpa_key_mgmt=WPA-PSK\n");
	    fprintf(f, "wpa_pairwise=TKIP\n");
		fprintf(f, "wpa_group_rekey=86400\n");		
    }	
	else
	{
        fprintf(f, "#wpa_key_mgmt=WPA-PSK\n");
...
int hostapd_config_hiddenssid(int sethiddenssid)
{
    int ret = 0;
	char buf[256], *pos;
	int line = 0;
...
int iteHostapd_config_read(const char *fname)
{
	FILE *f;
	char buf[256], *pos;
	int line = 0;
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*pos = '\0';
        ...
        if (os_strcmp(buf, "ssid") == 0) {
				...
				strcpy(softAPssid,pos);
			}
		if (os_strcmp(buf, "wpa_passphrase") == 0) {

				strcpy(softAPpsk,pos);
			}
	}
