...
static uint16_t pre_seq = 0;
static int      have_session = 0, multicast_mode = 0;
...
void rtp_putq(queue_t *q, mblk_t *mp)
{
    ...
    //if (rtp->seq_number-seqq > 1)    
    //	printf("rtp_putq(): Enqueuing packet with ts=%i and seq=(%i)\n",rtp->timestamp,rtp->seq_number);
	//seqq = rtp->seq_number;
    if (qempty(q)) {
        putq(q,mp);
        return;
...
#if defined(ENABLE_VIDEO_MULTICAST)		
		if(!(session->flags & RTP_SESSION_MULTICAST_MODE) && !have_session && pre_timestamp > rtp->timestamp && session->rtp.loc_port > 9000)
			pre_seq = 0;
		if(session->rtp.loc_port > 9000 && session->use_multicast && pre_seq)
		{
			if(rtp->seq_number < (pre_seq+1))
			{
				printf("###rtp_seq(new=%d, old=%d)\n", rtp->seq_number, pre_seq);
				rtp->seq_number = pre_seq+1;
				session->rtp.snd_seq = rtp->seq_number+1;
			}
		}
		if(session->flags & RTP_SESSION_MULTICAST_MODE)
		{
			pre_seq = rtp->seq_number;
			pre_timestamp = packet_ts;
			have_session = 1;
		}
#endif		
...
rtp_session_recvm_with_ts (RtpSession * session, uint32_t user_ts)
{
    ...
    if (session->flags & RTP_SESSION_RECV_NOT_STARTED)
    {
        ...
        consecutive calls for a same timestamp*/
    ...
    if (session->flags & RTP_SESSION_RECV_SYNC)
    {
        ...
        /* delete the recv synchronization flag */
    ...
    if (session->rtp.jittctl.enabled==TRUE){
        ...
        {
#if defined(ENABLE_VIDEO_MULTICAST)        
            mp=getq(&session->rtp.rq); /*for video not check timestamp*/
#else
...
/*set if session use multicast streaming*/
void
rtp_session_set_multicast_rtp (RtpSession * session, bool_t yesno)
{
    session->use_multicast = yesno;
}
