...
int RTSPServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort) {
  ...
  do {
    ...
    ourSocket = setupStreamSocket(env, ourPort, 1);
...
#define ENABLE_ONE_SERVICE_ONE_CLIENT       1

#if (CFG_DEMOD_SUPPORT_COUNT > 0)
    #if (CFG_DEMOD_SUPPORT_COUNT > 3)
        #define MAX_DEMOD_CNT         4
    #else
        #define MAX_DEMOD_CNT         CFG_DEMOD_SUPPORT_COUNT // 4
    #endif
#else
    #define MAX_DEMOD_CNT         1
#endif

#define MAX_SERVICE_CNT_PER_DEMOD       4

typedef enum CLIENT_SERVICE_STATUS_T
{
    CLIENT_SERVICE_STATUS_IDLE      = 0,
    CLIENT_SERVICE_STATUS_DESCRIBE,
    CLIENT_SERVICE_STATUS_ACT,
}CLIENT_SERVICE_STATUS;

typedef struct CLIENT_SERVICE_MGR_T
{
    char                    last_name[SERVICE_NAME_MAX_LENGTH];
    uint32_t                client_ip;
    CLIENT_SERVICE_STATUS   status;
    unsigned                sessionId;
}CLIENT_SERVICE_MGR;

static CLIENT_SERVICE_MGR   g_srvc_mgr[MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD] = {0};

static void
set_service_status(char const *streamName, uint32_t ip_addr, unsigned sessionId, CLIENT_SERVICE_STATUS status)
{
    uint32_t    i = 0;
    char const  *pLastName = 0; // it should be "channel_x.xx.airts"

#if (ENABLE_ONE_SERVICE_ONE_CLIENT)
    for(i = 0; i < (MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD); i++)
    {
       if( strlen(g_srvc_mgr[i].last_name) == 0 )
           continue;

       if( i == (MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD) )   break;

        if( strncmp(g_srvc_mgr[i].last_name, streamName, strlen(streamName)) == 0 )
        {
            switch( status )
            {
                case CLIENT_SERVICE_STATUS_IDLE:
                    if( g_srvc_mgr[i].client_ip == ip_addr &&
                        g_srvc_mgr[i].sessionId == sessionId )
                    {
                        // printf("--- clear ip %d.%d.%d.%d, %s\n",
                        //     (g_srvc_mgr[i].client_ip & 0x000000FF),
                        //     ((g_srvc_mgr[i].client_ip & 0x0000FF00) >> 8),
                        //     ((g_srvc_mgr[i].client_ip & 0x00FF0000) >> 16),
                        //     ((g_srvc_mgr[i].client_ip & 0xFF000000) >> 24),
                        //     streamName);

                        memset(g_srvc_mgr[i].last_name, 0x0, SERVICE_NAME_MAX_LENGTH);
                        g_srvc_mgr[i].status = CLIENT_SERVICE_STATUS_IDLE;
                        g_srvc_mgr[i].client_ip = g_srvc_mgr[i].sessionId = 0;
                    }
                    break;
                case CLIENT_SERVICE_STATUS_DESCRIBE:
                case CLIENT_SERVICE_STATUS_ACT:
                    g_srvc_mgr[i].status = status;
                    break;
            }
            break;
        }
...
Boolean RTSPServer::isServiceIdle(char const *streamName)
{
    Boolean     bFree = True, bExsit = False;
    uint32_t    demod_idx = 0, service_idx = 0;
    uint32_t    i = 0;

#if (ENABLE_ONE_SERVICE_ONE_CLIENT)
    //for(i = 0; i < MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD; i++)
    //    printf("  check name: %s, status=%d\n", g_srvc_mgr[i].last_name, g_srvc_mgr[i].status);
    ...
    for(i = 0; i < (MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD); i++)
    {
        ...
        if( strncmp(g_srvc_mgr[i].last_name, streamName, strlen(streamName)) == 0 )
        {
            if( g_srvc_mgr[i].status == CLIENT_SERVICE_STATUS_ACT )
            {
                bFree = False;
                printf("-- %s is using !!!\n", streamName);
                break;
    ...
    if( bFree == True )
    {
        if( bExsit == False )
        {
            ...
            if( i < (MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD) )
            {
                // snprintf(g_srvc_mgr[i].last_name, SERVICE_NAME_MAX_LENGTH, "%s", streamName);
                memset(g_srvc_mgr[i].last_name, 0x0, SERVICE_NAME_MAX_LENGTH);
                memcpy(g_srvc_mgr[i].last_name, streamName, strlen(streamName));

                g_srvc_mgr[i].client_ip = fIncomingClientAddr.sin_addr.s_addr; // uint32_t type
                g_srvc_mgr[i].sessionId = fClientSessionId;
                g_srvc_mgr[i].status    = CLIENT_SERVICE_STATUS_DESCRIBE;
            }
        }
        //uint32_t    act = i;
        //for(i = 0; i < MAX_DEMOD_CNT*MAX_SERVICE_CNT_PER_DEMOD; i++)
        //    printf("  act=%d, check name: %s, status=%d, %u\n",
        //        act, g_srvc_mgr[i].last_name, g_srvc_mgr[i].status, g_srvc_mgr[i].sessionId);
...
#if defined(CFG_DEMOD_ENABLE)
    set_service_status(fClient_streamName, fClientAddr.sin_addr.s_addr, fOurSessionId, CLIENT_SERVICE_STATUS_IDLE);
#endif
    ...
    if (parseSucceeded) {
      ...
      *fLastCRLF = '\r';
      if (parseSucceeded) {
        ...
        if(parseResponseString((char*)fRequestBuffer, responseCode, responseStr)) {
            handleCmd_Response(responseCode, responseStr);
...
#if defined(CFG_WFD_SOURCE_DEVICE) || defined(CFG_WFD_SINK_DEVICE) 
void RTSPServer::RTSPClientSession::handleCmd_Response(	unsigned responseCode, char const* responseStr) {
}
#endif
...
#if defined(CFG_DEMOD_ENABLE)
    fOurServer.fIncomingClientAddr = fClientAddr;
    fOurServer.fClientSessionId    = fOurSessionId;
    //printf(" **** ip %d.%d.%d.%d, %u, %s\n",
    //    (fOurServer.fIncomingClientAddr.sin_addr.s_addr & 0x000000FF),
    //    ((fOurServer.fIncomingClientAddr.sin_addr.s_addr & 0x0000FF00) >> 8),
    //    ((fOurServer.fIncomingClientAddr.sin_addr.s_addr & 0x00FF0000) >> 16),
    //    ((fOurServer.fIncomingClientAddr.sin_addr.s_addr & 0xFF000000) >> 24),
    //    fOurServer.fClientSessionId,
    //    urlTotalSuffix);
#endif    
...
#if defined(CFG_DEMOD_ENABLE)
    snprintf(fClient_streamName, SERVICE_NAME_MAX_LENGTH, "%s", urlTotalSuffix);
    memset(&fOurServer.fIncomingClientAddr, 0x0, sizeof(struct sockaddr_in));
    fOurServer.fClientSessionId = 0;
#endif
...
#if defined(CFG_DEMOD_ENABLE)
    set_service_status(streamName, 0, 0, CLIENT_SERVICE_STATUS_ACT);
#endif
...
UserAuthenticationDatabase::UserAuthenticationDatabase(char const* realm,
  ...
  : fTable(HashTable::create(STRING_HASH_KEYS)),
    fRealm(strDup(realm == NULL ? "RTSP Streaming Media" : realm)),
