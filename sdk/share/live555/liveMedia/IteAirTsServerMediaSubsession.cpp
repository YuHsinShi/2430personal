    ...
    char const *dataFileName)
{
	...
	int  tsi_dev_idx = 0;
	bool ts_used = IteAirTsStreamSource::get_ts_status();
	if(ts_used)
		return NULL;
	
	IteAirTsServerMediaSubsession   *iteAirTsSMS 
        = new IteAirTsServerMediaSubsession(env, dataFileName, tsi_dev_idx);
        //= new IteAirTsServerMediaSubsession(env, dataFileName, 0, 0);
#endif
    return iteAirTsSMS;
}
...
::IteAirTsServerMediaSubsession(
    ...
    int         tsi_dev_idx)
: FileServerMediaSubsession(env, fileName, false),
  _tsi_dev_idx(tsi_dev_idx), _iteAirSource(NULL)
{
    _inputDataSize = TRANSPORT_PACKETS_PER_NETWORK_PACKET*TRANSPORT_PACKET_SIZE;
    return;
}
...
#ifdef CFG_IPTV_TX
    = IteAirTsStreamSource::createNew(envir(), _tsi_dev_idx, _inputDataSize);
#else
