...
int mtal_pb_select_file(MTAL_SPEC *spec)
{
    ...
    if (g_mtal_spec.camera_in > NOT_CAMERA)
    {
        if(g_mtal_spec.camera_in == CAPTURE_IN)
            new_player = &captureplayer;
        else
        {
            #ifdef CFG_UVC_ENABLE
            new_player = &uvcplayer;
            #endif
        }
    }
    else
...
{
    printf("MTAL# %s +\n", __func__);
    if (media_player && media_player->SetCameraParameter)
    {
        media_player->SetCameraParameter(width, height, fps);
    }
    printf("MTAL# %s -\n", __func__);
}
