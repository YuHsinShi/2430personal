...
#define MAX_PES_PAYLOAD 100*1024

#define MAX_MP4_DESCR_COUNT 16

#define CFG_IP_SECURITY_MODE

#if ITE_TSSPEC_ADDITION
...
typedef struct {
	...
	unsigned int valid_resolution	:1;
	unsigned int input_lock		:1;
	unsigned int input_source	:4;
	uint16_t width;
	uint16_t height;
	uint16_t encoder_info;			// MMP_CAP_INPUT_INFO
	unsigned int reserved2		:4;
	unsigned int audio_codec	:4;
	unsigned long sampling_rate;
    unsigned long real_frame_rate_multi_1000;
...
static const AVOption options[] = {
    {"compute_pcr", "Compute exact PCR for each transport stream packet.", offsetof(MpegTSContext, mpeg2ts_compute_pcr), AV_OPT_TYPE_INT,
     0.0, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
    { NULL },
};
#else
static const AVOption options[] = {
...
#if 1  //Steven 2016/03/10
#ifdef MALLOC_DEBUG
                    {
	                    uint8_t* pOldBuffer = pes->buffer;
	                    int oldSize = pes->total_size;
	                    int newSize = 0;
	                    pes->total_size += (MAX_PES_PAYLOAD * 2);
	                    newSize = pes->total_size + FF_INPUT_BUFFER_PADDING_SIZE;
	                    uint8_t* pNewBuffer = av_malloc(newSize);
	                    memcpy(pNewBuffer, pOldBuffer, oldSize);
	                    av_free(pOldBuffer);
	                    pes->buffer = pNewBuffer;
                    }
#else
                    pes->total_size += (MAX_PES_PAYLOAD * 2);
                    pes->buffer = av_realloc(pes->buffer, pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE);
#endif
                    if (!pes->buffer)
                        return AVERROR(ENOMEM);
#else
...
#if ITE_TSSPEC_ADDITION
static SIT_SECTION_TABLE prev_sit = {0, 0, 0, 1, 0, 0 ,0 , 99};

static void sit_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
	MpegTSContext *ts = filter->u.section_filter.opaque;
	AVFormatContext *s = ts->stream;
	SectionHeader h1, *h = &h1;
	const uint8_t *p, *p_end;
	SIT_SECTION_TABLE sit1, *sit = &sit1, *previous_sit = &prev_sit;
	unsigned int val;
	int bUpdated = 0;
	bool bNotSupportResolution = false;

	//av_log(ts->stream, AV_LOG_INFO, "#parsing SITable+\n");
	p_end = section + section_len - 4;
	p = section;
	if (parse_section_header(h, &p, p_end) < 0)
		return;
	val = get8(&p, p_end);
	sit->hdcp = (val & 0x80) >> 7;
	sit->skipframe = (val & 0x40) >> 6;
	sit->valid_resolution = (val & 0x20) >> 5;
	sit->input_lock = (val & 0x10) >> 4;
	sit->input_source = (val & 0x0F);
	sit->width = get16(&p, p_end);
	sit->height = get16(&p, p_end);
	sit->encoder_info = get16(&p, p_end);
	val = get8(&p, p_end);
	sit->reserved2 = (val & 0xF0) >> 4;
	sit->audio_codec = (val & 0x0F);
	sit->sampling_rate = get32(&p, p_end);
    sit->real_frame_rate_multi_1000 = get32(&p, p_end);
#if 0
	av_log(ts->stream, AV_LOG_INFO, "#SIT\n#\tversion=%X, \n#\thdcp=%X,\n#\tskipframe=%X,\n#\tresolution=%X,\n#\tlock=%X,\n#\tsource=%X,\n#\twidth=%u, height=%u,\n#\tinfo=%u,\n#\taudio_codec=%X,\n#\tsample_rate=%u\n",
            h->version,
            sit->hdcp,
			sit->skipframe,
			sit->valid_resolution,
			sit->input_lock,
			sit->input_source,
			sit->width,
			sit->height,
	...
	if(sit->input_lock != previous_sit->input_lock)
	{
		if(!sit->input_lock)
		{
			mtal_set_source_lock_flag(true);
			mtal_put_message(MSG_NO_SIGNAL, NULL);	
		}
		else
		{
			mtal_set_source_lock_flag(false);
			mtal_put_message(MSG_CLEAN_OSD, NULL);
		}
		previous_sit->input_lock = sit->input_lock;
	}

	if(sit->encoder_info != MMP_CAP_INPUT_INFO_640X480_60P 
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_800X600_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1024X768_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X768_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X960_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X1024_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1360X768_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1440X900_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1600X900_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1680X1050_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X480_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X480_59P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X480_60I
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X480_59I
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X576_50P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_720X576_50I
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X720_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X720_59P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1280X720_50P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_60P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_59P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_50P
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_50I
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_60I
		&& sit->encoder_info != MMP_CAP_INPUT_INFO_1920X1080_59I)
	{
		av_log(ts->stream, AV_LOG_INFO, "SIT TABLE: Get Not Support Resolution, sit->encoder_info = %d\n", sit->encoder_info);
		bNotSupportResolution = true;
	}
	
	bUpdated = (sit->encoder_info != previous_sit->encoder_info && !bNotSupportResolution);

#if 0
    if (sit->hdcp != previous_sit->hdcp && sit->input_lock)
    {
        bUpdated = 1;
    }
	...
	if (bUpdated)
	{
		ithHDMITXDisable();
#if 1
		av_log(ts->stream, AV_LOG_INFO, "#PREV SIT\n#\tversion=%X, \n#\thdcp=%X,\n#\tskipframe=%X,\n#\tresolution=%X,\n#\tlock=%X,\n#\tsource=%X,\n#\twidth=%u, height=%u,\n#\tinfo=%u,\n#\taudio_codec=%X,\n#\tsample_rate=%u\n",
            h->version,
            previous_sit->hdcp,
			previous_sit->skipframe,
			previous_sit->valid_resolution,
			previous_sit->input_lock,
			previous_sit->input_source,
			previous_sit->width,
			previous_sit->height,
			previous_sit->encoder_info,
			previous_sit->audio_codec,
			previous_sit->sampling_rate);
		...
		av_log(ts->stream, AV_LOG_INFO, "#SIT\n#\tversion=%X, \n#\thdcp=%X,\n#\tskipframe=%X,\n#\tresolution=%X,\n#\tlock=%X,\n#\tsource=%X,\n#\twidth=%u, height=%u,\n#\tinfo=%u,\n#\taudio_codec=%X,\n#\tsample_rate=%u\n",
			...
			sit->sampling_rate);
#endif
		previous_sit->hdcp = sit->hdcp;
		previous_sit->skipframe = sit->skipframe;
		previous_sit->valid_resolution = sit->valid_resolution;
		previous_sit->input_source = sit->input_source;
		previous_sit->width = sit->width;
		previous_sit->height = sit->height;
		previous_sit->encoder_info = sit->encoder_info;
		previous_sit->audio_codec = sit->audio_codec;
		previous_sit->sampling_rate = sit->sampling_rate;
        previous_sit->input_lock = sit->input_lock;
        previous_sit->real_frame_rate_multi_1000 = sit->real_frame_rate_multi_1000;
		// TODO: you shall pass skipframe flag to itv when source is fixed
#if 0		
        if (sit->hdcp)
        {
            mtal_set_security_flag(true);
        }
        else
        {
            mtal_set_security_flag(false);
        }
        ...
        mtal_set_display_frameRate(((float)sit->real_frame_rate_multi_1000) / 1000);
#endif
        mtal_put_message(MSG_CHANGE_DISPLAY_FORMAT, ((sit->skipframe & 0x3) << 8) | sit->encoder_info);
		//mtal_put_message(MSG_CHANGE_DISPLAY_FORMAT, S2D_Transcode[sit->encoder_info].code);
	}
#if 0
	// FIXME: pass source information to upper layer
	if (s)
	{
		void *priv_data = (((URLContext*)(s->pb->opaque))->priv_data);
		FF_COMMON_DEF *comm = NULL;
		int ret = -1;

		if (priv_data)
			comm = (FF_COMMON_DEF*) &priv_data[0];
		else
			av_log(ts->stream, AV_LOG_ERROR, "URLProtocol priv_data not exist");

		if (comm->setinfo)
			ret = comm->setinfo(priv_data, sit);
		if (ret < 0)
			av_log(ts->stream, AV_LOG_ERROR, "# err in SIT parsing\n");
	}
#endif
	//av_log(NULL, AV_LOG_INFO, "#parsing SITable-\n");
}
#endif
...
#if ITE_TSSPEC_ADDITION
	mpegts_open_section_filter(ts, SIT_PID, sit_cb, ts, 1);
#endif
                        ...
                        const uint8_t *buf, int len)
{
    ...
    if(len1 == 1316 && (buf[0] != 0x47 || buf[188] != 0x47 || buf[376] != 0x47)) //decrypt
    {
        mtal_decrypt_data_by_size(buf, 1312);
    }
#endif
#endif
    ts->pkt = pkt;
