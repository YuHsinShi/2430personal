...
void av_register_all(void)
{
    ...
    REGISTER_MUXDEMUX (RTSP, rtsp); // [2012/11/23, Evan] rtsp client
#endif
    ...
    REGISTER_PROTOCOL (RTP, rtp); // [2012/11/23, Evan] rtsp client
#endif
#if CONFIG_TCP_PROTOCOL
    REGISTER_PROTOCOL (TCP, tcp); // [2012/11/23, Evan] rtsp client
#endif
    //REGISTER_PROTOCOL (TLS, tls);
#if CONFIG_UDP_PROTOCOL
    REGISTER_PROTOCOL (UDP, udp); // [2012/11/23, Evan] rtsp client
#endif

#ifdef CFG_TS_DEMUX_ENABLE
    /* iTE TS demod live streaming */
    REGISTER_PROTOCOL (TSDEMOD, tsdemod);
#endif // CFG_TS_DEMUX_ENABLE

#if CONFIG_TSMULTI_PROTOCOL
    REGISTER_PROTOCOL (TSMULTI, tsmulti);
#endif
}
