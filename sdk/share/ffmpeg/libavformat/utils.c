...
AVInputFormat *av_probe_input_format3(AVProbeData *pd, int is_opened, int *score_ret)
{
    ...
    fmt = NULL;
    while ((fmt1 = av_iformat_next(fmt1))) {    // ask all the registered demuxers if anyone knows the input container format
...
static void update_stream_timings(AVFormatContext *ic)
{
    ...
    for(i = 0;i < ic->nb_streams; i++) {
        ...
        if (st->start_time != AV_NOPTS_VALUE && st->time_base.den) {
            ...
            cq.num = 1;
            cq.den = AV_TIME_BASE;
            start_time1= av_rescale_q(st->start_time, st->time_base, cq);
#else
            ...
            if (st->duration != AV_NOPTS_VALUE) {
                ...
                end_time1 = start_time1
                          + av_rescale_q(st->duration, st->time_base, cq);
#else
                end_time1 = start_time1
        ...
        if (st->duration != AV_NOPTS_VALUE) {
            ...
            cq.num = 1;
            cq.den = AV_TIME_BASE;
            duration1 = av_rescale_q(st->duration, st->time_base, cq);
#else
    ...
    for(i = 0;i < ic->nb_streams; i++) {
        ...
        if (st->start_time == AV_NOPTS_VALUE) {
            ...
            if(ic->start_time != AV_NOPTS_VALUE)
                st->start_time = av_rescale_q(ic->start_time, cq, st->time_base);
            if(ic->duration != AV_NOPTS_VALUE)
                st->duration = av_rescale_q(ic->duration, cq, st->time_base);
#else
            if(ic->start_time != AV_NOPTS_VALUE)
...
static void compute_chapters_end(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_chapters; i++)
        if (s->chapters[i]->end == AV_NOPTS_VALUE) {
            ...
            cq.num = 1;
            cq.den = AV_TIME_BASE;
            end = max_time ? av_rescale_q(max_time, cq, ch->time_base)
                           : INT64_MAX;
#else
...
#if defined(WIN32)
    const int arr1[] = {24,30,60,12,15};
    if(i<60*12) return i*1001;
    else        return arr1[i-60*12]*1000*12;
#else
    if(i<60*12) return i*1001;
...
/*            if(!st->time_base.num)
    ...
    for(;;) {
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            cq.num = 1;
            cq.den = AV_TIME_BASE;
            bq.num = st->avg_frame_rate.den;
            bq.den = st->avg_frame_rate.num;
            if (st->time_base.den > 0)
                t = av_rescale_q(st->info->codec_info_duration, st->time_base, cq);
            if (st->avg_frame_rate.num > 0)
                t = FFMAX(t, av_rescale_q(st->codec_info_nb_frames, bq, cq));
#else
            if (st->time_base.den > 0)
                              ...
                              int (*compare)(AVFormatContext *, AVPacket *, AVPacket *))
{
    ...
    if(*next_point){
        if(chunked){
            ...
            cq.num = 1;
            cq.den = AV_TIME_BASE;
            max= av_rescale_q(s->max_chunk_duration, cq, st->time_base);
#else
...
static int ff_interleave_compare_dts(AVFormatContext *s, AVPacket *next, AVPacket *pkt)
{
    ...
    if(s->audio_preload && ((st->codec->codec_type == AVMEDIA_TYPE_AUDIO) != (st2->codec->codec_type == AVMEDIA_TYPE_AUDIO))){
        ...
        cq.num = 1;
        cq.den = AV_TIME_BASE;
        ts = av_rescale_q(pkt ->dts, st ->time_base, cq) - s->audio_preload*(st ->codec->codec_type == AVMEDIA_TYPE_AUDIO);
        ts2= av_rescale_q(next->dts, st2->time_base, cq) - s->audio_preload*(st2->codec->codec_type == AVMEDIA_TYPE_AUDIO);
#else
...
{
#if HAVE_STRPTIME && !defined(WIN32)
