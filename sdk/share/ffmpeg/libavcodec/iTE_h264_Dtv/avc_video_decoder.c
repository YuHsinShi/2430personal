...
#include "ith/ith_video.h" //#include "mmp_video.h"
...
static int decode_nal_units(const MMP_UINT8 *buf, int buf_size);
int AVC_Video_Decoder_DecodeExtradata(const MMP_UINT8 *buf, int size);
...
#if 0    
    ithVideoWait(gptAVCDecoder, 1000);
    ithVideoHostReset();
...
AVC_Video_Decoder_Init()
{
    ...
    if (pbuf)
    {
        ...
        av_log(NULL, AV_LOG_INFO, "Video decoder get buffer %p\n", pbuf);
    }
    else
        av_log(NULL, AV_LOG_ERROR, "Video decoder get buffer failed! Not enough Memory\n");
	...
	gptAVCVideoDecoder->bIsAVC = MMP_FALSE;
...
AVC_Video_Decoder_Reload()
{
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "not enough data to decode\n");
    }
    ...
    while (return_state == 0 && gptAVCVideoDecoder->remainByte > 0)
    {
        ...
        else
        {
            ...
            {
                av_log(NULL, AV_LOG_ERROR, "Error Start 1 %x %x\n", pNalStream, pBufferEndAddr);
        ...
        if (nalHeader.forbidden_zero_bit != 0)
        {
            ...
            av_log(NULL, AV_LOG_ERROR, "Nal-Uint parsing error !! \n");
        }
        ...
        if (pNalStream >= pBufferEndAddr)
        {
            av_log(NULL, AV_LOG_ERROR, "Error Start 2 %x %x\n", pNalStream, pBufferEndAddr);
            pNalStream = pBufferStartAddr + (pNalStream - pBufferEndAddr);
        }
        ...
        case NAL_UNIT_TYPE_SEQ_PARAM_SET:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_SEQ_PARAM_SET\n");
            if (nalSize <= MAX_NAL_SIZE)
                ret = avc_Seq_Parameter(pNalStream, nalSize);
            else
                av_log(NULL, AV_LOG_ERROR, "! Nal size (%d) > %d\n", nalSize, MAX_NAL_SIZE);

            if (ret != AVC_ERROR_SUCCESS)
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse SPS fail (0x%x)!!", ret);
            }
            break;

        case NAL_UNIT_TYPE_PIC_PARAM_SET:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_PIC_PARAM_SET\n");
            if (nalSize <= MAX_NAL_SIZE)
            ...
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse PPS fail (0x%x)!!", ret);
            }
            break;

        case NAL_UNIT_TYPE_SEI:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_SEI\n");
            if (nalSize <= MAX_NAL_SIZE)
            ...
            {
                //Error control
                av_log(NULL, AV_LOG_ERROR, "parse SEI fail (0x%x)!!", ret);
            }
            break;

        case NAL_UNIT_TYPE_IDR_PICTURE:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_IDR_PICTURE\n");

        case NAL_UNIT_TYPE_NON_IDR_PICTURE:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_NON_IDR_PICTURE\n");
            ...
            av_log(NULL, AV_LOG_DEBUG, "Slice firstMB %d\n", nextSliceFirstMB);
            ...
            {
                av_log(NULL, AV_LOG_ERROR, "resolution not support (width = %d, height = %d), frame_rate_code: %d\n",
            ...
            {
                av_log(NULL, AV_LOG_ERROR, "!!! HD Video Format \n");
        ...
        case NAL_UNIT_TYPE_ACCESS_UNIT_DELIMITER:
            av_log(NULL, AV_LOG_DEBUG, " nal_unit_type: NAL_UNIT_TYPE_ACCESS_UNIT_DELIMITER\n");
            break;
...
AVC_Video_Decoder_Wait(
    void)
{
    if (ithVideoWait(gptAVCDecoder, 3000) != 0)
    {
    ...
    *data_size = 0;
...
#endif    	
    {
        ...
        *data_size                              = sizeof(AVFrame);
        ...
        av_log(NULL, AV_LOG_DEBUG, "display frame buf %d\n", rdIndex);
    }
}
...
static int decode_nal_units(const MMP_UINT8 *buf, int buf_size)
{
    ...
    return AVC_Video_Decoder_Reload();
}
...
int AVC_Video_Decoder_DecodeExtradata(const MMP_UINT8 *buf, int size)
{
    ...
    if (buf[0] == 1)
    {
        ...
        gptAVCVideoDecoder->bIsAVC = MMP_TRUE;
        gptAVCVideoDecoder->bIsNalFormat = MMP_TRUE;

        if (size < 7)
        {
            av_log(NULL, AV_LOG_ERROR, "avcC too short\n");
            return -1;
        }
        ...
        for (i = 0; i < cnt; i++)
        {
            ...
            if (nalsize > size - (p - buf))
                return -1;
            if (decode_nal_units(p, nalsize) < 0)
            {
                av_log(NULL, AV_LOG_ERROR, "Decoding sps %d from avcC failed\n", i);
                return -1;
            }
            p += nalsize;
        }
        ...
        for (i = 0; i < cnt; i++)
        {
            ...
            if (decode_nal_units(p, nalsize) < 0)
            {
                av_log(NULL, AV_LOG_ERROR, "Decoding pps %d from avcC failed\n", i);
                return -1;
            }
            p += nalsize;
        }
        ...
        gptAVCVideoDecoder->NALUnitLength =(buf[4] & 0x03) + 1;
    }
    else
    {
        gptAVCVideoDecoder->bIsAVC = MMP_FALSE;
        gptAVCVideoDecoder->bIsNalFormat = 0;
        if (decode_nal_units(buf, size) < 0)
            return -1;
    }
    return 0;
}
...
int iTE_h264_Dtv_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->extradata_size > 0 && avctx->extradata &&
        AVC_Video_Decoder_DecodeExtradata(avctx->extradata, avctx->extradata_size))
        return -1;
...
{
    av_log(NULL, AV_LOG_INFO, "decode end\n");
    AVC_Video_Decoder_End();
    return 0;
}
...
static void print_video_return_state(
    MMP_UINT32     state)
{
    av_log(NULL, AV_LOG_DEBUG, "-- video state --\n");
    if (state & VIDEO_RETURN_NORMAL)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_NORMAL\n");
    if (state & VIDEO_RETURN_END_OF_PACKET)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_END_OF_PACKET\n");
    if (state & VIDEO_RETURN_RESOLUTION_NOT_SUPPORT)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_RESOLUTION_NOT_SUPPORT\n");
    if (state & VIDEO_RETURN_SKIP_DECODE)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_SKIP_DECODE\n");
    if (state & VIDEO_RETURN_UNEXPECTED_START_CODE)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_UNEXPECTED_START_CODE\n");
    if (state & VIDEO_RETURN_WAIT_FIRE)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_WAIT_FIRE\n");
    if (state & VIDEO_RETURN_OUT_REF_FRAME_RANGE)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_OUT_REF_FRAME_RANGE\n");
    if (state & VIDEO_RETURN_TOOL_NOT_SUPPORT)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_TOOL_NOT_SUPPORT\n");
    if (state & VIDEO_RETURN_GET_DEC_HEADER)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_GET_DEC_HEADER\n");
    if (state & VIDEO_RETURN_SKIP_DISPLAY_ONCE)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_SKIP_DISPLAY_ONCE\n");
    if (state & VIDEO_RETURN_FILL_BLANK)
        av_log(NULL, AV_LOG_DEBUG, "VIDEO_RETURN_FILL_BLANK\n");
}
...
int iTE_h264_Dtv_decode_frame(AVCodecContext *avctx, void *data, int *data_size, AVPacket *avpkt)
{
    ...
    if (gptAVCVideoDecoder->bIsAVC && buf_size >= 9 && buf[0] == 1 && buf[2] == 0 && (buf[4] & 0xFC) == 0xFC && (buf[5] & 0x1F) && buf[8] == 0x67)
    {
        int       cnt = buf[5] & 0x1f;
        MMP_UINT8 *p  = buf + 6;
        while (cnt--)
        ...
        return AVC_Video_Decoder_DecodeExtradata(buf, buf_size);
    }
...
not_extra:
    ...
    gptAVCVideoDecoder->pSegStartAddr    = gptAVCVideoDecoder->pPacketStartAddr;
    gptAVCVideoDecoder->pSegEndAddr      = gptAVCVideoDecoder->pPacketEndAddr;
    av_log(NULL, AV_LOG_DEBUG, "recv packet size %d 0x%02X %02X %02X %02X %02X %02X %02X\n", avpkt->size,
    ...
    {
        return_state = AVC_Video_Decoder_Reload();
        print_video_return_state(return_state);
            ...
            (return_state & VIDEO_RETURN_OUT_REF_FRAME_RANGE))
        {
            ...
            av_log(NULL, AV_LOG_ERROR, "error, return_state = %u\n", return_state);
            break;
        }
        ...
        {
            av_log(NULL, AV_LOG_ERROR, "return_state \"VIDEO_RETURN_SKIP_PACKET\"\n");
            break;
        }
...
#define TEST_FIREHW_BITRATE 0
        ...
        else
        {
            ...
            AVC_Video_Decoder_Display(data, data_size);
        }
#if TEST_FIREHW_BITRATE
        lt3 = ct = av_gettime();
    }
...
{
    av_log(NULL, AV_LOG_INFO, "flush dpb\n");
    flush_dpb();
}
#if 0
