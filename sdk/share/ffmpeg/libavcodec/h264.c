...
#ifndef CFG_FFMPEG_H264_SW //[2012/02/20, Evan]
#include "ith/ith_video.h"

AVC_DECODER *gpAVCDecoder = NULL;
#endif
...
#ifndef CFG_FFMPEG_H264_SW //[2012/04/05, Evan] for prepare slice_data use
	h->parse_history[h->parse_history_count++] = s->gb.index;
#endif
    ...
    if(h->deblocking_filter) {
        for(mb_x= start_x; mb_x<end_x; mb_x++){
            for(mb_y=end_mb_y - FRAME_MBAFF; mb_y<= end_mb_y; mb_y++){
                ...
                /*[2012/03/28, Evan]if (FRAME_MBAFF) {
