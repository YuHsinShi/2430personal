...
//=============================================================================
void
AVC_Video_Decoder_Release(void)
{
    _VIDEO_DECODER_ReleaseValue();
}

void
AVC_Video_Decoder_Init(void)
{
    int     i     = 0, j = 0;
    uint8_t *pbuf = NULL;
...
int iTE_h264_Dtv_decode_frame(AVC_FRAME *data, int *data_size, MMP_UINT8 *buf, int buf_size)
{
    ...
    //printf("recv packet size %d 0x%02X %02X %02X %02X %02X %02X %02X\n", buf_size, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6]);   
