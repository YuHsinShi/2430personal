...
#endif         
    st->use_ec      =lp_config_get_int(lc->config,SOUND,"echocancellation",CFG_AEC_ENABLE);
    st->ecdelay     =lp_config_get_int(lc->config,SOUND,"ecdelay",CFG_AEC_DELAY_MS);
    st->use_volsend =lp_config_get_int(lc->config,SOUND,"use_volsend",CFG_ENABLE_SEND_FILTER);  
    st->use_volrecv =lp_config_get_int(lc->config,SOUND,"use_volrecv",CFG_ENABLE_RECV_FILTER); 
    st->eq_SPK      =lp_config_get_int(lc->config,SOUND,"eq_SPK",CFG_SPK_EQUALIZER_ENABLE);
    st->eq_MIC      =lp_config_get_int(lc->config,SOUND,"eq_MIC",CFG_MIC_EQUALIZER_ENABLE);
    st->use_ng      =lp_config_get_int(lc->config,SOUND,"noisegate",CFG_NOISE_GATE_ENABLE);      
...
void _post_configure_audio_stream(AudioStream *st, LinphoneCore *lc, bool_t muted){
    ...
    if (st->volsend){
        ...
        bool_t use_ng  = st->use_ng;
        double coef     = lp_config_get_float(lc->config,SOUND,"ngcoef",CFG_NOISE_GATE_THRESHOLD);
        double gain     = lp_config_get_float(lc->config,SOUND,"mic_gain",CFG_MIC_GAIN);
        _msvoleme_default_set(st->volsend,use_agc,use_ng,coef,gain);    
    }

    if (st->volrecv){
        bool_t use_agc = CFG_AGC_ENABLE_SPK;
        bool_t use_ng  = CFG_NOISE_GATE_ENABLE_SPK;
        double coef     = lp_config_get_float(lc->config,SOUND,"ngcoef_spk",CFG_NOISE_GATE_THRESHOLD_SPK);
        double gain     = lp_config_get_float(lc->config,SOUND,"playback_gain_db",CFG_SPEAKER_GAIN);
        _msvoleme_default_set(st->volrecv,use_agc,use_ng,coef,gain); 
    }
...
void linphone_call_enable_echo_cancellation(LinphoneCall *call, bool_t enable) {
    if (call!=NULL && call->audiostream!=NULL && call->audiostream->ec){
        ...
        #ifdef ENABLE_DMNR_HAND_HELD_SEPARATION
        ms_filter_call_method(call->audiostream->separate,MS_FILTER_SET_BYPASS_MODE,&enable);
        #endif
    }
}
