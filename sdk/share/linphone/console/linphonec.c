...
static void linphonec_call_paused_notify_callback_default(char* addr, int id)
{
}
void (*linphonec_call_paused_notify_callback)(char* addr, int id) = linphonec_call_paused_notify_callback_default;
...
static void linphonec_call_state_changed(LinphoneCore *lc, LinphoneCall *call, LinphoneCallState st, const char *msg){
    ...
    switch(st){
        ...
        case LinphoneCallPaused:
            ...
            addr[len] = '\0';
            linphonec_call_paused_notify_callback(addr, (int)id);
            break;
...
linphonec_parse_cmdline(int argc, char **argv)
{
    ...
    while (arg_num < argc)
    {
		...
		else if (strncmp ("-CD", argv[arg_num], 3) == 0)
        {
