    ...
    struct details::modded_request *mr)
{
	...
	else if (0x0 != encoding &&
			(0 == strncasecmp(
			MHD_JSON_FORMAT,
			encoding,
			strlen(MHD_JSON_FORMAT))))
    {
        mr->pp = NULL;
    }
	...
	{
		RawData = new char[11]; // fit addressbook size
		strcpy(RawData, strrchr(mr->standardized_url->c_str(), '/'));
		if (!(strcmp(RawData, "/firmware")) ||
			!(strcmp(RawData, "/resource")) ||
			!(strcmp(RawData, "/addressbook")) ||
			!(strcmp(RawData, "/cardlist")))
		{
			printf("\nGoing to malloc... size are: %ul\n", sizeof(string));
			if (!filebuf)
				filebuf = (char*)malloc(FIRMWARESIZE);
			printf("\nMalloc done... Filebuf: %x\n",filebuf);
			if (filebuf == NULL)
			{
				printf("\nMalloc failed!...\n");
				return MHD_NO;
			}
			printf("filebuf is not NULL\n");
		}
		else
		{
			RawData = NULL;
    ...
    size_t *upload_data_size, struct details::modded_request *mr)
{
    if (0 == *upload_data_size)
	{
		if (RawData != NULL)
		{
			mr->dhr->set_firmware_pointer(filebuf);
			mr->dhr->set_firmware_size(calcu_size);
			calcu_size = 0;
			RawData = NULL;
		}
        return complete_request(connection, mr, version, method);
	...
	else
	{
		if (filebuf != NULL)
		{
			memcpy(filebuf + calcu_size, upload_data, *upload_data_size);
			calcu_size += *upload_data_size;
			printf("recv file %d bytes\n", calcu_size);
		}
	}
    if (mr->pp != NULL)
