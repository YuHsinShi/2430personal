...
/****************************************************************************
 * MACRO define of cytma448
 ****************************************************************************/
#define TP_I2C_DEVICE_ID       (0x50>>1)
...
#ifdef	CFG_TOUCH_BUTTON
#define TP_CYTMA448_BUTTON_REPORT_BY_COORDINATION
#endif
/****************************************
 *
 ***************************************/
...
static int _tpDoInitProgram_vendor(void)
{
	...
	//try to read TP controller if cytma448
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
	...
	if( buf[0] && (buf[0]!=0x02) )	
	{
		...
		if( buf[0] && (buf[0]!=0x02) )	
		{
			...
			//printf("\n		");
			//if(c>27)	for(i=27; i<37; i++)	printf("%02x ",buf[i]);
			printf("] r=%x\n",i2cret);		
		}
...
/*
ref: CYTMA448 Technical Reference Manual (TRM) "touch report" at page:142
		...
		for(i = 0; i < loopNum; i++)
		{
			...
			if(smp->x > 800)
			{
			    if( (smp->x > 880) && (smp->x < 980) )
			    {
			        ...
			        buf[2] = 0x03  ;    //mark as button report(follow cytma448's datasheet P144)
			    }
...
#ifdef	TP_CYTMA448_BUTTON_REPORT_BY_COORDINATION
		buf[5] = tmpKey;
		if( !tmpKey && gTpKeypadValue)  buf[2] = 0x03;  //set button report ID if gTpKeypadValue != 0
		//printf("GotKeyEv:%x\n",tmpKey);
#endif
 ...
 */ 
static int cytma448_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops cytma448_ops =
{
	cytma448_read,
};

TSAPI struct tslib_module_info *cytma448_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(cytma448_mod_init);
#endif
