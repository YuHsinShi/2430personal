 ...
 ****************************************************************************/
#define TP_I2C_DEVICE_ID       (0x32>>1)
...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpIntPullEnable     = (char)1;
...
static void _tpDoPowerOnSeq_vendor(void)
{
	...
	*/
    ...
    //2.set "Reset pin" output HIGH for 5ms
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	{		
		...
		for(i = 0; i < loopNum; i++)
		{
			...
			smp->id = (unsigned int)buf[1]; //touch id
			smp->finger = loopNum;
			smp->x = (int)((((unsigned int)buf[3+5*i]<<8)&0x0F00) | ((unsigned int)buf[2+5*i]));
			smp->y = (int)((((unsigned int)buf[5+5*i]<<8)&0x0F00) | ((unsigned int)buf[4+5*i]));
