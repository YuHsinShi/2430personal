...
#if 1
	printf("bsearch::m = %s, c = %x\n",module, countof(tslib_modules));

	for (i = 0; i < countof(tslib_modules); i++) {
		if (!strcmp(tslib_modules[i].name, module)) 
		{
		    printf("i=%x, [%s, %s]\n", i, tslib_modules[i].name, module);
