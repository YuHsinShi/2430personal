...
static unsigned int AudioRecordGetAvailBufferLength()
{
    unsigned int  rd  = I2S_AD32_GET_RP();
    unsigned int  wr  = I2S_AD32_GET_WP();
    //printf("r ptr %d w ptr %d \n",rd,wr);
    return getRecordLen(I2S_AD32_GET_RP(), I2S_AD32_GET_WP(), gRecordI2sBufSize);
}

// get audio data
static int AudioRecordGetI2sData(unsigned int nLength)
{
    unsigned int nTemp;

    nTemp                       = I2S_AD32_GET_RP();
    audioRecordMgr.nReadSize    = (int)nLength;
//    printf("%d %d %d \n",nTemp,I2S_AD32_GET_WP(),nLength);
    memcpy(audioRecordMgr.ptRecordBuf, gpRecordI2sBuf + nTemp, nLength);
    nTemp                       += nLength;
    if (nTemp >= gRecordI2sBufSize)
        nTemp = 0;
    I2S_AD32_SET_RP(nTemp);
}
...
{
    const MMP_WCHAR *tempPtr    = strPtr;
    MMP_INT         result      = 0;
...
void 
smtkAudioMgrSetInitRecord(SMTK_AUDIO_RECORD_PARAM *audiomgr_record,char *filename)
{
    audiomgr_record->nChannel   = 1;
    audiomgr_record->nSampleRate= 16000;
    audiomgr_record->nType      = SMTK_AUDIO_RECORD_TYPE_WAVE_FORMAT_PCM16;
    audiomgr_record->ptPathName = filename;
    //audiomgr_record->nFlag      = SMTK_AUDIO_RECORD_PARAM_FLAG_LIMIT_TIME;
    //audiomgr_record->nLimitTime = 9*1000;
}
...
MMP_INT
smtkAudioMgrRecordSetLevel(
    MMP_INT nReclevel)
{
    MMP_UINT        result = 0;
    i2s_ADC_set_rec_volperc(nReclevel);
    return result;
}

MMP_INT
smtkAudioMgrRecordGetLevel(
    void)
{
    unsigned int cLevlel = 0;
    cLevlel = i2s_ADC_get_current_volstep();
    return cLevlel;
}
