...
#include "api_f.h"
    ...
    #define IS_BIG_ENDIAN    1 //big
#elif (CFG_CHIP_FAMILY == 9860)
    #define TOINT(n)   n
    #define TOSHORT(n) n

    #define IS_BIG_ENDIAN    0 //little
#endif
...
int mainProcessorExcuteI2SInitDac()
{
    ...
    iteAudioGetAttrib(ITE_AUDIO_I2S_INIT, &nTemp);
    if(nTemp==0){
        nTemp=1;
...
//    initDAC(I2SBuf, nChannels, nSampeRate, nBufferLength, 1,0);
    ...
    {
        audioFilterProcessInit(nChannels, nSampeRate);
        iteAudioSetAttrib(ITE_AUDIO_I2S_INIT, &nTemp);
        return 0;
    }
#endif
    ...
    spec.is_big_endian            = IS_BIG_ENDIAN;
    ...
    unsigned short nRegisterStatus)
{
    ...
    switch (nTemp)
    {
        ...
        {
            // nResult = mainProcessorExcutePcmIdx();
#ifdef CFG_AUDIO_FILTER_ENABLE
            nResult = audioFilterProcessSoundRead();
#endif
            break;
        }
