...
static inline void perspective_map(const double *c,
...
static inline void perspective_unmap(const double *c,
...
static void threshold(struct quirc *q)
{
    ...
    for (y = 0; y < q->h; y++)
    {
        ...
        if (!row_average)
            continue;
        memset(row_average, 0, q->w * sizeof(int));//sizeof(row_average));
