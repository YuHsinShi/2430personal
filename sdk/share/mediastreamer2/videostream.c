...
//#define SUPPORT_SENDER_PLAYBACK_RECORD 1
...
void video_stream_free (VideoStream * stream) {
	...
	/*destroy encoder source berfoer decoder*/
	media_stream_free(&stream->ms);
	...
	if (stream->video_recv_ticker != NULL)
        ms_ticker_destroy (stream->video_recv_ticker);
...
int video_stream_start_with_source (VideoStream *stream, RtpProfile *profile, const char *rem_rtp_ip, int rem_rtp_port,
...
#ifdef SUPPORT_SENDER_PLAYBACK_RECORD		
    ...
    ms_ticker_set_name(stream->ms.sessions.ticker,"Video Send MSTicker");
#ifdef SUPPORT_SENDER_PLAYBACK_RECORD    
	if (stream->dir==VideoStreamSendOnly){
...
#endif	
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamRecvOnly){
		/*send/recv use different task to improve performance*/
		stream->video_recv_ticker= ms_ticker_new();
    	ms_ticker_set_name(stream->video_recv_ticker,"Video Recv MSTicker");
    	stream->av_recorder.ticker = ms_ticker_new();
    	ms_ticker_set_name(stream->av_recorder.ticker,"MKVRecoder MSTicker");
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamRecvOnly){
		if (stream->ms.rtprecv && !video_from_ipcam)
        	ms_ticker_attach (stream->video_recv_ticker, stream->ms.rtprecv);
    	else if (stream->sourceforIPCamStreaming)
        	ms_ticker_attach (stream->video_recv_ticker, stream->sourceforIPCamStreaming);
    	if (stream->av_recorder.video_input)
        	ms_ticker_attach (stream->av_recorder.ticker, stream->av_recorder.video_input);
...
video_stream_stop (VideoStream * stream)
{
    ...
    if (stream->ms.sessions.ticker){
        ...
        if (stream->ms.rtprecv)
            ms_ticker_detach(stream->video_recv_ticker,stream->ms.rtprecv);
        else if (stream->sourceforIPCamStreaming)
            ms_ticker_detach(stream->video_recv_ticker,stream->sourceforIPCamStreaming);
        if (stream->av_recorder.video_input)
