...
static void filter_process(MSFilter *f){
    ...
    int frameSize = (CFG_AUDIO_SAMPLING_RATE*obj->ptime)/1000;
	uint16_t inputBuffer[frameSize];
	...
	while(ms_bufferizer_get_avail(obj->bufferizer)>=sizeof(inputBuffer)){
		uint16_t totalPacketDataLength = 0;
		uint8_t bitStreamLength = 0;
		outputMessage = allocb(frameSize/8,0); /* output bitStream is 80 bits long * number of samples */
		...
		for (bufferIndex=0; (bufferIndex<frameSize/8) && (bitStreamLength!=2); bufferIndex+=10) {
