...
static void volume_noise_gate_process(Volume *v , float energy, mblk_t *om){
	static int counter;
	//static float tgain = 0;  /* start with floorgain */
	int nsamples=((om->b_wptr-om->b_rptr)/2);
	...
	if (!(++counter % 10))
		ms_debug("%d: nglevel=%f, energy=%f, v->ng_tgain=%f, ng_gain=%f",
				          (v->peer!=NULL)?1:0, energy, v->energy, v->ng_tgain, v->ng_gain);
}

static void apply_smooth(Volume *v,mblk_t *om){
    int16_t *sample;
    int i;
    for (sample=(int16_t*)om->b_rptr;sample<(int16_t*)om->b_wptr;++sample){
        ...
        int order = O_NUM - 1;
        for(i = 0;i<order;i++){
            v->smoothw[order-i] = v->smoothw[order-(i+1)]; 
        }
        v->smoothw[0] = *sample;

        for(i = 0;i<=order;i++){
            sum+=(v->smoothw[i]>>(order-i));
        }
        sum>>=1;
...
static int volume_enable_smooth(MSFilter *f, void *arg){
	Volume *v=(Volume*)f->data;
    int i;
	v->smooth_enable=*(int*)arg;
    for(i=0;i<O_NUM;i++) v->smoothw[i]=0;
	return 0;
}
...
static void volume_process(MSFilter *f){
	...
	if (v->peer!=NULL){
        ...
        while(ms_bufferizer_get_avail(v->buffer)>=nbytes){
			...
			update_energy((int16_t*)om->b_rptr, nbytes/2, v);
			target_gain = v->static_gain;
		...
		/*while((m=ms_queue_get(f->inputs[0]))!=NULL){
            ...
            if (v->agc_enabled) 
                target_gain/= volume_agc_process(v, m);
            
			apply_gain(v, m, target_gain);
            printf("%f %f %f\n",target_gain,v->ng_gain,v->ng_threshold);
...
static MSFilterMethod methods[]={
    ...
    {	MS_VOLUME_ENABLE_SMOOTH, volume_enable_smooth },
	{	0			,	NULL			}
};
