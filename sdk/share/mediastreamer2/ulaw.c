...
static void ulaw_enc_process(MSFilter *obj){
    ...
    size_of_pcm = 320*frame_per_packet;
#else
...
static void ulaw_dec_process(MSFilter *obj){
    ...
    while((m=ms_queue_get(obj->inputs[0]))!=NULL){
        ...
        msgpullup(m,-1);
#ifdef CFG_IOT_ENABLE
        int audioBufferSize = m->b_wptr-m->b_rptr;
        int count = (audioBufferSize / 172) + 1;
        int allocSize = count * 160 * 2;
        int index = 0;
        unsigned char* o_end = 0;
        o=allocb(allocSize,0);
        o_end = o->b_wptr + allocSize;
        mblk_meta_copy(m, o);

        for (index = 0; index < count; index++)
        {
            unsigned char* m_next_end = 0;

            if (index)
            {
                m->b_rptr += 12;
            }
            m_next_end = m->b_rptr + 160;
            for(;m->b_rptr<m_next_end;m->b_rptr++,o->b_wptr+=2){
