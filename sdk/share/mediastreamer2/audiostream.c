...
void _msvoleme_default_set(MSFilter *f,bool_t use_agc,bool_t use_ng,double coef,double vgain);
...
static void audio_stream_free(AudioStream *stream) {
    ...
    if (stream->voladj!=NULL) ms_filter_destroy(stream->voladj);
    ms_free(stream);
}
...
static void audio_mkv_rec_graph_link(MSConnectionHelper *h,AudioStream *stream,LinphoneAudioStreamFlow select_flow){
    if (stream->teeforrecord) {
        ...
        // [udprecv]--pin0--[teeforrecord]--pin0--
        //                                --pin1--[itcsink]
        if(select_flow==AudioFromSoundRead && stream->voladj){
            ms_filter_link(stream->teeforrecord,1,stream->voladj,0);
            ms_filter_link(stream->voladj,0,stream->itcsink,0);
        }else{
...
static void audio_mkv_rec_graph_unlink(MSConnectionHelper *h,AudioStream *stream,LinphoneAudioStreamFlow select_flow){
    if (stream->teeforrecord!=NULL) {
        ms_connection_helper_unlink(h,stream->teeforrecord,0,0);
        if(select_flow==AudioFromSoundRead && stream->voladj){
            ms_filter_unlink(stream->teeforrecord,1,stream->voladj,0);
            ms_filter_unlink(stream->voladj,0,stream->itcsink,0);            
        }else{
    ...
    if (stream->itcsink){
        ...
        if (pt){
            ...
            if(select_flow == AudioFromSoundRead)
                pinfmt.fmt=ms_factory_get_audio_format(ms_factory_get_fallback(),"A_PCML",pt->clock_rate,pt->channels,NULL);    
...
#if defined(CFG_AUDIO_CODEC_OPUS) && defined(CFG_AUDIO_CODEC_G729) && defined(CFG_AUDIO_CODEC_G722) && (defined(CFG_AUDIO_CODEC_G726_ALAW) || defined(CFG_AUDIO_CODEC_G726_ULAW))
#define ENABLE_CHANGE_CODEC
static char codec_type[16] = "opus";
char *get_audio_stream_codec_type(char *mime)
{
    printf("get audio stream codec type : %s\n", codec_type);
    if (strlen(codec_type) == 0) return mime;
    else return codec_type;
}
#endif
void set_audio_stream_codec_type(int id)
{
#ifdef ENABLE_CHANGE_CODEC
    switch (id)
    {
    case 0:
        strcpy(codec_type, "opus");
        break;
    case 1:
        strcpy(codec_type, "G722");
        break;
    case 2:
        strcpy(codec_type, "G726-32");
        break;
    case 3:
        strcpy(codec_type, "G729");
        break;
    }
#endif
}
...
int audio_stream_start_full(AudioStream *stream, RtpProfile *profile, const char *rem_rtp_ip,int rem_rtp_port,
    ...
    if(stream->use_codec){
#ifdef ENABLE_CHANGE_CODEC
        stream->ms.encoder = ms_filter_create_encoder(get_audio_stream_codec_type(pt->mime_type));
        stream->ms.decoder = ms_filter_create_decoder(get_audio_stream_codec_type(pt->mime_type));
#else
...
//    MSFilterDesc *ec_desc=ms_filter_lookup_by_name("MSOslec");
    ...
    if (ms_factory_lookup_filter_by_id(ms_factory_get_fallback(), MS_MKV_RECORDER_ID)){
        ...
        if (stream->itcsink)
            ...
            stream->voladj      =ms_filter_new(MS_VOLUME_ID);
            if(stream->voladj)
                _msvoleme_default_set(stream->voladj,1,1,0.85,4.0);
    }
#endif
...
#ifdef TWO_WAY_AUDIORECORD
            if(select_flow == AudioFromSoundRead)
                audio_mkv_rec_graph_unlink(&h,stream,select_flow);
#endif
#ifdef PURE_WAV_RECORD
            if(select_flow == AudioFromSoundRead)
...
void _msvoleme_default_set(MSFilter *f,bool_t use_agc,bool_t use_ng,double coef,double vgain){
    float ng_thres = 0.025;
    float ng_coef=coef;
    float gain=vgain;
    printf("%d %d %f %f\n",use_agc,use_ng,ng_coef,gain);
    if (use_ng){
        ms_filter_call_method(f,MS_VOLUME_ENABLE_NOISE_GATE,&use_ng);
        ms_filter_call_method(f,MS_VOLUME_SET_MAX_ENERGY,&ng_coef);
    }
    if(use_agc)
        ms_filter_call_method(f,MS_VOLUME_ENABLE_AGC,&use_agc);
    if(gain)
        ms_filter_call_method(f,MS_VOLUME_SET_GAIN,&gain); 
    if(ng_thres!=-1)
        ms_filter_call_method(f,MS_VOLUME_SET_NOISE_GATE_THRESHOLD,&ng_thres);
}
...
MS2_PUBLIC VoiceMemoRecordStream *voice_memo_start_record (const char * file, MSSndCard *sndcard)
{
    ...
    stream->msvolume=ms_filter_new(MS_VOLUME_ID);
    stream->msdenoise=ms_filter_new(MS_FILTER_DENOISE_ID);
    ...
    if (stream->msdenoise)
        ms_connection_helper_link(&h,stream->msdenoise,0,0);
    if (stream->msvolume)
        ms_connection_helper_link(&h,stream->msvolume,0,0);
    if (stream->ms.encoder)
        ms_connection_helper_link(&h,stream->ms.encoder,0,0);
    ...
    if(stream->msvolume)
        _msvoleme_default_set(stream->msvolume,1,1,0.95,4.0);//agc,ng,ngcoef,gain
...
MS2_PUBLIC void voice_memo_stop_record (VoiceMemoRecordStream * stream)
{
    ...
    if(stream->msdenoise)
        ms_connection_helper_unlink(&h,stream->msdenoise,0,0);
    if(stream->msvolume)
        ms_connection_helper_unlink(&h,stream->msvolume,0,0);
    if(stream->ms.encoder)
        ms_connection_helper_unlink(&h,stream->ms.encoder,0,0);
...
MS2_PUBLIC void voic_mix_pcm_ctrl(AudioStream* stream,const char *file,double mixlevel,int times)
{
    if(stream->mixvoice){
        ms_filter_call_method(stream->mixvoice,MS_MIXVOISE_SET_REPEAT,&times);  
        if(mixlevel>=0) ms_filter_call_method(stream->mixvoice,MS_MIXVOISE_SET_LEVEL,&mixlevel);   
        if(file!=NULL && times!=0) ms_filter_call_method(stream->mixvoice,MS_WAV_FILE_OPEN,(void*)file); 
    }
}
/*
