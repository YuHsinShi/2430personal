...
static void capvpmem_process(MSFilter *f)
{
    ...
    {
#if 0//defined(CFG_SENSOR_ENABLE)
        mblk_t  *output = allocb(sizeof(AVFrame), 0);
        AVFrame oneframe;
        oneframe.width       = SENSOR_WIDTH;
        oneframe.height      = SENSOR_HEIGHT;
        oneframe.linesize[0] = SENSOR_WIDTH;
        oneframe.linesize[1] = SENSOR_WIDTH;
        oneframe.data[0]     = c_ptr->ISPBUFFERINFO->pISPBufAdrY[ISPWRINDEX];
        oneframe.data[1]     = c_ptr->ISPBUFFERINFO->pISPBufAdrU[ISPWRINDEX];
        oneframe.data[2]     = c_ptr->ISPBUFFERINFO->pISPBufAdrU[ISPWRINDEX];
...
#if 0
static int capvpmem_set_preview(MSFilter *f, void *data)
{
	CAPFILTER *c_ptr = (CAPFILTER *)f->data;
...
MS_FILTER_DESC_EXPORT(ms_capvpmem_desc)