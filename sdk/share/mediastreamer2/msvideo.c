...
#ifdef ENABLE_GENERAL_PLAYER
	err=sws_scale(fctx->ctx,(const uint8_t * const*)src,src_strides,0,fctx->src_h,dst,dst_strides);
#endif
#else
#ifdef ENABLE_GENERAL_PLAYER	
	err=sws_scale(fctx->ctx,(uint8_t **)src,src_strides,0,fctx->src_h,dst,dst_strides);
#endif
#endif
	if (err<0) return -1;
	return 0;
