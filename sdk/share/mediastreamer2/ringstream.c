...
static void linphonec_call_mp3_ring(const char* ring,int interval,MSSndCard *sndcard);
...
static void linphonec_call_mp3_ring(const char* ring,int interval,MSSndCard *sndcard)
{
    ...
    nResult = ms_snd_card_get_level(sndcard,MS_SND_CARD_PLAYBACK);
    smtkAudioMgrSetVolume(nResult);
    // check mp3 ring
...
#ifdef CFG_MP3_RING 
    linphonec_call_mp3_ring(file,interval,sndcard);
    if(AL_Local_player_http_handle)
...
/*RingStream * ring_start_audiomgr(const char *file, int interval, MSSndCard *sndcard){   
    ...
    stream->source=ms_filter_new(MS_FILE_AUDIOMGR_ID);
    if(file){
        interval = 200;
        ms_filter_call_method(stream->source,MS_AUDIOMGR_SET_LOOP,&interval);
        ms_filter_call_method(stream->source,MS_AUDIOMGR_OPEN,(void*)file);
    ...
    if(channels==2){
        int output=2;
        stream->gendtmf=ms_filter_new(MS_CHANNEL_ADAPTER_ID);
        ms_filter_call_method(stream->gendtmf,MS_FILTER_SET_NCHANNELS,&channels);
        ms_filter_call_method(stream->gendtmf,MS_CHANNEL_ADAPTER_SET_OUTPUT_NCHANNELS,&output);
        Castor3snd_reinit_for_diff_rate(rate,16,output);
    }
    stream->sndwrite=ms_snd_card_create_writer(sndcard);
    ms_filter_call_method(stream->source,MS_FILTER_SET_PEER,&stream->sndwrite);
    
    stream->ticker=ms_ticker_new();
    ms_ticker_set_name(stream->ticker,"Audio (ring) MSTicker_audiomgr");
    ms_connection_helper_start(&h);
    ms_connection_helper_link(&h,stream->source,-1,0);
    if(stream->gendtmf)
        ms_connection_helper_link(&h,stream->gendtmf,0,0);
...
/*
void ring_stop_audiomgr(RingStream *stream){   
    MSConnectionHelper h;
    
    if(stream->source)
        ms_filter_call_method_noarg(stream->source,MS_AUDIOMGR_CLOSE);
...
void ring_mix_pcm_ctrl(RingStream* stream,char *file,double mixlevel,int times)
{
    if(stream->mixer){
        ms_filter_call_method(stream->mixer,MS_MIXVOISE_SET_REPEAT,&times);  
        if(mixlevel>=0) ms_filter_call_method(stream->mixer,MS_MIXVOISE_SET_LEVEL,&mixlevel);   
        if(file!=NULL && times!=0) ms_filter_call_method(stream->mixer,MS_WAV_FILE_OPEN,(void*)file); 
    }
}
/*
