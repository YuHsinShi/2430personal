...
muxer_t *muxer_new_muxer(int type,stream_t *stream, int cache_data_in_file){
    ...
    muxer->stream = stream;
    muxer->cache_data_in_file = cache_data_in_file;
    switch (type) {
#ifdef CONFIG_FFMPEG
...
void muxer_free_muxer(muxer_t *m)
{
    ...
    for (num = 0; num < m->avih.dwStreams; ++num)
    {
        muxer_stream_t *str = m->streams[num];
        if (str->muxer->cont_free_stream)
            muxer_free_stream(str->muxer, str);
    }
    if (m->idx)
        free(m->idx);
    if (m->priv)
...
int muxer_write_chunk(muxer_stream_t *s, size_t len, unsigned int flags, double dts, double pts) {
    ...
    if (s->muxer->muxbuf_skip_buffer) {
        result = s->muxer->cont_write_chunk(s, len, flags, dts, pts);
    }
    else {
