...
typedef struct demux_packet {
    ...
    double endpts;
    double stream_pts;
    offset_t pos;  // position in index (AVI) or file (MPG)
...
typedef struct {
    ...
    offset_t pos;            // position in the input stream (file)
    offset_t dpos;           // position in the demuxed stream
...
typedef struct demuxer {
    ...
    offset_t filepos; // input stream current pos.
    stream_t *stream;
...
static inline void *realloc_struct(void *ptr, size_t nmemb, size_t size) {
    ...
    return av_realloc_f(ptr, nmemb, size);
}
...
void ds_read_packet(demux_stream_t *ds, stream_t *stream, int len, double pts, offset_t pos, int flags);

int demux_fill_buffer(demuxer_t *demux,demux_stream_t *ds);
int ds_fill_buffer(demux_stream_t *ds);

static inline offset_t ds_tell(demux_stream_t *ds){
...
int ds_parse(demux_stream_t *sh, uint8_t **buffer, int *len, double pts, offset_t pos);
