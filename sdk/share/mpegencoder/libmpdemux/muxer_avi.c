...
static muxer_stream_t* avifile_new_stream(muxer_t *muxer,int type)
{
    ...
    si->riffofssize=16;
    si->riffofs=calloc((si->riffofssize+1), sizeof(offset_t));
    memset(si->riffofs, 0, sizeof(offset_t)*si->riffofssize);
...
static void avifile_free_stream(muxer_stream_t* ms)
{
    if (ms->bih)
        free(ms->bih);
    if (ms->wf)
    ...
    if (ms->priv)
    {
        struct avi_stream_info* ai = (struct avi_stream_info*)ms->priv;
        if (ai->idx)
            free(ai->idx);
        if (ai->riffofs)
            free(ai->riffofs);
...
static void write_avi_chunk(stream_t *stream,unsigned int id,int len,void* data){
    ...
    if(len>0){
        if(data){
            ...
            // JUNK
            static char avi_junk_data[20];
            if(len&1) ++len; // padding
            while(len>0){
                int l=sizeof(avi_junk_data);
                if(l>len) l=len;
...
static void avifile_odml_new_riff(muxer_t *muxer)
{
    ...
    if (vsi->riffofspos>=vsi->riffofssize) {
        vsi->riffofssize+=16;
        vsi->riffofs=realloc_struct(vsi->riffofs,(vsi->riffofssize+1),sizeof(offset_t));
    }
...
static void avifile_write_header(muxer_t *muxer){
    ...
    for (i = 0; i < MUXER_MAX_STREAMS && muxer->streams[i]; ++i) {
        muxer_stream_t *s = muxer->streams[i];  // loop – stream
        if (s->type == MUXER_TYPE_AUDIO && muxer->audio_delay_fix > 0.0) {
            // rate / scale 单 – samples 计
            // 丁 * rate / scale 单 frames 计
...
static void avifile_write_chunk(muxer_stream_t *s,size_t len,unsigned int flags, double dts, double pts){
    ...
    if (s->type == MUXER_TYPE_VIDEO && !s->h.dwSuggestedBufferSize) {
        // 秨﹍矪瞶 video stream 
        offset_t pos=stream_tell(muxer->stream);
        stream_seek(muxer->stream, 0);
        avifile_write_header(muxer);    // 秨繷糶header
...
static void avifile_odml_write_index(muxer_t *muxer){
    ...
    for (i=0; i<muxer->avih.dwStreams; i++) {
        ...
        for (j=0; j<si->superidxpos; j++) {
            offset_t start = si->idx[idxpos].ofs;
            int duration;
...
int muxer_init_muxer_avi(muxer_t *muxer){
    ...
    muxer->cont_free_stream         = &avifile_free_stream;
    return 1;
}
