...
static unsigned int  pcmDumpIdx;
...
short gFrame[FLAC_AVG_FRAME_SIZE];
...
void AudioPluginAPI(int nType)
{
    ...
    switch (nType)
    {
        ...
        case SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX:
                pBuf = (unsigned char*)gtAudioPluginBuffer;
                memcpy(&pBuf[0], &pcmDumpIdx, sizeof(int));
            break;
...
static void FillWriteBuffer(int nPcmBytes) {
    ...
    if (nPcmBytes > 0) {
        ...
        if (pcmWriteIdx+nPcmBytes >= OUTBUF_SIZE) {
            if(OUTBUF_SIZE-pcmWriteIdx) {
                memcpy(&gOutBuf[pcmWriteIdx],&gFrame[0],OUTBUF_SIZE-pcmWriteIdx);
            #ifdef CFG_CPU_WB
                ithFlushDCacheRange((void*)(gOutBuf + pcmWriteIdx), OUTBUF_SIZE-pcmWriteIdx);
            #endif
            }
            memcpy(&gOutBuf[0],&gFrame[OUTBUF_SIZE-pcmWriteIdx],nPcmBytes-(OUTBUF_SIZE-pcmWriteIdx));
        #ifdef CFG_CPU_WB
            ithFlushDCacheRange((void*)(gOutBuf), nPcmBytes-(OUTBUF_SIZE-pcmWriteIdx));
            ithFlushMemBuffer();
        #endif
        ...
        #ifdef CFG_CPU_WB
            ithFlushDCacheRange((void*)(gOutBuf + pcmWriteIdx), nPcmBytes);
            ithFlushMemBuffer();
        #endif
        ...
        #if defined(__OPENRTOS__)
        if(isMusicCodecDump()) {
            if(pcmDumpIdx <= pcmWriteIdx)
            {
                i = nPcmBytes;
                while(i > (nPcmBytes>>1)) {
                    pcmDumpIdx += (nPcmBytes>>1);
                    i -= (nPcmBytes>>1);
                    AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
                }
                if(i) {
                    pcmDumpIdx += i;
                    AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
            ...
            {
                uint32_t szsec0 = OUTBUF_SIZE - pcmDumpIdx;
                uint32_t szsec1 = nPcmBytes - szsec0;
                i = szsec0;
                while(i > (nPcmBytes>>1)) {
                    pcmDumpIdx += (nPcmBytes>>1);
                    if(pcmDumpIdx == OUTBUF_SIZE) pcmDumpIdx = 0;
                    i -= (nPcmBytes>>1);
                    AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
                }
                if(i) {
            ...
            // AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
            // for (i=0;i<100000;i++);
            while (CODEC_I2S_GET_OUT_FREE_LEN() < (CODEC_I2S_GET_OUT_BUF_LEN())/2)
                AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
            return;
        } else
    ...
    do
    {
        ...
        if (len < FLAC_AVG_FRAME_SIZE && !isSTOP()) {
            #if defined(__OPENRTOS__)
                //PalSleep(2);
                for (i=0;i<100000;i++);
            #else
