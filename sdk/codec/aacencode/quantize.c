...
static Word16 quantizeSingleLine(const Word16 gain, const Word32 absSpectrum)
{
  ...
  if (absSpectrum) {
    ...
    minusFinalExp = ((e << 2) + gain)*3 + ((INT_BITS-1) << 4);
                          ...
                          Word16 *quaSpectrum)
{
  ...
  Word32 mdctSpeL;
  //const Word16 *pquat;
  Word16 pquat0, pquat1, pquat2, pquat3;
    /* gain&3 */

  //pquat = quantBorders[m];
  pquat0 = quantBorders[m][0];
  pquat1 = quantBorders[m][1];
  pquat2 = quantBorders[m][2];
  pquat3 = quantBorders[m][3];
                   ...
                   Word16  gain)
{
    ...
    Word16 pquat0, pquat1, pquat2, pquat3;
	Word16 repquat0, repquat1, repquat2;
    /* gain&3 */
    ...
    pquat2 = quantBorders[m][2];
    pquat3 = quantBorders[m][3];
    repquat0 = quantRecon[m][0];
    repquat1 = quantRecon[m][1];
    repquat2 = quantRecon[m][2];
    ...
    if(g2 < 0 && g >= 0) {
      ...
      for(line=0; line<sfbWidth; line++) {
          if (spec[line]) {
              ...
              if (saShft < pquat0) {
                  distSingle = (MUL(saShft,saShft)) >> g2;
              }
              else {
                  if(saShft >= pquat3) {
					  ...
					  if (saShft < pquat1) {
						  diff = saShft - repquat0;
						  distSingle = (MUL(diff,diff)) >> g2;
					  }
                      else {
						  if (saShft < pquat2) {
							  diff = saShft - repquat1;
							  distSingle = (MUL(diff,diff)) >> g2;
						  }
						  else {
                              diff = saShft - repquat2;
                              distSingle = (MUL(diff,diff)) >> g2;
						  }					  	
                      }
                  }
