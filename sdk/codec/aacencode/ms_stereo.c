...
void MsStereoProcessing(Word32       *sfbEnergyLeft,
  ...
  for (sfb=0; sfb<sfbCnt; sfb+=sfbPerGroup) {
    for (sfboffs=0;sfboffs<maxSfbPerGroup;sfboffs++) {
	  ...
	  nrgL = Div(thrL << shift, nrgL << shift);
      nrgR = max(nrgR,thrR) + 1;
      shift = norm_l(nrgR);
	  //nrgR = Div_32(thrR << shift, nrgR << shift);
	  nrgR = Div(thrR << shift, nrgR << shift);
	  ...
	  nrgL = Div(minThreshold << shift, nrgL << shift);

      nrgR = max(nrgR,minThreshold) + 1;
      shift = norm_l(nrgR);
	  //nrgR = Div_32(minThreshold << shift, nrgR << shift);
	  nrgR = Div(minThreshold << shift, nrgR << shift);

      pnms = fixmul(nrgL, nrgR);

      temp = Div((pnlr + 1), ((pnms >> 8) + 1));
