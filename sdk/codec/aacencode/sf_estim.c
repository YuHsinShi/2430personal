    ...
    for (sfbOffs=0; sfbOffs<psyOutChan->sfbCnt; sfbOffs+=psyOutChan->sfbPerGroup){
        for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
            ...
            if (psyOutChan->sfbEnergy[i] > psyOutChan->sfbThreshold[i]) {
                ...
                for (j=sfbw; j; j--) {
                    accu += formfac_sqrt(L_abs(*mdctSpec)); 
					mdctSpec++;
                }
                ...
                if(avgFormFactor)
				    ...
				    sfbNRelevantLines[i] = Div(accu, avgFormFactor);
                else
...
static Word16 calcSingleSpecPe(Word16 scf, Word16 sfbConstPePart, Word16 nLines)
{
	...
	if (ldRatio < PE_C1_8) {
      ...
      asm volatile("l.mac %0, %1" : : "%r"(nLines), "r"(ldRatio));
      asm volatile("l.macrc %0, 0" : "=r"(specPe) );
      asm volatile("l.mac %0, %1" : : "%r"(specPe), "r"(PE_SCALE));
      asm volatile("l.macrc %0, 14" : "=r"(specPe) );
...
#if defined(__OR32__)
            asm volatile("l.mac %0, %1" : : "%r"(sfbNRelevantLines[sfb]), "r"((ldRatioNew - ldRatioOld)));
#else
...
#if defined(__OR32__)
      asm volatile("l.macrc %0, 0" : "=r"(specPeDiff) );
      asm volatile("l.mac %0, %1" : : "%r"(specPeDiff), "r"(PE_SCALE));
      asm volatile("l.macrc %0, 14" : "=r"(specPeDiff) );
                            ...
                            Word16          *sfbNRelevantLines)
{
    ...
    for (i=0; i<psyOutChan->sfbCnt; i++) {
        ...
        if ((maxSpec > 0) && (energy > thresh)) {
            ...
            /* -20 = 4*log2(6.75) - 32 */
#if defined(__OR32__)
            asm volatile("l.mac %0, %1" : : "%r"((thresholdPart - energyPart - 20)), "r"(SCALE_ESTIMATE_COEF));
            asm volatile("l.macrc %0, 15" : "=r"(scfInt) );
