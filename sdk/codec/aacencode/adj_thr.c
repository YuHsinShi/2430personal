      ...
      for(sfbGrp = 0;sfbGrp < psyOutChan->sfbCnt;sfbGrp+= psyOutChan->sfbPerGroup){
		...
		for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
          ...
          if (sfbEn > avgEn && avgEn > 0) {
			...
			tmpMinSnr = Div(L_mpy_ls(avgEn, minSnrLimit) << shift, sfbEn << shift );
          ...
          if ((sfbEn < (avgEn >> 1)) && (sfbEn > 0)) {
            ...
            if(minSnrEn < sfbEn) {
              ...
              tmpMinSnr = Div( minSnrEn << shift, sfbEn<<shift);
            }
            else {
              tmpMinSnr = MAX_16;
            }
      ...
      if (psyOutElement->toolsInfo.msMask[sfb]) {
        ...
        if(maxThr >= sfbEnM) {
		  ...
		  psyOutChanM->sfbMinSnr[sfb] = min(max(psyOutChanM->sfbMinSnr[sfb],
		      ...
		      round16(Div(maxThr<<shift, sfbEnM << shift))), minSnrLimit);
        }
        ...
        if(maxThr >= sfbEnS) {
          ...
          psyOutChanS->sfbMinSnr[sfb] = min(max(psyOutChanS->sfbMinSnr[sfb],
		      ...
		      round16(Div(maxThr << shift, sfbEnS << shift))), minSnrLimit);
        }
    ...
    for(sfbGrp = 0;sfbGrp < psyOutChan->sfbCnt;sfbGrp+= psyOutChan->sfbPerGroup){
	  ...
	  for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
        ...
        if (((*pahFlag < AH_ACTIVE) || (deltaPe > 0)) && (redThrExp > 0) ) {
          ...
          *psfbPeFactors = (*psfbNActiveLines) * Div(0x7fffffff, redThrExp);
          ...
          sfbThr = psyOutChan->sfbThreshold[sfbGrp+sfb];
            ...
            */
            ...
            if(thrFactor > sfbThr) {
			  ...
			  sfbThrReduced = Div( sfbThr << shift, thrFactor<<shift );
            }
            else {
                           ...
                           const Word16     desiredPe)
{
  ...
  if (actPe > desiredPe) {
    ...
    if(ahCnt) {
	  ...
	  iahCnt = Div( 1 << shift, ahCnt << shift );
                                ...
                                MINSNR_ADAPT_PARAM *msaParam)
{
  ...
  if ((100 * redPe < 115 * desiredPe)) {
    ...
    Word16 desiredPe105 = Div((105 * desiredPe), 100);
                          ...
                          const Word16 maxBitSave)
{
  ...
  if(clipHigh-clipLow)
  bitsave = (maxBitSave - Div(((maxBitSave-minBitSave)*(fillLevel-clipLow)),
                              (clipHigh-clipLow)));
                           ...
                           const Word16 maxBitSpend)
{
  ...
  if(clipHigh-clipLow)
  bitspend = (minBitSpend + Div(((maxBitSpend - minBitSpend)*(fillLevel - clipLow)),
                                (clipHigh-clipLow)));
                           ...
                           Word16      *peMax)
{
  ...
  Word16 diff;
  Word16 minDiff = extract_l(Div(currPe, 6));
  minFacHi = 30;
  maxFacHi = 100;
  minFacLo = 14;
  maxFacLo = 7;
  ...
  if (diff > 0) {
    *peMin = *peMin + Div((diff * minFacHi), 100);
    *peMax = *peMax + Div((diff * maxFacHi), 100);
  } else {
    diff = *peMin - currPe;

    if (diff > 0) {
      *peMin = *peMin - Div((diff * minFacLo), 100);
      *peMax = *peMax - Div((diff * maxFacLo), 100);
    } else {
      *peMin = *peMin + Div(((currPe - *peMin) * minFacHi), 100);
      *peMax = *peMax - Div(((*peMax - currPe) * maxFacLo), 100);
    }
  }
  ...
  if ((*peMax - *peMin) < minDiff) {
    ...
    *peMax = currPe + Div((partHi * minDiff), (partLo + partHi));
    *peMin = currPe - Div((partLo * minDiff), (partLo + partHi));
    *peMin = max(0, *peMin);
  }
}
                                ...
                                ATS_ELEMENT   *adjThrChan)
{
  ...
  fillLevel = extract_l(Div((100* bitresBits), maxBitresBits));
  ...
  if(adjThrChan->peMax != adjThrChan->peMin)
    bitresFac = (100 - bitSave) + extract_l(Div(((bitSpend + bitSave) * (pex - adjThrChan->peMin)),
				    (adjThrChan->peMax - adjThrChan->peMin)));
  else
	bitresFac = 0x7fff;

  bitresFac = min(bitresFac,
                    (100-30 + extract_l(Div((100 * bitresBits), avgBits))));
                ...
                Word32         chBitrate)
{
  ...
  /* parameters for bitres control */
  atsElem->peMin = extract_l(Div((80*meanPe), 100));
  atsElem->peMax = extract_l(Div((120*meanPe), 100));

  /* additional pe offset to correct pe2bits for low bitrates */
  atsElem->peOffset = 0;
  if (chBitrate < 32000) {
    atsElem->peOffset = max(50, (100 - extract_l(Div((100 * chBitrate), 32000))));
  }
                             ...
                             const Word16 bitsLast)
{
  ...
  if ((bitsLast > 0) &&
    ...
    {
      Word16 newFac = Div((100 * peLast), peBitsLast);
      /* dead zone */

      if (newFac < 100) {
        newFac = min(Div((110 * newFac), 100), 100);
        newFac = max(newFac, 85);
      }
      else {
        newFac = max(Div((90 * newFac), 100), 100);
      ...
      if ((*correctionFac < 100 && newFac < *correctionFac) ||
        ...
        *correctionFac = Div((85 * *correctionFac + 15 * newFac), 100);
      else
        *correctionFac = Div((70 * *correctionFac + 30 * newFac), 100);
                      ...
                      const Word16     maxBitFac)
{
  ...
  /* desired pe */
  grantedPe = Div((bitFactor * bits2pe(avgBits)), 100);
  ...
  grantedPeCorr = Div((grantedPe * AdjThrStateElement->peCorrectionFactor), 100);
  ...
  for (ch=0; ch<nChannels; ch++) {
    ...
    if (peOffsDiff > 0) {
      ...
      chBitDistribution[ch] = chBitDistribution[ch] +
		  Div((temp * peData.peChannelData[ch].pe), peOffsDiff);
    }
  }
