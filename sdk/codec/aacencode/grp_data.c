...
#if defined(__OR32__)
      groupedSfbOffset[i] = offset + (Word16)MUL((Word32)sfbOffset[sfb] ,(Word32)groupLen[grp]);
#else
...
#if defined(__OR32__)
    offset += MUL((Word32)groupLen[grp],FRAME_LEN_SHORT);
#else    
...
#if defined(__OR32__)
        Word16 lineOffset = (Word16)MUL(FRAME_LEN_SHORT , (wnd + j));
#else        
