                                     ...
                                     const Word16 *bandStartOffset) /*!< table of band borders */
{
  ...
  lineNumber = (extract_l(fixmul((bandStartOffset[numOfBands] << 2), Div(freq << shift,fs << shift))) + 1) >> 1;
    ...
    for( sfb = lpcStartBand; sfb < lpcStopBand; sfb++) {
      ...
      if( tmp2 > 0) {
        ...
        if(tmp > INT_BITS_SCAL)
        {
            ...
            tmp = Div( INT_BITS_SCAL << shift, tmp << shift );
        }
        else
        {
            tmp = 0x7fffffff;
...
void AutoCorrelation(const Word16		 input[],
  ...
  accu = 0;
    ...
    for(j=0; j<isamples; j++) {
        asm volatile("l.mac %0, %1" : : "%r"(input[j]), "r"(input[j]));    
    }
    asm volatile("l.macrc %0, 9" : "=r"(accu) );
...
#if defined(__OR32__)
    for(j=0; j<isamples; j++) {
        asm volatile("l.mac %0, %1" : : "%r"(input[j]), "r"(input[j+i]));    
    }
    asm volatile("l.macrc %0, 9" : "=r"(accu) );
    corr[i] = accu;
#else
...
static Word16 AutoToParcor(Word32 workBuffer[], Word32 reflCoeff[], Word16 numOfCoeff) {
  ...
  for(i=0; i<numOfCoeff; i++) {
	...
	workBuffer0 = Div(1 << shift, workBuffer[0] << shift);
