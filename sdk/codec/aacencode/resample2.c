  ...
  else
  {
    for(dst_index=0; dst_index < dst_size; dst_index++)
    {
        ...
        if(sample_index < 0)
        {
            ...
            for(i=0; i<c->filter_length; i++) {
                asm volatile ("l.mac %0,%1" : : "%r"((int)src[FASTABS(sample_index + i) % src_size] ), "r"((FELEM2)filter[i]));
            }
            asm volatile ("l.macrc %0, 0" : "=r"(val));
    #else
            for(i=0; i<c->filter_length; i++) {
                val += MULSHIFT(src[FFABS(sample_index + i) % src_size],(FELEM2)filter[i],0);
            }
    #endif            
#endif
        }
        ...
        else
        {
            for(i=0; i<c->filter_length; i++)
            {
                ...
                val += src[sample_index + i] * (FELEM2)filter[i];
#else                
                //val += MULSHIFT(src[sample_index + i],(FELEM2)filter[i],0);
                for(i=0; i<c->filter_length; i++) {
                    asm volatile ("l.mac %0,%1" : : "%r"((int)src[(sample_index + i)] ), "r"((FELEM2)filter[i]));
                }
                asm volatile ("l.macrc %0, 0" : "=r"(val));
#endif
            }
