                    ...
                    Word32       *bandEnergySum)
{
  ...
  for (i=0; i<numBands; i++) {
    ...
    for (j=bandOffset[i]; j<bandOffset[i+1]; j++){
        asm volatile("l.mac %0, %1" : : "%r"(mdctSpectrum[j]), "r"(mdctSpectrum[j]));
    }
    asm volatile("l.macrc %0, 32" : "=r"(accu) );    
#else    
    for (j=bandOffset[i]; j<bandOffset[i+1]; j++)
                      ...
                      Word32       *bandEnergySideSum)
{
  ...
  for(i=0; i<numBands; i++) {
    ...
    for (j=bandOffset[i]; j<bandOffset[i+1]; j++) {
      Word32 specm, specs;
      specm = mdctSpectrumLeft[j] + mdctSpectrumRight[j];
      specs = mdctSpectrumLeft[j] - mdctSpectrumRight[j];
      accuMid = L_add(accuMid, MUL_33(specm, specm));
      accuSide = L_add(accuSide, MUL_33(specs, specs));
    }
