...
#ifdef STATIC_MEMORY
  static Word16 gQuantSpec[MAX_CHANNELS*FRAME_LEN_LONG];
  static Word16 gScf[MAX_CHANNELS*MAX_GROUPED_SFB];
  static UWord16 gMaxValueInSfb[MAX_CHANNELS*MAX_GROUPED_SFB];
#endif
			  ...
			  Word16 ancillaryDataBytes)
{
  ...
  for (ch = 0; ch < nChannels; ch++) {
    ...
    maxChDynBits[ch] = extract_l(Div((chBitDistribution[ch] * maxDynBits), 1000));
  }
                              ...
                              QC_OUT* qcOut)
{
  ...
  nFullFillElem = Div(max((qcOut->totFillBits - 1), 0), maxFillElemBits) * maxFillElemBits;
