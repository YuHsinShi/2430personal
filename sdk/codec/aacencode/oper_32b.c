...
#if defined(__OR32__)
    Word32 result;
    asm volatile("l.div %0, %1, %2" : "=r"(result) : "%r"(L_num), "r"(denom));
    return result;
#else
...
Word32 pow2_xy(Word32 x, Word32 y)
{
  ...
  temp2 = MUL(POW2_TABLE_SIZE,fPart);
  res = pow2Table[temp2/y] >> iPart;
  ...
  fPart = tmp2 - iPart*y;
  iPart = min(iPart,INT_BITS-1);
  res = pow2Table[Div((POW2_TABLE_SIZE*fPart),y)] >> iPart;
