...
#ifdef CFG_CPU_WB
        ithFlushDCacheRange((void*)(pcmWriteBuf + pcmWriteIdx), frameSamps*sizeof(short));
        ithFlushMemBuffer();
#endif
...
//            #endif //defined(MP3_DUMP_PCM) && defined(__FREERTOS__)        
        ...
        if(isMusicCodecDump()) {
            ...
            // else
            // {
                // uint32_t szsec0 = I2SBUFSIZE - pcmDumpIdx;
                // uint32_t szsec1 = (frameSamps * sizeof(short)) - szsec0;
                // i = szsec0;
                // while(i > 4608) {
                    // pcmDumpIdx += 4608;
                    // if(pcmDumpIdx == I2SBUFSIZE) pcmDumpIdx = 0;
                    // i -= 4608;
                    // AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
                // }
                // if(i) {
            ...
            // int i;
            // for (i=0;i<100000;i++);
            while (CODEC_I2S_GET_OUT_FREE_LEN() < (CODEC_I2S_GET_OUT_BUF_LEN())/4)
                AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PCMIDX);
        } else
