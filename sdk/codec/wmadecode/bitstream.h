...
#if defined(CPU_ARM) || defined(_ARM_)
...
void avpriv_put_string(PutBitContext * pbc, char *s, int put_zero);
...
#    ifdef __GNUC__
#    define unaligned(x)                                \
static __inline uint##x##_t unaligned##x(const void *v) { \
    struct Unaligned {                                  \
        uint##x##_t i;                                  \
 ...
 */
static __inline int get_vlc2(GetBitContext *s, VLC_TYPE *table,
