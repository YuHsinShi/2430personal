...
#define INIT_BACKGROUND_W 50

//The initial background height %
#define INIT_BACKGROUND_H 50
		...
		if (desta == 255)
		{
			...
			else
			{
				if (widget->angle == 0)
				{
					if (widget->transformType == ITU_TRANSFORM_NONE)
					{//Bless
						if ((icon->totalframe > 0) && icon->working)
						{
							if (icon->frame < icon->totalframe)
							{
								...
								ituStretchBlt(dest, destx, desty, rect->width, rect->height, icon->bsurf, 0, 0, icon->bsurf->width, icon->bsurf->height);
						}
						//else
						//{
						...
						if (icon->working)
						{
							...
							if (icon->totalframe == 0)
							{
								...
								emptycolor.alpha = icon->widget.color.alpha;
								emptycolor.red = icon->widget.color.red;
								emptycolor.green = icon->widget.color.green;
								emptycolor.blue = icon->widget.color.blue;
								if (icon->bsurf)
									ituSurfaceRelease(icon->bsurf);

								icon->bsurf = ituCreateSurface(icon->widget.rect.width, icon->widget.rect.height, 0, ITU_RGB565, NULL, 0);
								ituColorFill(icon->bsurf, 0, 0, icon->widget.rect.width, icon->widget.rect.height, &emptycolor);
								ituBitBlt(dest, destx, desty, icon->bsurf->width, icon->bsurf->height, icon->bsurf, 0, 0);
							}
...
void ituDragIconLoad(ITUDragIcon* dragicon, uint32_t base)
{
	...
	dragicon->pw = INIT_BACKGROUND_W;
	dragicon->ph = INIT_BACKGROUND_H;
