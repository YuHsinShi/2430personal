...
void ituShadowDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    if (shadow->target)
    {
        ...
        if (surf)
        {
            ...
            {
            #ifdef _WIN32
                ITUSurface* surf2 = ituCreateSurface(rect->width, rect->height, 0, ITU_A8, NULL, 0);
            #else
                ITUSurface* surf2 = ituCreateSurface(rect->width, rect->height, 0, ITU_MASK_A8, NULL, 0);
            #endif
                if (surf2)
                {
                    ITUColor startColor, endColor;
                    ...
                    ituSetColor(&startColor, widget->color.alpha, widget->color.alpha, widget->color.alpha, widget->color.alpha);
                    ituSetColor(&endColor, 0, 0, 0, 0);
                    ituGradientFill(surf2, 0, 0, rect->width, rect->height, &startColor, &endColor, ITU_GF_VERTICAL);
                    shadow->maskSurf = surf2;
                }
            }
