...
wchar_t* convertArabic(wchar_t* normal)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (CharacterMapContains(current))
        {
            ...
            if (onDigitalStart >= 0)
            {
                int count = (writeCount - onDigitalStart) / 2;
                int j;
                for (j = 0; j < count; ++j)
                {
                    unsigned short tmp = shaped[onDigitalStart + j];
                    shaped[onDigitalStart + j] = shaped[writeCount - 1 - j];
                    shaped[writeCount - 1 - j] = tmp;
                }
                onDigitalStart = -1;
            }
        ...
        {
            if (iswalnum(current) && onDigitalStart == -1)
                onDigitalStart = writeCount;
...
{
    int len = wcslen(normal);
    wchar_t* shaped = (wchar_t*)malloc(sizeof(wchar_t) * (len + 1));
    int writeCount = 0;
    int i, d;
    wchar_t* toReturn = NULL;
    int onDigitalStart = -1;
