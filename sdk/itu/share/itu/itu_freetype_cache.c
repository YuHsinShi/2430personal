...
static int _AddDeferDestroyData(void* pData, void* destroyFunc)
{
...
int ituFtGetDeferDestroyData(void** pData)
{
    ...
    if (gTmpDeferDestroyCount)
    {
        ...
        memcpy(ptDestroyDataArray->pDestroyData, gptTmpDeferDestroyArray, sizeof(DeferDestroyData)*gTmpDeferDestroyCount);
        *pData = ptDestroyDataArray;
        //printf("destroy count: %u\n", gTmpDeferDestroyCount);
...
#ifdef ITU_ENABLE_DEFER_DESTROY
            _AddDeferDestroyData(glyph, FT_Done_Glyph);
#else
            FT_Done_Glyph(glyph);
#endif
            prev_gindex = gindex;
...
#ifdef ITU_ENABLE_DEFER_DESTROY
                        _AddDeferDestroyData(node, _FreeFTCNode);
#else
