...
namespace GUIDesigner
{
    public class CodeGenerator
    {
        ...
        public void GenerateLayerCode(string layerPath, HashSet<string> widgetNames, HashSet<string> functions)
        {
            using (StreamWriter outputFile = new StreamWriter(layerPath))
            {
                ...
                outputFile.WriteLine("/* widgets:");

                foreach (var widgetName in widgetNames)
                {
                    if (!string.IsNullOrEmpty(widgetName))
                    {
                        outputFile.WriteLine(widgetName);
                    }
                }
