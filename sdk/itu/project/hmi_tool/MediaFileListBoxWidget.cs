...
{
    class MediaFileListBoxWidget : ListBox, IWidget
    {
        public MediaFileListBoxWidget()
        {
            ...
            this.FontChanged += new EventHandler(MediaFileListBoxWidget_FontChanged);
        ...
        void MediaFileListBoxWidget_FontChanged(object sender, EventArgs e)
        {
        ...
        {
            ITUMediaFileListBox mflistbox = new ITUMediaFileListBox();

            mflistbox.type = ITUWidgetType.ITU_MEDIAFILELISTBOX;
            mflistbox.name = this.Name;
            mflistbox.flags |= this.TabStop ? ITU.ITU_TAPSTOP : 0;

            PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(this);
            mflistbox.visible = (bool)properties["Visible"].GetValue(this);

            mflistbox.active = false;
            mflistbox.dirty = false;
            mflistbox.alpha = 255;
            mflistbox.tabIndex = this.TabIndex;
            mflistbox.rect.x = this.Location.X;
            mflistbox.rect.y = this.Location.Y;
            mflistbox.rect.width = this.Size.Width;
            mflistbox.rect.height = this.Size.Height;
            mflistbox.color.alpha = this.BackAlpha;
            mflistbox.color.red = this.BackColor.R;
            mflistbox.color.green = this.BackColor.G;
            mflistbox.color.blue = this.BackColor.B;
            mflistbox.bound.x = 0;
            mflistbox.bound.y = 0;
            mflistbox.bound.width = 0;
            mflistbox.bound.height = 0;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                node.widget = text;
                mflistbox.items.Add(node);
            }
            mflistbox.pageIndexName       = this.PageIndexTarget;
            mflistbox.pageCountName       = this.PageCountTarget;
            mflistbox.focusColor.alpha    = this.FocusColor.A;
            mflistbox.focusColor.red      = this.FocusColor.R;
            mflistbox.focusColor.green    = this.FocusColor.G;
            mflistbox.focusColor.blue     = this.FocusColor.B;
            mflistbox.focusFontColor.alpha = this.FocusFontColor.A;
            mflistbox.focusFontColor.red = this.FocusFontColor.R;
            mflistbox.focusFontColor.green = this.FocusFontColor.G;
            mflistbox.focusFontColor.blue = this.FocusFontColor.B;
            mflistbox.orgFontColor.alpha = this.ForeColor.A;
            mflistbox.orgFontColor.red = this.ForeColor.R;
            mflistbox.orgFontColor.green = this.ForeColor.G;
            mflistbox.orgFontColor.blue = this.ForeColor.B;
            mflistbox.readFontColor.alpha = this.ReadFontColor.A;
            mflistbox.readFontColor.red = this.ReadFontColor.R;
            mflistbox.readFontColor.green = this.ReadFontColor.G;
            mflistbox.readFontColor.blue = this.ReadFontColor.B;
            mflistbox.scrollDelay         = this.ScrollDelay;
            mflistbox.stopDelay           = this.StopDelay;
            mflistbox.path                = this.Path;
            mflistbox.extensions          = this.Extensions;

            mflistbox.actions[0].action = (ITUActionType)this.Action01.Action;
            mflistbox.actions[0].ev = (ITUEvent)this.Action01.Event;
            mflistbox.actions[0].target = this.Action01.Target;
            mflistbox.actions[0].param = this.Action01.Parameter;
            mflistbox.actions[1].action = (ITUActionType)this.Action02.Action;
            mflistbox.actions[1].ev = (ITUEvent)this.Action02.Event;
            mflistbox.actions[1].target = this.Action02.Target;
            mflistbox.actions[1].param = this.Action02.Parameter;
            mflistbox.actions[2].action = (ITUActionType)this.Action03.Action;
            mflistbox.actions[2].ev = (ITUEvent)this.Action03.Event;
            mflistbox.actions[2].target = this.Action03.Target;
            mflistbox.actions[2].param = this.Action03.Parameter;
            mflistbox.actions[3].action = (ITUActionType)this.Action04.Action;
            mflistbox.actions[3].ev = (ITUEvent)this.Action04.Event;
            mflistbox.actions[3].target = this.Action04.Target;
            mflistbox.actions[3].param = this.Action04.Parameter;
            mflistbox.actions[4].action = (ITUActionType)this.Action05.Action;
            mflistbox.actions[4].ev = (ITUEvent)this.Action05.Event;
            mflistbox.actions[4].target = this.Action05.Target;
            mflistbox.actions[4].param = this.Action05.Parameter;
            mflistbox.actions[5].action = (ITUActionType)this.Action06.Action;
            mflistbox.actions[5].ev = (ITUEvent)this.Action06.Event;
            mflistbox.actions[5].target = this.Action06.Target;
            mflistbox.actions[5].param = this.Action06.Parameter;
            mflistbox.actions[6].action = (ITUActionType)this.Action07.Action;
            mflistbox.actions[6].ev = (ITUEvent)this.Action07.Event;
            mflistbox.actions[6].target = this.Action07.Target;
            mflistbox.actions[6].param = this.Action07.Parameter;
            mflistbox.actions[7].action = (ITUActionType)this.Action08.Action;
            mflistbox.actions[7].ev = (ITUEvent)this.Action08.Event;
            mflistbox.actions[7].target = this.Action08.Target;
            mflistbox.actions[7].param = this.Action08.Parameter;
            mflistbox.actions[8].action = (ITUActionType)this.Action09.Action;
            mflistbox.actions[8].ev = (ITUEvent)this.Action09.Event;
            mflistbox.actions[8].target = this.Action09.Target;
            mflistbox.actions[8].param = this.Action09.Parameter;
            mflistbox.actions[9].action = (ITUActionType)this.Action10.Action;
            mflistbox.actions[9].ev = (ITUEvent)this.Action10.Event;
            mflistbox.actions[9].target = this.Action10.Target;
            mflistbox.actions[9].param = this.Action10.Parameter;
            mflistbox.actions[10].action = (ITUActionType)this.Action11.Action;
            mflistbox.actions[10].ev = (ITUEvent)this.Action11.Event;
            mflistbox.actions[10].target = this.Action11.Target;
            mflistbox.actions[10].param = this.Action11.Parameter;
            mflistbox.actions[11].action = (ITUActionType)this.Action12.Action;
            mflistbox.actions[11].ev = (ITUEvent)this.Action12.Event;
            mflistbox.actions[11].target = this.Action12.Target;
            mflistbox.actions[11].param = this.Action12.Parameter;
            mflistbox.actions[12].action = (ITUActionType)this.Action13.Action;
            mflistbox.actions[12].ev = (ITUEvent)this.Action13.Event;
            mflistbox.actions[12].target = this.Action13.Target;
            mflistbox.actions[12].param = this.Action13.Parameter;
            mflistbox.actions[13].action = (ITUActionType)this.Action14.Action;
            mflistbox.actions[13].ev = (ITUEvent)this.Action14.Event;
            mflistbox.actions[13].target = this.Action14.Target;
            mflistbox.actions[13].param = this.Action14.Parameter;
            mflistbox.actions[14].action = (ITUActionType)this.Action15.Action;
            mflistbox.actions[14].ev = (ITUEvent)this.Action15.Event;
            mflistbox.actions[14].target = this.Action15.Target;
            mflistbox.actions[14].param = this.Action15.Parameter;
