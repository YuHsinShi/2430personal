...
namespace GUIDesigner
{
    class ButtonWidget : Button, IWidget
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            TextFormatFlags textflag;
            uint fontWidth = (uint)Math.Round(this.Font.Size);
            int ix = 0, iy = 0;
            
            //default painting for system drawing
            base.OnPaint(e);

            switch (this.TextAlign)
            {
                case ContentAlignment.BottomLeft:
                    textflag = TextFormatFlags.Bottom | TextFormatFlags.Left;
                    break;

                case ContentAlignment.MiddleLeft:
                    textflag = TextFormatFlags.VerticalCenter | TextFormatFlags.Left;
                    break;

                case ContentAlignment.TopLeft:
                    textflag = TextFormatFlags.Top | TextFormatFlags.Left;
                    break;

                case ContentAlignment.BottomCenter:
                    textflag = TextFormatFlags.Bottom | TextFormatFlags.HorizontalCenter;
                    break;

                case ContentAlignment.MiddleCenter:
                    textflag = TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter;
                    break;

                case ContentAlignment.TopCenter:
                    textflag = TextFormatFlags.Top | TextFormatFlags.HorizontalCenter;
                    break;

                case ContentAlignment.BottomRight:
                    textflag = TextFormatFlags.Bottom | TextFormatFlags.Right;
                    break;

                case ContentAlignment.MiddleRight:
                    textflag = TextFormatFlags.VerticalCenter | TextFormatFlags.Right;
                    break;

                case ContentAlignment.TopRight:
                    textflag = TextFormatFlags.Top | TextFormatFlags.Right;
                    break;
            ...
            if (image != null)
            {
                //rectimage = new Rectangle(0, (this.Height - image.Height) / 2, image.Width, image.Height);

                if (this.Stretch)
                {
                    rectimage = new Rectangle(0, 0, this.Width, this.Height);
                }
                else
                {
                    ...
                    {
                        case ContentAlignment.BottomLeft:
                            rectimage = new Rectangle(0, (this.Height - image.Height), image.Width, image.Height);
                            break;
                        ...
                        case ContentAlignment.TopLeft:
                            rectimage = new Rectangle(0, 0, image.Width, image.Height);
                            break;

                        case ContentAlignment.BottomCenter:
                            rectimage = new Rectangle((this.Width - image.Width) / 2, (this.Height - image.Height), image.Width, image.Height);
                            break;

                        case ContentAlignment.MiddleCenter:
                            rectimage = new Rectangle((this.Width - image.Width) / 2, (this.Height - image.Height) / 2, image.Width, image.Height);
                            break;

                        case ContentAlignment.TopCenter:
                            rectimage = new Rectangle((this.Width - image.Width) / 2, 0, image.Width, image.Height);
                            break;

                        case ContentAlignment.BottomRight:
                            rectimage = new Rectangle((this.Width - image.Width), (this.Height - image.Height), image.Width, image.Height);
                            break;

                        case ContentAlignment.MiddleRight:
                            rectimage = new Rectangle((this.Width - image.Width), (this.Height - image.Height) / 2, image.Width, image.Height);
                            break;

                        case ContentAlignment.TopRight:
                            rectimage = new Rectangle((this.Width - image.Width), 0, image.Width, image.Height);
                            break;
                ...
                g.DrawImage(image, rectimage, 0, 0, image.Width, image.Height, GraphicsUnit.Pixel);
            }
            else
                g.FillRectangle(brush, rect);

            if (text.Length > 0)
                TextRenderer.DrawText(g, text, this.Font, rect, this.ForeColor, textflag);
        }

        private ContentAlignment imagepos = ContentAlignment.TopLeft;
        public ContentAlignment ImageAlignPos
        {
            get
            {
                return imagepos;
            ...
            set
            {
                if ((this.BackgroundImage != null) && (!this.Stretch))
                {
                    if ((this.BackgroundImage.Width >= this.Width) && (this.BackgroundImage.Height >= this.Height))
                        imagepos = ContentAlignment.TopLeft;
                    else if (this.BackgroundImage.Width >= this.Width)
                        imagepos = ContentAlignment.MiddleLeft;
                    else if (this.BackgroundImage.Height >= this.Height)
                        imagepos = ContentAlignment.TopCenter;
                    else
                        imagepos = value;
                }
                else
        ...
        public override Image BackgroundImage
        {
            ...
            set
            {
                if (previewImage == ButtonImageType.Default && this.DefaultImage == null)
        ...
        public Image FocusImage
        {
            ...
            set
            {
                ...
                if (previewImage == ButtonImageType.Focus)
        ...
        public Image PressImage
        {
            ...
            set
            {
                ...
                if (previewImage == ButtonImageType.Press)
                    base.BackgroundImage = value;
            }
        }

        private Image defaultImage;
        [LocalizedDescription("DefaultImage", typeof(WidgetManual))]
        [Editor(typeof(ImageLocationEditor), typeof(UITypeEditor))]
        [DefaultValue(typeof(Image), null)]
        public Image DefaultImage
        {
            ...
            set
            {
                ...
                if (previewImage == ButtonImageType.Default)
                    base.BackgroundImage = value;
            }
        }
        ...
        private ButtonImageType previewImage = ButtonImageType.Default;
        public ButtonImageType PreviewImage
        {
            ...
            set
            {
                switch (value)
                {
                    case ButtonImageType.Focus:
                        base.BackgroundImage = this.FocusImage;
                        break;

                    case ButtonImageType.Press:
                        base.BackgroundImage = this.PressImage;
                        break;
        ...
        public int MouseLongPressDelay { get; set; }
        public int MouseUpDelay { get; set; }
        ...
        public ITUWidget CreateITUWidget()
        {
            ...
            {
                btn.staticSurf = ITU.CreateSurfaceNode(this.DefaultImage as Bitmap, this.PixelFormat, this.Compress, this.External, this.Dither);
            }
            if (this.External)
            ...
            {
                btn.staticFocusSurf = ITU.CreateSurfaceNode(this.FocusImage as Bitmap, this.PixelFormat, this.Compress, this.External, this.Dither);
            }
            ...
            {
                btn.staticPressSurf = ITU.CreateSurfaceNode(this.PressImage as Bitmap, this.PixelFormat, this.Compress, this.External, this.Dither);
            }            
            btn.pressed = 0;
            btn.mouseLongPressDelay = this.MouseLongPressDelay;
            btn.mouseUpDelay = this.MouseUpDelay;

            //for imagepos
            switch (this.imagepos)
            {
                case ContentAlignment.BottomLeft:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_BOTTOM_LEFT;
                    break;

                case ContentAlignment.MiddleLeft:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_MIDDLE_LEFT;
                    break;

                case ContentAlignment.TopLeft:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_TOP_LEFT;
                    break;

                case ContentAlignment.BottomCenter:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_BOTTOM_CENTER;
                    break;

                case ContentAlignment.MiddleCenter:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_MIDDLE_CENTER;
                    break;

                case ContentAlignment.TopCenter:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_TOP_CENTER;
                    break;

                case ContentAlignment.BottomRight:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_BOTTOM_RIGHT;
                    break;

                case ContentAlignment.MiddleRight:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_MIDDLE_RIGHT;
                    break;

                case ContentAlignment.TopRight:
                    btn.imagepos = (int)ITULayout.ITU_LAYOUT_TOP_RIGHT;
                    break;
