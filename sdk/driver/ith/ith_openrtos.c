...
void ithInit(void)
{
    ...
    #endif
    #if !defined(CFG_USB1_ENABLE) && (CFG_CHIP_FAMILY == 970)
    ...
    //NGPL WIFI power enable for 9860
...
#if (CFG_CHIP_FAMILY == 970 || CFG_CHIP_FAMILY == 9860)
    uint32_t arm_base = (CacheID926() == ITH_ARM_CACHE_TYPE) ? 0xD8300000 : 0xD8400000;
  //workaround 970 CPU power saving mode problem
    ithWriteRegMaskA(arm_base, 0x00010000, 0x00010000);
#endif
    ...
    if (CacheID926() == ITH_ARMLITE_CACHE_TYPE)
        ithWriteRegA(arm_base, 0x1);
    else
        ithWriteRegMaskA(arm_base, 0x00000000, 0x00010000);
#endif
