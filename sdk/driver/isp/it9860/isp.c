...
//#define     WEIGHT_NUM 9
    ...
    #if defined(CFG_LCD_ENABLE) && defined(CFG_LCD_HEIGHT) && defined(CFG_LCD_WIDTH) && (CFG_LCD_HEIGHT > 0) && (CFG_LCD_WIDTH > 0)
...
static const ISP_YUV_TO_RGB gtIspYuv2RgbColorMatrix_ForBypassMode =
{
    // _11      // _12      // _13
...
static const ISP_RGB_TO_YUV gtIspRgb2YuvColorMatrix_ForBypassMode =
{
    // _11      // _12      // _13
    0x0100,     0x0000,     0x0000,
    // _21      // _22      // _23
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->OutInfo.EnableCCFun)
    {
    ...
    MMP_FLOAT  weightMatrix[][ISP_SCALE_TAP])
{
    ...
    else
    {
        ...
        case 5:     fscale   = 0.7f;
                    rcosbeta = 0.0f;   break;
        case 4:     fscale   = 0.7f;
                    rcosbeta = 0.32f;  break;
        case 3:     fscale   = 0.7f;
                    rcosbeta = 0.47f;  break;
        case 2:     fscale = 0.83f;    break;
        case 1:     fscale = 0.91f;    break;
        case 0:     fscale = 1.0f;     break;
        case -1:    fscale = 1.5f;     break;
        case -2:    fscale = 2.0f;     break;
        case -3:    fscale = 2.5f;     break;
        case -4:    fscale = 3.0f;     break;
        default:    fscale = 1.0f;     break;
        }
    }
    ...
    else if (method == 11)
    {
        ...
        for (i = 0; i <= (tapSize >> 1); i++)
        {
            ...
            for (j = 0; j < taps; j++)
            {
                W[j] = rcos( ((MMP_FLOAT)point + i / ((MMP_FLOAT)tapSize)) / (MMP_FLOAT)scale, rcosbeta);
                point--;
                WW  += W[j];
            }
    ...
    ISP_DEVICE ptDev)
{
    ...
    else if (pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO_BYPASS ||
            pISPctxt->ispMode == MMP_ISP_MODE_SHOW_IMAGE_BYPASS ||
            pISPctxt->ispMode == MMP_ISP_MODE_TRANSFORM_BYPASS)
    {
        pISPctxt->YUV2RGBFun = &gtIspYuv2RgbColorMatrix_ForBypassMode;
        pISPctxt->RGB2YUVFun = &gtIspRgb2YuvColorMatrix_ForBypassMode;
    }
    else
    {
    ...
    MMP_BOOL  useyuvformat)
{
    ...
    V_brightness[0][0]     = V_brightness[1][0] = V_brightness[2][0] = (MMP_FLOAT)brightness;
    ...
    while (i < 12)
    {
        if (i < 9)
        {
            ...
            {
                ui16CCRegTemp[i] = (MMP_UINT16) (CC_MatrixCurrect.M_color_correction[0][i] * -256);
                colorMatrix[i] = (MMP_FLOAT)((MMP_FLOAT)ui16CCRegTemp[i] / 256.0f) * -1.0f;
            }
        }
        else
        {
            //Write Vector (Vector values are signed numers)
            if (CC_MatrixCurrect.V_color_correction[i - 9][0] >= 0)
            {
                ui16CCRegTemp[i] = (MMP_UINT16) (CC_MatrixCurrect.V_color_correction[i - 9][0]);
                colorMatrix[i]   = (MMP_FLOAT)((MMP_FLOAT)ui16CCRegTemp[i]);
            }
            else
            {
                ui16CCRegTemp[i] = (MMP_UINT16) (0 - CC_MatrixCurrect.V_color_correction[i - 9][0]);
                colorMatrix[i]   = (MMP_FLOAT)((MMP_FLOAT)ui16CCRegTemp[i] * -1);
            }
        }
    ...
    MMP_ISP_INFORMAT format)
{
    ...
    case MMP_ISP_IN_YUV420:
        ...
        pISPctxt->OutInfo.EnableCSFun         = MMP_TRUE;
        pISPctxt->InInfo.EnableRdRqDoubleLine = (pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO || pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO_BYPASS);
        break;
    ...
    if (pISPctxt->EnableYUVProcess ||
            pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO_BYPASS ||
            pISPctxt->ispMode == MMP_ISP_MODE_SHOW_IMAGE_BYPASS ||
            pISPctxt->ispMode == MMP_ISP_MODE_TRANSFORM_BYPASS)
        pISPctxt->OutInfo.EnableCSFun = MMP_FALSE;
    ...
    ISP_DEVICE ptDev)
{
    ...
    MMP_UINT16  j                                            = 0;
    MMP_UINT16  index                                        = 0;
    MMP_FLOAT   WeightMat[ISP_SCALE_TAP_SIZE][ISP_SCALE_TAP] = {0};
    ...
    // Initial Video Weight Matrix for Sharpness Tuning
    if (pISPctxt->OutInfo.EnableCCFun)
    {
    ...
    ISP_DEVICE ptDev)
{
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_EngineMode || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutParameter)
    {
        ...
        if (pISPctxt->ispCore == MMP_ISP_CORE_0)
        {
            ISP_SetOutputRGBAlpha_Reg(pISPctxt, pISPctxt->OutInfo.EnableOutputRGBAlpha, pISPctxt->OutInfo.OutputRGBAlpha);
        }
#endif
