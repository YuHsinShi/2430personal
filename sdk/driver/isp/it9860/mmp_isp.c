 ...
 */
//=============================================================================
pthread_mutex_t ISP_CORE_0_MUTEX = PTHREAD_MUTEX_INITIALIZER;
...
//=============================================================================
/**
 * Power-down the ISP h/w engine.
 *
 * @return ISP_SUCCESS if succeed, error codes of ISP_ERR_ERROR otherwise.
 * @attention Both of the two isp cores in the isp engine will be powered down.
 *
    ...
    MMP_ISP_MODE ispMode)
{
    ...
    case MMP_ISP_MODE_PLAY_VIDEO_BYPASS:
    ...
    case MMP_ISP_MODE_SHOW_IMAGE_BYPASS:
        pISPctxt->EngineMode.EnableBlockMode      = MMP_FALSE;
        pISPctxt->EngineMode.EnableReadMemoryMode = MMP_TRUE;
        pISPctxt->EngineMode.EnableJPEGDECODE     = MMP_TRUE;
        pISPctxt->EngineMode.EnableRawDataMode    = MMP_FALSE;
    ...
    case MMP_ISP_MODE_TRANSFORM_BYPASS:
...
#if (CFG_CHIP_FAMILY == 9860)
    case MMP_ISP_OUTPUT_RGB_ALPHA:
        pISPctxt->OutInfo.EnableOutputRGBAlpha  = MMP_TRUE;
        pISPctxt->UpdateFlags                  |= ISP_FLAGS_UPDATE_OutParameter;
        break;
#endif
...
#if (CFG_CHIP_FAMILY == 9860)
    case MMP_ISP_OUTPUT_RGB_ALPHA:
        pISPctxt->OutInfo.EnableOutputRGBAlpha  = MMP_FALSE;
        pISPctxt->UpdateFlags                  |= ISP_FLAGS_UPDATE_OutParameter;
        break;
#endif
    ...
    MMP_ISP_CAPS cap)
{
    ...
    case MMP_ISP_PREVIEW_VIDEO_MODE:
        result = pISPctxt->OutInfo.EnableVideoPreview;
        break;

#if (CFG_CHIP_FAMILY == 9860)
    case MMP_ISP_OUTPUT_RGB_ALPHA:
        result = pISPctxt->OutInfo.EnableOutputRGBAlpha;
        break;
#endif
    ...
    MMP_INT            value)
{
    ...
    if (pISPctxt->EngineMode.EnableCaptureOnFly)
        if (!(pISPctxt->WorkFlags & ISP_FLAGS_CAP_ONFLY_INIT))
            En_ISP_CORE_MUTEX = MMP_FALSE;
    ...
    case MMP_ISP_SHARPNESS:
        if (OUT_OF_RANGE(value, -4, 5))
        {
            result = ISP_ERR_COLOR_CTRL_OUT_OF_RANGE;
            isp_msg_ex(ISP_MSG_TYPE_ERR, "Sharpness value (%d) out range !!", value);
        }
        else
            pISPctxt->sharp = (MMP_INT16) value;
        break;
    ...
    MMP_UINT   height)
{
    ...
    if (pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO || pISPctxt->ispMode == MMP_ISP_MODE_MOTION_JPEG || pISPctxt->ispMode == MMP_ISP_MODE_TRANSFORM)
    {
    ...
    const MMP_ISP_SHARE *data)
{
    ...
    else if (pISPctxt->EngineMode.EnableCaptureOnFly)
    {
        if (pISPctxt->WorkFlags & ISP_FLAGS_CAP_ONFLY_INIT)
        {
            //ISP_LogReg(pISPctxt->ispCore);
    ...
    MMP_ISP_OUTPUT_INFO *PVoutInfo)
{
    ...
    MMP_UINT32  CMDQWaitSize = 6;
    MMP_UINT32  CMDQTemp     = 0;
    MMP_UINT32  CMDQMask     = 0;
    MMP_UINT32  Addr0        = 0;
    MMP_UINT32  Addr1        = 0;
    MMP_UINT32  Addr2        = 0;
    MMP_UINT32  PVAddr0      = 0;
    ...
    if (outInfo == MMP_NULL || PVoutInfo == MMP_NULL)
    {
        result = ISP_ERR_INVALID_INPUT_PARAM;
        isp_msg_ex(ISP_MSG_TYPE_ERR, " NULL pointer !\n");
        goto end;
    ...
    CMDQAddr = ithCmdQWaitSize(8 * CMDQWaitSize, ITH_CMDQ1_OFFSET);

    CMDQMask = ISP_BIT_DRIVER_FIRE_EN << ISP_SHT_DRIVER_FIRE_EN;
    isp_ReadHwReg(ISP_REG_SET500 + ISP_CORE_1_BASE, (MMP_UINT32 *)&CMDQTemp);
    CMDQTemp &= ~CMDQMask;
    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_SET500 + ISP_CORE_1_BASE, CMDQTemp);

    CMDQMask = ISP_MASK_HIGHBIT;
    isp_ReadHwReg(ISP_REG_PVOUT_HEIGHT + ISP_CORE_1_BASE, (MMP_UINT32 *)&CMDQTemp);
    CMDQTemp &= CMDQMask;
    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_PVOUT_HEIGHT + ISP_CORE_1_BASE,
            (PVAddr0 & ISP_BIT_PVOUT_ADDR_L) << 16 | CMDQTemp);

    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_PVOUT_ADDR_0H + ISP_CORE_1_BASE,
            (PVAddr0 >> 16) & ISP_BIT_PVOUT_ADDR_H | (Addr0 & ISP_BIT_OUT_ADDR_L) << 16);

    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_OUT_ADDR_0H + ISP_CORE_1_BASE,
            (Addr0 >> 16) & ISP_BIT_OUT_ADDR_H | (Addr1 & ISP_BIT_OUT_ADDR_L) << 16);

    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_OUT_ADDR_1H + ISP_CORE_1_BASE,
            (Addr1 >> 16) & ISP_BIT_OUT_ADDR_H | (Addr2 & ISP_BIT_OUT_ADDR_L) << 16);

    CMDQMask = ISP_MASK_LOWBIT;
    isp_ReadHwReg(ISP_REG_OUT_ADDR_2H + ISP_CORE_1_BASE, (MMP_UINT32 *)&CMDQTemp);
    CMDQTemp &= CMDQMask;
    ITH_CMDQ_SINGLE_CMD(CMDQAddr, ISP_REG_OUT_ADDR_2H + ISP_CORE_1_BASE,
            (Addr2 >> 16) & ISP_BIT_OUT_ADDR_H | CMDQTemp);

    ithCmdQFlush(CMDQAddr, ITH_CMDQ1_OFFSET);
#endif
    ...
    MMP_UINT   constantAlpha)
{
    ...
    pISPctxt->OutInfo.OutputRGBAlpha = (MMP_UINT8)constantAlpha;
    pISPctxt->UpdateFlags           |= ISP_FLAGS_UPDATE_OutParameter;
    ...
    MMP_UINT   *finishIndex)
{
    ...
    isp_ReadHwReg(ISP_REG_WB_OUT_STATUS, (MMP_UINT32 *)&status);
    ...
    *currentIndex = (status & ISP_BIT_WB_STATUS_CURRENT_INDEX << ISP_SHT_WB_STATUS_CURRENT_INDEX) >> ISP_SHT_WB_STATUS_CURRENT_INDEX;
    *finishIndex  = (status & ISP_BIT_WB_STATUS_FINISH_INDEX << ISP_SHT_WB_STATUS_FINISH_INDEX) >> ISP_SHT_WB_STATUS_FINISH_INDEX;
