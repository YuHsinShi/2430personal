    ...
    const ISP_INPUT_INFO *pInInfo)
{
    ...
    if (pISPctxt->OutInfo.EnableFieldScale)
    {
        ...
        {
            if (pInInfo->PlaneFormat == YUV420 || pInInfo->PlaneFormat == YUV422R || pInInfo->EnableNVMode)
                Value |= (MMP_UINT16)(pInInfo->PitchUV & ISP_BIT_INPUT_PITCH_UV) << 16;
            else
                Value |= (MMP_UINT16)((pInInfo->PitchUV << 1) & ISP_BIT_INPUT_PITCH_UV) << 16;
        }
    ...
    const ISP_INPUT_INFO *pInInfo)
{
    ...
    if (pISPctxt->OutInfo.EnableFieldScale == MMP_TRUE)
    {
        ...
        {
            Value = (pISPctxt->top_field_first)
                    ? (MMP_UINT32)pInInfo->AddrY  - vramBaseAddr
                    : (MMP_UINT32)pInInfo->AddrYp - vramBaseAddr;
        }
    ...
    const MMP_UINT8   constantAlpha)
{
    ...
    Value |= (constantAlphaEnable & ISP_BIT_OUT_RGB_ALPHA_EN) << ISP_SHT_OUT_RGB_ALPHA_EN |
            (constantAlpha & ISP_BIT_OUT_RGB_ALPHA) << ISP_SHT_OUT_RGB_ALPHA;
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_PANEL_COLOR_V, (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
