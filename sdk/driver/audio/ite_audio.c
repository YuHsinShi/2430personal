...
//#define DUMP_AUDIO_ENCODER_FROM_HDMI
...
//
//#define LOG_ERROR   ((void) ((ITH_ZONE_ERROR & LOG_ZONES) ? (printf("[AUD][ERROR]"
//#define LOG_WARNING ((void) ((ITH_ZONE_WARNING & LOG_ZONES) ? (printf("[AUD][WARNING]"
//#define LOG_INFO    ((void) ((ITH_ZONE_INFO & LOG_ZONES) ? (printf("[AUD][INFO]"
//#define LOG_DEBUG   ((void) ((ITH_ZONE_DEBUG & LOG_ZONES) ? (printf("[AUD][DEBUG]"
//#define LOG_ENTER   ((void) ((ITH_ZONE_ENTER & LOG_ZONES) ? (printf("[AUD][ENTER]"
//#define LOG_LEAVE   ((void) ((ITH_ZONE_LEAVE & LOG_ZONES) ? (printf("[AUD][LEAVE]"
...
static AUDIO_ENCODE_PARAM  AUD_AudioEncode;
...
//printf("readableQueueLen %d bottomLen %d Audio_Encode_Rdptr %d \n",  readableQueueLen,bottomLen,Audio_Encode_Rdptr);
        ...
        ithInvalidateDCacheRange((AUD_baseAddress + Audio_Encode_Bufptr + Audio_Encode_Rdptr), bottomLen);
        ...
        if (topLen > 0)
        {
            ithInvalidateDCacheRange((AUD_baseAddress + Audio_Encode_Bufptr), topLen);
    ...
    unsigned long length)
{
    ...
    unsigned int writableQueueLen=0;
    unsigned int bottomLen=0;
    unsigned int topLen=0;
    unsigned int residualLength=0;
    unsigned int wav_length   = 0;
    unsigned int adpcm_length = 4;
    unsigned int nTempBufptr=0;
    unsigned int nTempBuflen=0;
    unsigned int nTempWrptr=0;
...
int iteAudioSetEncodeParam(AUDIO_ENCODE_PARAM  encParam)
{
     unsigned short command = 0;

     printf("iteAudioSetEncodeParam ch %d , sample rate %d, input type %d ,buf 0x%x, buflen %d, (0x%x,0x%x),(0x%x,%d) \n",encParam.nChannels,encParam.nSampleRate,encParam.nInputBufferType,encParam.pInputBuffer,encParam.nBufferLength,TOINT((int)encParam.pInputBuffer - CODEC_BASE),TOINT(encParam.nBufferLength),gAudioStream->codecStreamBuf1,gAudioStream->codecStreamLength1);
     memcpy(&AUD_AudioEncode,&encParam,sizeof(AUDIO_ENCODE_PARAM));

     if (!encParam.pInputBuffer){
        printf("iteAudioSetEncodeParam buffer null !  \n");
        return -1;
     }

     if ( encParam.nChannels == 1){
         command |= 0x0000;
     }  else if (encParam.nChannels ==2) {
         command |= 0x0001;
     }

     if (encParam.nSampleRate== 48000){
         command |= 0x0010;
    } else  if (encParam.nSampleRate == 44100){
         command |= 0x0020;
    }  else  if (encParam.nSampleRate == 32000){
         command |= 0x0030;
    }

     if (encParam.nInputBufferType== 0){
         command |= 0x0000;
     }  else if (encParam.nInputBufferType ==1) {
         command |= 0x0100;
     }  else if (encParam.nInputBufferType ==2) {
         command |= 0x0200;
     }
    ...
    gAudioStream->codecStreamBuf1    = TOINT((int)encParam.pInputBuffer - CODEC_BASE);
    //gAudioStream->codecStreamBuf1    = TOINT((int)buf);
    gAudioStream->codecStreamLength1 = TOINT(encParam.nBufferLength);
    if (gAudioStream->codecStreamBuf1 <0){
        printf("gAudioStream->codecStreamBuf1 <0 \n");
    }
    gEncodeTimeStamp = encParam.nStartTime;
...
#ifdef DUMP_AUDIO_ENCODER_FROM_HDMI
static FILE *fwav = NULL;
#define READ_SIZE 4608
int nWavRd;
int nWavWr;
static int read_wav()
{
    ...
    char *filename = "a:/mp2enc.wav";
    int rd,wr;
    fwav = fopen(filename, "rb");


    if (fwav == NULL){
        printf("Can't fopen fwav\n");
    } else {
        printf("fopen success fwav\n");
    }

    fseek(  fwav,  0,  SEEK_END);
    nFileSize = ftell(fwav);
    ...
    do
    {
        // read input audio
        rd = nWavRd;
        if (wr > rd && ( (sizeof(ptRecordBuf-wr))+rd >READ_SIZE) ) {
            if ( (wr+READ_SIZE) <= sizeof(ptRecordBuf)){
                nSize = fread(ptRecordBuf+wr, 1, READ_SIZE, fwav);
                wr += READ_SIZE;
                nWavWr = wr;
                if (nSize != READ_SIZE)
                {
            ...
            } else if ((wr+READ_SIZE) > sizeof(ptRecordBuf)) {
                nSize = fread(ptRecordBuf+wr, 1, sizeof(ptRecordBuf)-wr, fwav);
                nSize = fread(ptRecordBuf, 1, READ_SIZE - (sizeof(ptRecordBuf)-wr), fwav);
                wr = READ_SIZE - (sizeof(ptRecordBuf)-wr);
                 nWavWr = wr;

                if (nSize != READ_SIZE - (sizeof(ptRecordBuf)-wr))
                {
                    printf("eof else \n");
                    done = 1;
                }
...
static FILE    *fI2Soutput = NULL;
static     char *i2sfilename = "a:/mp2enc.wav";

static FILE    *fI2Soutput2 = NULL;
static     char *i2sfilename2 = "a:/mp2enc2.wav";
#endif
...
#else
  // hdmi
    uint32_t rp =  AUD_AudioEncode.GetRP();  //I2S_AD32_GET_HDMI_RP();
    uint32_t wp = AUD_AudioEncode.GetWr();  //I2S_AD32_GET_HDMI_WP();
    uint32_t newRp;
#endif
    pthread_t task_event;
    //pthread_create(&task_event, NULL, read_wav, NULL);
    ...
    if (fI2Soutput==NULL){
        fI2Soutput = fopen(i2sfilename, "wb");

        if (fI2Soutput == NULL){
            printf("Can't fopen fI2Soutput !!!!!\n");
            return 0;
        } else {
            printf("fopen success fI2Soutput\n");
        }
        tWavInfo.format = ITE_WAVE_FORMAT_PCM;
        tWavInfo.bitsPerSample  = 16;
        tWavInfo.nChans         = AUD_AudioEncode.nChannels;
        tWavInfo.sampRate       = AUD_AudioEncode.nSampleRate;

        iteAudioSetWaveDecodeHeader(tWavInfo);
        fwrite(wave_header, sizeof(char), (sizeof(wave_header) - 4), fI2Soutput);
    ...
    //printf(" newRp %d wp %d, AUD_nKeepI2SRP %d  \n",newRp,wp,AUD_nKeepI2SRP);

    if (AUD_nKeepI2SRP != newRp){
       //printf("iteAudioUpdateI2SADStatus rp %d  (0x%x,0x%x,0x%x,0x%x)\n",AUD_nKeepI2SRP,rec_buf[AUD_nKeepI2SRP],rec_buf[AUD_nKeepI2SRP+1],rec_buf[AUD_nKeepI2SRP+2],rec_buf[AUD_nKeepI2SRP+3]);
//printf(".");
#if 0

        {
            if (AUD_nKeepI2SRP<newRp) {
                fwrite(AUD_AudioEncode.pInputBuffer+AUD_nKeepI2SRP, sizeof(char), newRp-AUD_nKeepI2SRP, fI2Soutput);
            } else if (AUD_nKeepI2SRP>newRp) {
                fwrite(AUD_AudioEncode.pInputBuffer+AUD_nKeepI2SRP, sizeof(char), AUD_AudioEncode.nBufferLength-AUD_nKeepI2SRP, fI2Soutput);

                fwrite(AUD_AudioEncode.pInputBuffer, sizeof(char), newRp, fI2Soutput);
            }
        }
...
#else
  // hdmi
        //I2S_AD32_SET_HDMI_RP(newRp);
        AUD_AudioEncode.SetRP(newRp);
...
static int       nFWrite = 0;
static FILE    *foutput = NULL;
static     char *filename = "a:/mp2enc.mp3";
static unsigned char ptMp3Buf[80*1024];
static int nMp3Size =0;
#endif
...
/*
        ...
        if (gTimeStamp % 50 ==0)
            printf("encode timeStamp %d datasize %d \n",ptEncodeData->nTimeStamp,ptEncodeData->nDataSize);
*/
        ...
        if (ptEncodeData->nDataSize>0)
        {
            ...
            if (nFWrite==4800){ //gStopRecord
                ...
                if (foutput) {
                    memcpy(ptMp3Buf+nMp3Size,ptEncodeData->ptData,ptEncodeData->nDataSize);
                    nMp3Size += ptEncodeData->nDataSize;

                    if (nFWrite % 102 == 0) {
                        //fwrite(ptEncodeData->ptData, sizeof(char), ptEncodeData->nDataSize, foutput);


                        fwrite(ptMp3Buf, sizeof(char),nMp3Size, foutput);
                        memset(ptMp3Buf,0,sizeof(ptMp3Buf));
                        nMp3Size = 0;
                    ...
                    if (nFWrite%50 ==0)
                        printf("fwrite %d , %d ,(0x%x 0x%x 0x%x 0x%x  , 0x%x 0x%x )\n",ptEncodeData->nDataSize,ptEncodeData->nTimeStamp,ptEncodeData->ptData[0],ptEncodeData->ptData[1],ptEncodeData->ptData[2],ptEncodeData->ptData[3]);

               // printf("fwrite %d 0x%x 0x%x 0x%x 0x%x  , 0x%x 0x%x\n",readableQueueLen,pTemp[0+Audio_Encode_Rdptr],pTemp[1+Audio_Encode_Rdptr],pTemp[2+Audio_Encode_Rdptr],pTemp[3+Audio_Encode_Rdptr],AUD_baseAddress,Audio_Encode_Bufptr);
               // reg = ITE_AUDIO_READ_REG(DrvEncode_RdPtr);
               // reg1 = ITE_AUDIO_READ_REG(DrvEncode_WrPtr);

              //  printf("encode rd ptr %d, wr ptr %d \n",reg,reg1);
            }
        }
#endif
