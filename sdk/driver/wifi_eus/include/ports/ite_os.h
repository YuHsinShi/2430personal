...
struct completion_ite {
    int isFinish;
};

#define init_completion_ite(_comp)          ((_comp)->isFinish = 0)
#define wait_for_completion_ite(_comp)      while(1){ \
                                            mdelay(1);  \
                                            if ((_comp)->isFinish){     \
                                                (_comp)->isFinish = 0;  \
                                                break;                  \
...
#define complete_ite(_comp)                 ((_comp)->isFinish = 1)
#define complete_and_exit_ite(_comp, _flag) ((_comp)->isFinish = 1)

unsigned long RT_wait_for_completion_timeout_ite(struct completion_ite *x, unsigned long timeout);
