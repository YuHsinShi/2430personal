...
#include <drv_types.h>
#include "linux/err.h" //eason, for MP mode
...
#define complete_and_exit(_comp, _flag)
...
//	desc->txdw3 |= cpu_to_le32((pattrib->seqnum & 0xFFF) << SEQ_SHT);
	...
	//3 6. start thread
#ifdef PLATFORM_LINUX
	pmp_priv->tx.PktTxThread = (_thread_hdl_)kthread_run((void *)mp_xmit_packet_thread, pmp_priv, "RTW_MP_THREAD");
	if (IS_ERR((void *)(pmp_priv->tx.PktTxThread)))
