...
char g_hidden_ssid[32];
u32 rtw_get_scan_cnt=0;
...

{
	...
	iwe.u.qual.qual = (pnetwork->network.Rssi)*(-1); //reverse to positive
...
*/
	...
	while(1)
	{
		if (rtw_end_of_queue_search(phead,plist)== _TRUE){
		    rtw_get_scan_cnt++;
		    printf("rtw_wx_get_scan: end of queue search(%u)\n", rtw_get_scan_cnt);
			break;
		}
		...
		{
		   //write SSID INFO into pnetwork**************
		   if (strcmp(pnetwork->network.Ssid.Ssid, "") == 0 && strlen(pnetwork->network.Ssid.Ssid) == 0 &&
		    pnetwork->network.MacAddress[0] && rtw_get_scan_cnt > 1){
    		   strcpy(pnetwork->network.Ssid.Ssid, g_hidden_ssid); //SSID
               pnetwork->network.Ssid.SsidLength = strlen(pnetwork->network.Ssid.Ssid);
               rtw_get_scan_cnt = 0;

               printf("Found Hidden SSID: %s [%02X:%02X:%02X:%02X:%02X:%02X]\n", pnetwork->network.Ssid.Ssid,
...
int rtw_mp_ioctl_hdl(struct net_device *dev, struct iw_request_info *info,
						union iwreq_data *wrqu, char *extra)
{
#ifdef CONFIG_MP_INCLUDED
			...
			union iwreq_data *wdata, char *extra)
{
	...
	while ((token = (char *)strsep(&pch, ",")) != NULL)
	{
		if (i > 2) break;
		tmp[i] = token;
		i++;
...
//		DBG_871X("\n");
	...
	else if (strcmp(tmp[0], "rmap") == 0)
	{
		...
		// rmap addr cnts
		addr = (u16)simple_strtoul(tmp[1], &ptmp, 16);
		DBG_871X("%s: addr=%x\n", __FUNCTION__, addr);
...
int rtw_ioctl_smInfo(struct net_device *dev, struct net_device_info *netDeviceInfo)
{
    ...
    if(netDeviceInfo->infoType == WLAN_INFO_LINK_HIDDEN)
    {
        if (netDeviceInfo->connect_hidden_ssid_enable)
            strcpy(g_hidden_ssid, netDeviceInfo->hidden_ssid);
        else
            printf("====>rtw_ioctl_smInfo: No Support Hidden SSID\n");
    }
        ...
        while((work - scanResult) < iwReq.data.length && i<(WLAN_SCAN_LIST_NUMBER-1))
        {
            ...
            switch(iwEvt.cmd)
            {
				...
				case SIOCGIWRATE:
					break;
				case IWEVQUAL:
					netDeviceInfo->apList[i].rfQualityQuant = iwEvt.u.qual.level; //%
					netDeviceInfo->apList[i].rfQualityRSSI  = (-1)*(iwEvt.u.qual.qual);  //dBm
					break; 
				case IWEVCUSTOM:
