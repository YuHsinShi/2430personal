...
static struct net_device *alloc_etherdev(unsigned int priv_size)
{
    void *priv;

    memset((void*)&net_dev, 0x0, sizeof(struct net_device));
	priv = malloc(priv_size);
    memset(priv, 0x0, priv_size);
	
	net_dev.priv = priv;
	net_dev.hard_header_len = ETH_HLEN;
	net_dev.mtu = ETH_DATA_LEN;
	//net_dev.addr_len = ETH_ALEN;
	net_dev.flags = IFF_BROADCAST|IFF_MULTICAST;
	net_dev.state |= __LINK_STATE_NOCARRIER;
	net_dev.priv_flags |= TX_QUEUE_XOFF;
...
int netif_rx(struct usbnet *dev, struct sk_buff *skb)
{
    dev->net->rx_cb(dev->net->rx_netif, (void*)(skb->data), skb->len);
    dev_kfree_skb(skb);
...
void usbnet_skb_return (struct usbnet *dev, struct sk_buff *skb)
{
	...
	netif_dbg(dev, rx_status, dev->net, "< rx, len %u, type 0x%x\n",
...
static void wait_skb_queue_empty(struct sk_buff_head *q)
{
	...
	while (!skb_queue_empty(q)) {
		spin_unlock_irqrestore(&q->lock, flags);
		schedule_timeout(NULL, msecs_to_jiffies(UNLINK_TIMEOUT_MS));
...
int usbnet_stop (struct net_device *net)
{
	...
	tasklet_kill (&dev->bh);
    #if 1 // for ite
	destroy_timer(&dev->delay); 
	mutex_destroy(&dev->phy_mutex);
	mutex_destroy(&dev->interrupt_mutex);
    #endif
	if (!pm)
...
int usbnet_open (struct net_device *net)
{
	...
	if ((retval = usb_autopm_get_interface(dev->intf)) < 0) {
		netif_info(dev, ifup, dev->net,
			   "resumption fail (%d) usbnet usb%d, %s\n",
			   retval,
			   dev->udev->bus->busnum,
			   info->description);
		goto done_nopm;
	...
	if (info->reset && (retval = info->reset (dev)) < 0) {
		netif_info(dev, ifup, dev->net,
			   "open reset fail (%d) usbnet usb%d, %s\n",
			   retval,
			   dev->udev->bus->busnum,
			   info->description);
		goto done;
...
fail_lowmem:
	...
	if (test_bit (EVENT_LINK_RESET, &dev->flags)) {
		...
		if(info->link_reset && (retval = info->link_reset(dev)) < 0) {
			usb_autopm_put_interface(dev->intf);
skip_reset:
			netdev_info(dev->net, "link reset failed (%d) usbnet usb%d, %s\n",
				    retval,
				    dev->udev->bus->busnum,
				    info->description);
		} else {
...
void usbnet_disconnect (struct usb_interface *intf)
{
	...
	netif_info(dev, _probe, dev->net, "unregister '%s' usb%d, %s\n",
...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	int				status;
	const char			*name;
	struct usb_driver 	*driver = to_usb_driver(udev);
	...
	if (status)
		goto out5;
	netif_info(dev, _probe, dev->net,
		   "register '%s' at usb%d, %s, %pM\n",
		   udev->driver->name,
		   xdev->bus->busnum,
		   dev->driver_info->description,
		   net->dev_addr);
