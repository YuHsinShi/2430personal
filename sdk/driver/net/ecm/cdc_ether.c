...
#if 0//IS_ENABLED(CONFIG_USB_NET_RNDIS_HOST)
...
void usbnet_cdc_status(struct usbnet *dev, struct urb *urb)
{
	...
	case USB_CDC_NOTIFY_NETWORK_CONNECTION:
		netif_dbg(dev, _timer, dev->net, "CDC: carrier %s\n",
			  event->wValue ? "on" : "off");
		usbnet_link_change(dev, !!event->wValue, 0);
		break;
	case USB_CDC_NOTIFY_SPEED_CHANGE:	/* tx/rx rates */
		netif_dbg(dev, _timer, dev->net, "CDC: speed change (len %d)\n",
			  urb->actual_length);
