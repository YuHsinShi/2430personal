    ...
    MMP_BOOL*    bFrameEnd)
{
    ...
    {       
        ...
        if (ptEncoder->bIFrame)
        {
            ...
            memcpy(ptEncoder->pStreamBufAdr[ptEncoder->streamBufSelect], ptEncoder->pHdrBufAddr[0], offset);
#else
            {
                MMP_UINT8 * streambuf;
            
                streambuf = (MMP_UINT8 *) malloc(0, offset);
        	...
        	if (offset)
        	    memcpy(ptEncoder->pStreamBufAdr[ptEncoder->streamBufSelect], ptEncoder->pHdrBufAddr[1], offset); 
        }
    } 
    else 
#endif    	
