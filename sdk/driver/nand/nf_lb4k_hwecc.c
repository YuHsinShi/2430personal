...
static MMP_UINT8       LB4K_HWECC_WaitCmdReady(NF_CMD_TYPE type);
static HWECC_RESP_TYPE LB4K_HWECC_CheckECC(MMP_UINT8 *data, MMP_UINT8 *spare, MMP_BOOL EnableEcc);
...
static MMP_UINT8 LB4K_HWECC_Read1kBytes( MMP_UINT32 pba, MMP_UINT32 ppo, MMP_UINT8* pBuffer, MMP_UINT8  index);

static MMP_UINT8
compare2Buf(MMP_UINT8* pBuf1, MMP_UINT8* pBuf2, MMP_UINT32 CprSize);


void LB4K_HWECC_RWE_TEST(void);
    ...
    NF_CMD_TYPE type)
{
	...
	NAND_DbgPrintf("[ENTER]LB4K_HWECC_WaitCmdReady\n");
    ...
    do
    {
		...
		if ( timeOut-- == 0 )
		{
			...
			printf("\tLB4K_HWECC_WaitCmdReady wait idle time out[1]!!!,0X131A=%X.\n",nfStatus);
	...
	NAND_DbgPrintf("[LEAVE]LB4K_HWECC_WaitCmdReady: Return %s\n", result == LL_OK ? "LL_OK" : "LL_ERROR");
	return result;
}
    ...
    unsigned long* pPageSize)
{
	...
	NAND_DbgPrintf("[ENTER]LB4K_HWECC_Initialize: \n");
	...
	if (g_NfAttrib.bInit)
	{
		...
		*pPageSize     = g_NfAttrib.pageSize;
		...
		// Awin@20071015
		NAND_DbgPrintf("\tLB4K_HWECC_Initialize: numOfBlocks  = %d.\n\r", g_NfAttrib.numOfBlocks);
		NAND_DbgPrintf("\tLB4K_HWECC_Initialize: pagePerBlock = %d.\n\r", g_NfAttrib.pagePerBlock);
		NAND_DbgPrintf("\tLB4K_HWECC_Initialize: pageSize     = %d.\n\r", g_NfAttrib.pageSize);
    ...
    result = LB4K_HWECC_WaitCmdReady(NF_READ_ID);
	...
	if ( !NF_InitDma() )
	{
	    printf("LB4K_HWECC_Initialize: Initial DMA fail.\n");
	    result = LL_ERROR;
        goto end;
	}
#endif
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_Initialize: return %s\n", result == LL_OK ? "OK" : "FALSE");
...
{
	//NOTHING TO DO
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_Getwear: \n");
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_Getwear: \n");
}
    ...
    MMP_UINT32 pba)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_Erase: pba(%d)\n", pba);
#ifdef SHOW_RWE_MSG
printf("Ers[%x]\r\n",pba);
#endif
//while(1);
    ...
    MMP_UINT8* pSpareBuffer)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_Write: pba(%d), ppo(%d)\n", pba, ppo);
...
//while(1);    
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_Write: return %s\n", result == LL_OK ? "OK" : "FALSE");

    return result;
}

MMP_UINT8
LB4K_HWECC_LBA_Write(
    MMP_UINT32 pba,
    MMP_UINT32 ppo,
    MMP_UINT8* pDataBuffer,
    MMP_UINT8* pSpareBuffer)
    ...
    MMP_UINT8* pBuffer1)
{
    ...
    //printf("W1[%x,%x,%x,%x]\r\n",pba,ppo,pDataBuffer,pSpareBuffer);
#endif
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_WriteDouble: pba(0x%X), ppo(0x%X)\n", pba, ppo);
    // ----------------------------------------
    // 1. Switch controller to nand
	EnableNfControl();
    ...
    result = LB4K_HWECC_WaitCmdReady(NF_WRITE);

    // ----------------------------------------
    // 7. Enable write protect bit and disable CE pin
    //HOST_WriteRegisterMask(NF_REG_GENERAL,(MMP_UINT16)0x0000,0x0800); // 0x1316

    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_WriteDouble: return %s\n", result == LL_OK ? "OK" : "FALSE");
    ...
    MMP_UINT8* pBuffer)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_Read: pba(0x%X), ppo(0x%X), realAddress = 0x%08x\n",
...
/*
    ...
    result = LB4K_HWECC_WaitCmdReady(NF_READ);
    ...
    if ( result == LL_OK )
    {
        ...
        // ----------------------------------------
        // 8. Check ECC
        eccResult = LB4K_HWECC_CheckECC((MMP_UINT8*)pBuffer, (MMP_UINT8*)(pBuffer+PAGE_SIZE), !bIserased);
        ...
        switch(eccResult)
        {
            ...
            case NF_ECCCORPASS:
                printf("LB4K_HWECC_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;
                
            case NF_PARITYADDRERR:
                printf("LB4K_HWECC_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                printf("LB4K_HWECC_Read: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                printf("LB4K_HWECC_Read: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_Read: return %s\n", result == LL_OK ? "OK" :
    ...
    MMP_UINT8* pBuffer)
{
    ...
    if ( result == LL_OK )
    {
        ...
        // ----------------------------------------
        // 8. Check ECC
        //eccResult = LB4K_HWECC_CheckECC(pBuffer, pBuffer+PAGE_SIZE, !bIserased);
        ...
        case NF_ECCCORPASS:
            printf("LB4K_HWECC_LBA_Read: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
            break;

        case NF_PARITYADDRERR:
            printf("LB4K_HWECC_LBA_Read: ECC Correction failed. eccResult == PARITYADDRERR\n");
            result = LL_ERROR;
            break;

        case NF_OVER4ERR:
            printf("LB4K_HWECC_LBA_Read: ECC Correction failed. eccResult == OVER4ERR\n");
            result = LL_ERROR;
            break;

        default:
            printf("LB4K_HWECC_LBA_Read: Unknown ECC value. eccResult = %d\n", eccResult);
            result = LL_ERROR;
            break;
        }
    ...
    MMP_UINT8  index)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_ReadPart: pba(0x%X), ppo(0x%X), index(0x%X), realAddress = 0x%08x\n",
	...
	switch(index)
	{
		...
		case LL_RP_SPARE:
			...
			break;
        default:
            printf("LB4K_HWECC_ReadPart: Unknown ECC value. eccResult = %d\n", eccResult);
            result = LL_ERROR;
            break;
	}
	#endif
    ...
    if ( result == LL_OK )
    {
        ...
        // ----------------------------------------
        // 8. Check ECC
        eccResult = LB4K_HWECC_CheckECC((MMP_UINT8*)pPagePointer, (MMP_UINT8*)pSparePointer, !bIserased);
        switch(eccResult)
        {
            case NF_NOECCEN:
            case NF_NOERROR:
            ...
            case NF_ECCCORPASS:
                printf("LB4K_HWECC_ReadPart: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;
                
            case NF_PARITYADDRERR:
                printf("LB4K_HWECC_ReadPart: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                printf("LB4K_HWECC_ReadPart: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_ReadPart: return %s\n", result == LL_OK ? "OK" :
                                                            result == LL_ERASED ? "ERASED" : "FALSE");
    return result;
}

MMP_UINT8
LB4K_HWECC_Read1kBytes(
    MMP_UINT32 pba,
    MMP_UINT32 ppo,
    MMP_UINT8* pBuffer,
    MMP_UINT8  index)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_Read1kB: pba(0x%X), ppo(0x%X), index(0x%X), realAddress = 0x%08x\n",
		pba, ppo, index, (((pba * g_NfAttrib.pagePerBlock)+ ppo)* PAGE_SIZE));
    ...
    if ( result == LL_OK )
    {
        ...
        // ----------------------------------------
        // 8. Check ECC
        eccResult = LB4K_HWECC_CheckECC(pPagePointer, pSparePointer, !bIserased);
        switch(eccResult)
        {
            case NF_NOECCEN:
            case NF_NOERROR:
    ...
    MMP_UINT32 pba)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_IsBadBlock: pba(%d), realAddress = 0x%08x\n",
    ...
    {
        printf("\tLB4K_HWECC_IsBadBlock: ERROR! pba(%u) >= g_NfAttrib.numOfBlocks(%u)\n", pba, g_NfAttrib.numOfBlocks);
        return LL_ERROR;
    }
    ...
    // 2. Read first page
    {
        ...
        bIibResult = LB4K_HWECC_Read(pba, 0, dataBuffer);
        ...
        #endif
        {
        	...
        	if( IfDataEqual00FF == LL_OK )
        	{
            	...
            	{
            	    printf("LB4K_HWECC_IsBadBlock: ERROR! g_NfAttrib.pfCheckMethod == MMP_NULL\n");
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_IsBadBlock: pba(%d), %s\n", pba, blockStatus == LL_OK ? "Normal Block" : "Bad Block");

    return blockStatus;
}

MMP_UINT8
LB4K_HWECC_IsBadBlockForBoot(
    MMP_UINT32 pba)
{
    MMP_UINT8  result    = LL_ERROR;
    MMP_UINT8  blockStatus = LL_ERROR;
    ...
    MMP_UINT8* pByte)
{
    ...
    NAND_DbgPrintf("[ENTER]LB4K_HWECC_ReadOneByte: pba(0x%X), ppo(0x%X), sparepos(0x%X), realAddress = 0x%08x\n",
    ...
    if ( result == LL_OK )
    {
        ...
        switch(eccResult)
        {
            ...
            case NF_ECCCORPASS:
                printf("LB4K_HWECC_Read 1B: pba(%u), ppo(%u), eccResult == NF_ECCCORPASS\n", pba, ppo);
                break;
                
            case NF_PARITYADDRERR:
                printf("LB4K_HWECC_Read 1B: ECC Correction failed. eccResult == PARITYADDRERR\n");
                result = LL_ERROR;
                break;

            case NF_OVER4ERR:
                printf("LB4K_HWECC_Read 1B: ECC Correction failed. eccResult == OVER4ERR\n");
                result = LL_ERROR;
                break;

            default:
                printf("LB4K_HWECC_Read 1B: Unknown ECC value. eccResult = %d\n", eccResult);
                result = LL_ERROR;
                break;
        }
    ...
    NAND_DbgPrintf("[LEAVE]LB4K_HWECC_ReadOneByte: *ch = 0x%X, result(%d)\n", *pByte, result);
...
unsigned char*
LB4K_HWECC_GetBlockErrorRecord()
{
#ifdef LB_ECC_RECORD
...
{    
    ...
    //AHB_WriteRegisterMask((ITH_GPIO_BASE+0x4C), PULL_UP_CF_IOWR, CF_IOWR_PULL_UP_DOWN_MASK);
    //AHB_WriteRegisterMask((ITH_GPIO_BASE+0x4C), PULL_UP_NDCS0, NDCS0_PULL_UP_DOWN_MASK);
    return;
}
    ...
    MMP_UINT32 pba)
{
    ...
    // ----------------------------------------
    // 1. Read 1st page
    result = LB4K_HWECC_Read(pba, 0, dataBuffer);
    if (   result == LL_ERASED
        && dataBuffer[0]      == 0xFF 
        && dataBuffer[CI2048] == 0xFF )
    {
        // ----------------------------------------
        // 2. Read 2nd page
        result = LB4K_HWECC_Read(pba, 1, dataBuffer);
        if (   result == LL_ERASED
            && dataBuffer[0]      == 0xFF 
            && dataBuffer[CI2048] == 0xFF )
        {
    ...
    MMP_UINT32 pba)
{
    ...
    // ----------------------------------------
    // 1. Read 1st page
    result = LB4K_HWECC_Read(pba, lastPage, dataBuffer);
    if (   result == LL_ERASED
        && dataBuffer[0]      == 0xFF 
        && dataBuffer[CI2048] == 0xFF )
    {
    ...
    MMP_UINT32 pba)
{
        ...
        && (dataBuffer[CI2048] == 0xFF) )
    {
        // ----------------------------------------
        // 2. Read 2nd page
        result = LB4K_HWECC_Read(pba, last2Page, dataBuffer);
        if (   (result == LL_ERASED)
            && (dataBuffer[CI2048] == 0xFF) )
        {
            blockStatus = LL_OK;
    ...
    MMP_UINT32* NfStatus)
{
    ...
    *NfStatus = 0;
    ...
    do
    {
        ...
        if ( timeOut-- == 0 )
        {
            printf("\tLB4K_HWECC_WaitCmdReady wait idle time out[2]!!!\n");
            result = LL_ERROR;
            goto end;
        }
    }while( (*NfStatus&0xc000) != NFIDLE );
    ...
    MMP_UINT32* CmdID0, MMP_UINT32* CmdID1, MMP_UINT32 IdCycles)
{
    ...
    if ( LB4K_HWECC_WaitCmdReady(NF_READ_ID)==LL_OK )
    {
...
#endif  // End #ifdef LARGEBLOCK_4KB
