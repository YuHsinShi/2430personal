    ...
    void)
{
    ...
    if (VPctxt == MMP_NULL)
    {
        VPctxt = malloc(sizeof(VP_CONTEXT));
        if (!VPctxt)
        {
    ...
    memset((void*)VPctxt, 0, sizeof(VP_CONTEXT));
    ...
    void)
{
    ...
    //Input Information
    memset((void*)(&(VPctxt->InInfo)), 0, sizeof(VP_INPUT_INFO));

    //Deinterlace
    VPctxt->DeInterlace.Enable = MMP_FALSE;

    //Jpeg encode
    memset((void*)(&(VPctxt->JpegEncode)), 0, sizeof(VP_JEPG_ENCODE_CTRL));

    //Output Information
    memset((void*)(&(VPctxt->OutInfo)), 0, sizeof(VP_OUTPUT_INFO));
