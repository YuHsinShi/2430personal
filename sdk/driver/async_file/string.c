...
PalCharToUpper(
    MMP_CHAR* string)
{
    MMP_SIZE_T strlen = PalStrlen(string);
...
MMP_INT
PalStricmp(
    const MMP_CHAR *string1,
    const MMP_CHAR *string2)
{
    MMP_CHAR f,l;
    ...
    do
    {
        f = _f_tolower(*string1);
        l = _f_tolower(*string2);
        string1++;
        string2++;
    } while ( (f) && (f == l) );
    ...
    const MMP_CHAR *string)
{
    ...
    if (memory = (MMP_CHAR *) calloc(size, sizeof(MMP_CHAR)))
    {
        strncpy(memory, string, size);
        return memory;
    }
