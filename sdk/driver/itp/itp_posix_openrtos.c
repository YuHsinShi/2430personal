...
int ftruncate(int file, off_t length)
{
    ...
    else
    {
        ...
        {
            /*  Pass call on to device driver.  Return result.        */
            return dev->ftruncate(file & ITP_FILE_MASK, length);
        }
        else
        {
            errno = ENOENT;
...
unsigned long itpEventWaitBits(ITPEvent* event, unsigned long bits, bool clear, bool waitall, unsigned long ms)
{
    ...
    result = xEventGroupWaitBits(event->pxEvent, bits, clear ? pdTRUE : pdFALSE, waitall ? pdTRUE : pdFALSE, xTicksToWait);
    if (result == 0)
    {
...
void itpTaskResume(pthread_t pthread)
{
    ...
    if (ithGetCpuMode() == ITH_CPU_SYS)
    {
        vTaskResume(t->xHandle);
    }
    else
    {
        BaseType_t xYieldRequired = xTaskResumeFromISR(t->xHandle);
