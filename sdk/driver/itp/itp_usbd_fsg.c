...
static int UsbdFsgGetDeviceOffset(void)
{
	...
	{
		if (strcmp(str, "A") == 0)
			norPartitionOffset = 0x10;
		else if (strcmp(str, "B") == 0)
			norPartitionOffset = 0x10 + disk.size[0]/512;
		else if (strcmp(str, "C") == 0)
			norPartitionOffset = 0x10 + (disk.size[0]+ disk.size[1])/512;
		else if (strcmp(str, "D") == 0)
			norPartitionOffset = 0x10 +(disk.size[0]+ disk.size[1]+ disk.size[2])/512;
		else
		{
