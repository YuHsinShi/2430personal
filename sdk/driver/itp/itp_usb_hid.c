...
static QueueHandle_t usbip_queue;
...
#ifdef CFG_USBIP_ENABLE
static void usbip_cb_func(struct urb *urb)
{
	if(!(xQueueSend(usbip_queue, (urb), 0)))
    	ithPrintf(" usbip_kbd: queue full! \n");
}

static int UsbKbdGetURB(struct urb *urb)
{
	if(!usbip_queue)
    {
        printf(" usbip_kbd: queue not created??? \n");
        errno = (ITP_DEVICE_USBKBD << ITP_DEVICE_ERRNO_BIT) | 2;
        return -1;
    }

    if (xQueueReceive(usbip_queue, (void *)urb, 0))
    {
    	return sizeof(urb_t);
    }
	return 0;
...
static int UsbHidRead(int file, char *ptr, int len, void* info)
{
    if(!queue)
    {
        printf(" Kbd: queue not created??? \n");
        errno = (ITP_DEVICE_USBHID << ITP_DEVICE_ERRNO_BIT) | 2;
        return -1;
    }
...
static void UsbHidInit(void)
{
    ...
    if(!queue)
        printf(" hid: create queue fail! \n");

    //iteUsbKbdSetCb(cb_func);
}

static int UsbHidIoctl(int file, unsigned long request, void* ptr, void* info)
{
    switch (request)
    {
    case ITP_IOCTL_INIT:
    ...
    default:
        errno = (ITP_DEVICE_USBHID << ITP_DEVICE_ERRNO_BIT) | 1;
        return -1;
    }
    return 0;
}

const ITPDevice itpDeviceUsbHid =
{
    ":usbhid",
    itpOpenDefault,
    itpCloseDefault,
