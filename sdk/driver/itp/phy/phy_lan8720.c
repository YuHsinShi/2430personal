...
int lan8720_read_mode(int* speed, int* duplex)
{
    uint32_t status;
    ...
    *duplex = DUPLEX_HALF;
    ...
    status = phy_reg_read(31);
    //ithPrintf("phy status: 0x%04X! \n", status);

    //if (status & (0x1 << 12))
    //    ithPrintf("lan8720: auto-negotiation not done! \n");
    
    if (((status >> 2) & 0x0x7) == 0x1) {
        *speed = SPEED_10;
        *duplex = DUPLEX_HALF;
    }
    else if (((status >> 2) & 0x0x7) == 0x5) {
        *speed = SPEED_10;
        *duplex = DUPLEX_FULL;
    }
    else if (((status >> 2) & 0x0x7) == 0x2) {
        *speed = SPEED_100;
        *duplex = DUPLEX_HALF;
    }
    else if (((status >> 2) & 0x0x7) == 0x6) {
        *speed = SPEED_100;
        *duplex = DUPLEX_FULL;
    }
    else {
        //ithPrintf("lan8720: not support!! \n");
        return -1;
    }
...
*/
int(*itpPhyReadMode)(int* speed, int* duplex) = lan8720_read_mode;
/**
* Get link status.
*/
