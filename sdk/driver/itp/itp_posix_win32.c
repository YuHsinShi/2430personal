...
ITPEvent* itpEventCreate(void)
{
    ...
    handles = calloc(sizeof(unsigned long) * 8, sizeof(HANDLE));
    if (!handles)
    {
        free(ev);
        return NULL;
...
{
    DWORD result;
    HANDLE* handles = (HANDLE*)event->pxEvent;
    int i, count = 0;
    HANDLE buf[sizeof(unsigned long) * 8];
    for (i = 0; i < sizeof(unsigned long) * 8; ++i)
    {
        if ((bits >> i) & 0x1)
        {
    ...
    result = WaitForMultipleObjects(count, buf, waitall ? TRUE: FALSE, ms);
    if (result == WAIT_TIMEOUT)
    {
        LOG_WARN "WAIT EVENT TIMEOUT\n" LOG_END
        return 0;
    }
...
{
    ptw32_thread_t * tp = (ptw32_thread_t *)pthread.p;
    DWORD result = SuspendThread(tp->threadH);
    if (result < 0)
    {
        LOG_ERR "%s\r\n", GetErrorString(GetLastError()) LOG_END
    }
...
{
    ptw32_thread_t * tp = (ptw32_thread_t *)pthread.p;
    DWORD result = ResumeThread(tp->threadH);
    if (result < 0)
    {
        LOG_ERR "%s\r\n", GetErrorString(GetLastError()) LOG_END
    }
