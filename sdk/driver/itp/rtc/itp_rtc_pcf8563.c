...
void itpRtcSetTime(long sec, long usec)
{
	...
	data[0] = 0x02;
	data[1] = _DEC2BCD((uint8_t)t->tm_sec);
	data[2] = _DEC2BCD((uint8_t)t->tm_min);
	data[3] = _DEC2BCD((uint8_t)t->tm_hour);
	data[4] = _DEC2BCD((uint8_t)t->tm_mday);
	data[5] = (uint8_t)t->tm_wday % 7;
    
    if (t->tm_year > 99) 
    {
		data[6] = 0x80 | _DEC2BCD((uint8_t)t->tm_mon + 1);
		data[7] = _DEC2BCD((uint8_t)t->tm_year - 100);
    }
    else 
    {
		data[6] = _DEC2BCD((uint8_t)t->tm_mon + 1);
		data[7] = _DEC2BCD((uint8_t)t->tm_year);
    }
