...
static int I2cIoctl(int file, unsigned long request, void *ptr, void *info)
{
    ...
    uint32_t    init_clock  = 100 * 1024;
#ifdef CFG_I2C0_ENABLE    
    if(iic_port == IIC_PORT_0)
        init_clock = CFG_I2C0_CLOCKRATE;
#endif
#ifdef CFG_I2C1_ENABLE
    if(iic_port == IIC_PORT_1)
        init_clock = CFG_I2C1_CLOCKRATE;
#endif
#ifdef CFG_I2C2_ENABLE    
    if(iic_port == IIC_PORT_2)
        init_clock = CFG_I2C2_CLOCKRATE;
#endif
#ifdef CFG_I2C3_ENABLE
    if(iic_port == IIC_PORT_3)
        init_clock = CFG_I2C3_CLOCKRATE;
#endif

    switch (request)
    {
    case ITP_IOCTL_INIT:
        mmpIicInitialize(iic_port, iic_mode, 0, 0, init_clock, 0);
        break;
