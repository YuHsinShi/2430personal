...
static int StcRead(int file, char *ptr, int len, void* info)
{
    ...
    {
        *(uint64_t*)ptr = GetTimeDiff() - stc_info[index].offset - stc_info[index].pause_duration;
    }    
    else
    {
        ...
        case ITP_IOCTL_PAUSE:
            {
                ...
                switch(state)
                {
                    case STATE_RUN: // Set pause
                        {
                            uint64_t clk = GetTimeDiff(); //Benson
                    ...
                    case STATE_PAUSE: // Set resume
                        stc_info[index].pause_duration += GetTimeDiff() - stc_info[index].last_pause;
                        stc_info[index].state = STATE_RUN;
                        break;
