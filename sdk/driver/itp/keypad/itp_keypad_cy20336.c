...
static uint32_t _i2c_read(uint8_t *b1, uint32_t len1, uint8_t *b2, uint32_t len2)
{
    ...
    evt.slaveAddress   = CY20336_IIC_ADDR;       //device slave address
...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, IIC_MASTER_MODE, CY20336_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}
...
static uint32_t _i2c_write(uint8_t cmd, uint8_t *buf, uint32_t cnt)
{
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, IIC_MASTER_MODE, CY20336_IIC_ADDR, cmd, buf, cnt);
#endif
}
...
//	result = mmpIicReceiveData(IIC_MASTER_MODE, CY20336_IIC_ADDR, CY_TOUCH_REG_ADRRESS, &buf, 1);
    ...
    result  = _i2c_read(&buf_reg, 1, &buf_data, 1);
    if (result)
    {
        printf("Err1\n");
        return 0;
...
//	result = mmpIicSendData(IIC_MASTER_MODE, CY20336_IIC_ADDR, CY_LIGHT_REG_ADRRESS, &light_status, 1);
    ...
    result = _i2c_write(CY_LIGHT_REG_ADRRESS, &light_status, 1);
...
void itp_cy_keypad_light_on_off(unsigned int led_num, bool on)
{
    ...
    result  = _i2c_read(&buf_reg, 1, &light_status, 1);
...
//	    printf("[LED2]-status %x\n",light_status);
    buf_reg = CY_LIGHT_REG_ADRRESS;
    result  = _i2c_write(buf_reg, &light_status, 1);
/*
   mmpIicSendData(
    IIC_PORT    port,
    IIC_OP_MODE mode,
