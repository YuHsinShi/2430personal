...
static uint32_t _i2c_read(uint8_t *b1, uint32_t len1, uint8_t *b2, uint32_t len2)
{
    ...
    evt.slaveAddress   = IT7235_IIC_ADDR;       //device slave address
...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, IIC_MASTER_MODE, IT7235_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}
...
static uint32_t _i2c_write(uint8_t cmd, uint8_t *buf, uint32_t cnt)
{
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, IIC_MASTER_MODE, IT7235_IIC_ADDR, cmd, buf, cnt);
#endif
