...
#endif
#ifdef  CFG_TOUCH_KEYPAD_I2C1
static uint8_t              g_kpI2cPort     = IIC_PORT_1;
#endif
#ifdef  CFG_TOUCH_KEYPAD_I2C2
static uint8_t              g_kpI2cPort     = IIC_PORT_2;
#endif
#ifdef  CFG_TOUCH_KEYPAD_I2C3
static uint8_t              g_kpI2cPort     = IIC_PORT_3;
#endif
/**************************************************************************
** "IT7230_TouchKey.c"                                                   **
***************************************************************************/
...
static unsigned int _i2c_read(uint8_t *b1, uint32_t len1, uint8_t *b2, uint32_t len2)
(
    ...
    evt.slaveAddress   = IT7230_IIC_ADDR;       //device slave address
...
#else
    uint8_t buf[1] = {*b1};
    return mmpIicReceiveData(g_kpI2cPort, IIC_MASTER_MODE, IT7230_IIC_ADDR, &buf[0], len1, b2, len2);
#endif
}

static unsigned int _i2c_write(uint8_t cmd, uint8_t *buf, uint32_t cnt)
(
#ifdef EN_NEW_I2C_CTRL
    ITPI2cInfo evt;
    uint32_t i2cRes = 0;
    ...
    return i2cRes;
#else
    return mmpIicSendData(g_kpI2cPort, IIC_MASTER_MODE, IT7230_IIC_ADDR, cmd, buf, cnt);
#endif
}
...
static void IT7230_CheckPage(BYTE page)
{
    ...
    if (page != gRegPage)
    {
        for (i = 0; i < Retry; i++)
        {
            ...
            if (_i2c_write(CAPS_PSR, wPage, 2) == 0)
            {
#ifdef ENABLE_SAVE_RESTORE_ALL_INTR
                if ((IT7230_Init_Status() == FINISH_RESTORT) || (IT7230_Init_Status() == PRE_RESTORT))
                {
...
void IT7230_CapS_Write_Reg(BYTE page, BYTE addr_byte, WORD data_word)
{
    ...
    for (i = 0; i < Retry; i++)
    {
        ...
        if (_i2c_write(addr_byte, wPage, 2) == 0)
        {
#ifdef ENABLE_SAVE_RESTORE_ALL_INTR
            if ((IT7230_Init_Status() == FINISH_RESTORT) || (IT7230_Init_Status() == PRE_RESTORT))
            {
