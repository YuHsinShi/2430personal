...
#if defined(CFG_CHIP_FT)
#undef MSC_MAX_LUN
#define MSC_MAX_LUN 1
static sem_t ft_sem[2];
sem_t *ft_usb_sem[2] = { NULL };
#endif
...
static void* MscPollHandler(void* arg)
{
    ...
    #if defined(CFG_CHIP_FT)
    sem_init(&ft_sem[itpusb->index], 0, 0);
    ft_usb_sem[itpusb->index] = &ft_sem[itpusb->index];
    #endif
    ...
    while (1)
    {
        for(lun=0; lun<MSC_MAX_LUN; lun++)
        {
            ...
            retry = 1;
            do {
                res = iteMscGetStatus(itpusb->intf[0].ctxt, lun);
            } while(res && --retry);
            ...
            else /* original is not active */
            {
                ...
                {
                    #if defined(CFG_CHIP_FT)
                    sem_post(ft_usb_sem[itpusb->index]);
                    #else
                    cardStatus.card = (itpusb->index==0) ? (ITP_CARD_MSC00 << lun) : (ITP_CARD_MSC10 << lun);
...
static void MscConnect(ITPUsb* itpusb, int idx)
{
    if(USB_DEVICE_MSC(itpusb->intf[idx].type))
    {
        pthread_attr_t attr;
...
static void MscDisconnect(ITPUsb* itpusb, int idx)
{
    if(USB_DEVICE_MSC(itpusb->intf[idx].type))
    {
...
static void UasConnect(ITPUsb* itpusb, int idx)
{
    if(USB_DEVICE_UAS(itpusb->intf[idx].type))
    {
        pthread_attr_t attr;
        struct sched_param param;
        ...
        uas_lun_num[itpusb->index] = iteUasGetLunNum(itpusb->intf[idx].ctxt);
...
static void UasDisconnect(ITPUsb* itpusb, int idx)
{
    if(USB_DEVICE_UAS(itpusb->intf[idx].type))
    {
