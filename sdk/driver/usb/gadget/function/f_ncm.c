...
#define crc32_le(a,b,c)		0    // TODO if has crc
#endif
			  ...
			  struct sk_buff_head *list)
{
	...
	if (get_unaligned_le32(tmp) != opts->nth_sign) {
		...
		#if 1
		ithPrintf("HEAD:\n");
		ithPrintVram32((uint32_t)skb->data, 4);
		#else
...
static int ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	hs_ncm_notify_desc.bInterval = USB_MS_TO_HS_INTERVAL(NCM_STATUS_INTERVAL_MS);
	#endif
	...
	ss_ncm_notify_desc.bEndpointAddress =
		fs_ncm_notify_desc.bEndpointAddress;
	#if 1 // Irene Lin: for build fail
	fs_ncm_notify_desc.bInterval= USB_MS_TO_HS_INTERVAL(NCM_STATUS_INTERVAL_MS);
	#endif
...
#else
static struct usb_function_driver ncm_usb_func = {
    .name = "ncm",
    .mod = THIS_MODULE,
    .alloc_inst = ncm_alloc_inst,
    .alloc_func = ncm_alloc,
};

int iteUsbdNcmRegister(void)
{
    usb_function_register(&ncm_usb_func);

    DBG(NULL, "iteUsbdNcmRegister\n");
}

int iteUsbdNcmUnRegister(void)
{
    usb_function_unregister(&ncm_usb_func);

    DBG(NULL, "iteUsbdNcmUnRegister\n");
}
#endif
MODULE_LICENSE("GPL");
