...
extern struct net_device    *usbd_ncm_netdev;
...
#define free_netdev(dev)	do { if (dev->priv) free(dev->priv); } while(0)
...
{
    usbd_ncm_netdev = NULL;
}

static int netif_rx(struct net_device *dev, struct sk_buff *skb)
{
    dev->rx_cb(dev->rx_netif, (void*)(skb->data), skb->len);
    dev_kfree_skb(skb);
...
struct net_device *gether_setup_name_default(const char *netname)
{
	...
	//pr_warn("using random %s ethernet address\n", "host");
	#if 1
	INFO(dev, "device MAC %02X%02X%02X%02X%02X%02X\n", dev->dev_mac[0], dev->dev_mac[1],
...
int gether_get_host_addr_cdc(struct net_device *net, char *host_addr, int len)
{
	...
	dev = netdev_priv(net);
	#if 1
    snprintf(host_addr, len, "%02X%02X%02X%02X%02X%02X", dev->host_mac[0], dev->host_mac[1], 
		dev->host_mac[2], dev->host_mac[3], dev->host_mac[4], dev->host_mac[5]);
	#else
