...
/**
 * @brief Disable FIFO's interrupt
 *
 * @param ep    FIFO corresponding endpoint
 */
...
/**
 * @brief Inidicate the firmware has finished the whole packet
 *        transaction for endpoint
 *
 * @param fotg210       potiner to the usb device controller
 */
...
/**
 * @brief Mapping a fifo to an endpoint, and enable FIFO
 *
 * @param ep            pointor to the endpoint
 * @param epnum         endpoint number
 * @param dir_in        IN or OUT direction
 */
...
/**
 * @brief Set FIFO's diection
 *
 * @param ep            pointer to endpoint
 * @param epnum         endpoint number
 * @param dir_in        0: OUT; 1: IN;
 */
...
/**
 * @brief Set FIFO's transfer type
 *
 * @param ep            pointer to endpoint
 * @param epnum         endpoint number
 * @param type          0: reserved;
 *                      1: isochronous;
 *                      2: bulk
 *                      3: interrupt
 */
...
/**
 * @brief Set FIFO's max packet size
 *
 * @param ep            pointer to endpoint
 * @param epnum         endpoint number
 ...
 *
 * @param ep    the endpoint to be configed
...
/**
 * @brief Enable enpoint and config endpoint depending on the endpoint
 *        descriptor.
 *
 * @param _ep
 * @param desc
...
/**
 * @brief Allocate a USB request
 *
 * @param _ep                   pointer to endpoint
 * @param gfp_flags
...
static u32 fotg210_start_dma(struct fotg210_ep *ep,
			struct fotg210_request *req)
{
	dma_addr_t d;
	u8 *buffer;
...
/**
 * @brief Queue (Submit) a usb request to endpoint 0.
 *
 * @param ep
 * @param req
 */
...
/**
 * @brief Queues (submits) a usb request to an endpoint.
 *
 * @param _ep
 * @param _req
 * @param gfp_flags
...
/**
 * @brief Get if the endpoint is in the stall state
 *
 * @param ep 	the endpoint
 * @return 1: in stall state, 0: not in stall state
 */
...
static void fotg210_out_fifo_handler(struct fotg210_ep *ep)
{
	struct fotg210_request *req = list_entry(ep->queue.next,
						 struct fotg210_request, queue);
#if 1 // Irene Lin: fix bug
    u32 len = fotg210_start_dma(ep, req);
...
static void fotg210_set_unplug(struct fotg210_udc *fotg210)
{
    u32 reg = ioread32(fotg210->reg + FOTG210_PHYTMSR);
...
static void fotg210_a_bus_drop(struct fotg210_udc *fotg210)
{
    u32 value;
    ...
    ithPrintf("0x%08X = 0x%08X \n", fotg210->reg + 0x80, value);
}

static void fotg210_a_bus_drive(struct fotg210_udc *fotg210)
{
    u32 value;
    ...
    value |= 0x10;
    iowrite32(value, fotg210->reg + 0x80);
    ithPrintf("0x80 = 0x%08X \n", ioread32(fotg210->reg + 0x80));
}
...
static int fotg210_udc_stop(struct usb_gadget *g)
{
    ...
    fotg210_a_bus_drive(fotg210);
    fotg210_set_unplug(fotg210);
...
int iteUdcInitialize(void)
{
	...
	fotg210->reg = (void*)UDC_BASE;	// why not get the addresss from input parameter?
	if (fotg210->reg == NULL) {
...
err_req:
	fotg210_ep_free_request(&fotg210->ep[0]->ep, fotg210->ep0_req);	// memory leak
...
int  iteUdcRestart(void)
{
    if (iteOtgIsDeviceMode() == true) {
        ithWriteRegMaskA(UDC_BASE + FOTG210_PHYTMSR, 1, PHYTMSR_UNPLUG);
        iteUsbUnRegisterGadget();
        usleep(1000000);
        ithWriteRegMaskA(UDC_BASE + FOTG210_PHYTMSR, 0, PHYTMSR_UNPLUG);
