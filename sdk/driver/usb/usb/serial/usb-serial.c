...
#define USB_SERIAL_TTY_MINORS	32//512	/* should be enough for a while */
...
static void destroy_serial(struct kref *kref)
{
    ...
    #if 1
    mutex_destroy(&serial->disc_mutex);
	#endif
...
#if 1
static int serial_is_ready(struct tty_struct *tty)
{
    if (tty->port && tty->port->active)
		return 1;
	else
		return 0;
}
...
static int serial_open(struct tty_struct *tty)
{
	struct usb_serial_port *port = tty->driver_data;

	dev_dbg(tty->dev, "%s\n", __func__);

	return tty_port_open(&port->port, tty);
}
...
static void serial_close(struct tty_struct *tty)
{
	struct usb_serial_port *port = tty->driver_data;
    __func_enter__
	dev_dbg(tty->dev, "%s\n", __func__);

	tty_port_close(&port->port, tty);
	__func_leave__
}
					...
					struct usb_interface *iface)
{
	...
	struct usb_serial_driver *drv;
	struct usb_driver *driver = to_usb_driver(iface);
...
#if 1 // Irene Lin
#define tty_port_init	tty_port_init_h

static void *usb_serial_probe(struct usb_interface *interface,
#else
    ...
    const struct usb_device_id *id)
{
    ...
    */
    ...
    for (i = 0; i < num_ports; ++i) {
        ...
        port = serial->port[i];
		sprintf(name, "ttyUSB%d", port->minor);
...
exit:
    ...
    destruct_tty_driver(usb_serial_tty_driver);
    return result;
}
				...
				const struct usb_device_id *id_table)
{
	...
	#if 0 // not attach in serial driver register
