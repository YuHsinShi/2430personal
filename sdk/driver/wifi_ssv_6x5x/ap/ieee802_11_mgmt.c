...
ssv_type_u8 * hostapd_eid_supp_rates(ApInfo_st *pApInfo, ssv_type_u8 *eid)
{
	...
	*pos++ = num;
	...
	for (count = 0; i < pApInfo->config.nNumrates && count < num;
		i++) {
			count++;
			*pos = (ssv_type_u8)pApInfo->config.pRates[i]->bitrate;
...
int hostapd_get_aid(ApInfo_st *pApInfo, APStaInfo_st *sta)
{
	...
	for (i = 0; i < ((gMaxAID + 31) / 32); i++) {
		if (pApInfo->sta_aid[i] == (ssv_type_u32) -1)
			continue;
		for (j = 0; j < 32; j++) {
...
static ssv_type_u16 check_ssid(ApInfo_st *pApInfo, APStaInfo_st *sta,
		      const ssv_type_u8 *ssid_ie, ssv_type_size_t ssid_ie_len)
{
	if (ssid_ie == NULL)
		return WLAN_STATUS_UNSPECIFIED_FAILURE;

	if (ssid_ie_len != pApInfo->config.ssid_len ||
	    ssv6xxx_memcmp(ssid_ie, pApInfo->config.ssid, ssid_ie_len) != 0) {
...
static ssv_type_u16 check_wmm(ApInfo_st *pApInfo, APStaInfo_st *sta,
	const ssv_type_u8 *wmm_ie, ssv_type_size_t wmm_ie_len)
{
...
static ssv_type_u16 copy_supp_rates(ApInfo_st *pApInfo, APStaInfo_st *sta,
