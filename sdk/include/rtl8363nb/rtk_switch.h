...
#if 1//defined(RTK_X86_CLE)  // Irene Lin
#include <pthread.h>
#include "ite/ith.h"  // Irene Lin
#endif
...
#define RTK_API_LOCK()     do { if (ithGetCpuMode() == ITH_CPU_SYS) pthread_mutex_lock(&api_mutex);  } while(0)
#define RTK_API_UNLOCK()   do { if (ithGetCpuMode() == ITH_CPU_SYS) pthread_mutex_unlock(&api_mutex);  } while(0)
#else
#define RTK_API_LOCK()
