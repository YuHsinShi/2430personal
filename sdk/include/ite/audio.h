...
typedef struct AUDIO_ENCODE_PARAM_TAG
{
    ...
    unsigned int (*GetRP)();   // get buffer read porinter
    unsigned int (*GetWr)();   // get buffer write porinter 
    void (*SetRP)(unsigned int data);    
}AUDIO_ENCODE_PARAM;
//=============================
...
void iteAudioSetMusicWithoutASFHeader(int32_t nEnable);
...
/*get specturm*/
int smtkAudioMgrSetSpectrum(unsigned char* buffer);
/*get codec buffer index*/
void smtkAudioMgrSetPcmIdx(int index);
