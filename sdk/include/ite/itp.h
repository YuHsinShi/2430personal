 ...
 */
/** @addtogroup itp ITE Platform Library
 *  @{
 */
#ifndef ITE_ITP_H
#define ITE_ITP_H
...
/** @addtogroup itp_posix POSIX Implementations
 *  @{
 */
 ...
 *
 * @param sem semaphore handle.
 * @param ms timeout value. -1 for infinite.
 * @return 0 (OK), or -1 (ERROR).
 */
...
/** @addtogroup itp_device Device Drivers
 *  @{
 */
...
typedef enum
{
    ...
    ITP_DEVICE_IR0           = (15 << ITP_DEVICE_BIT),       ///< IR0
    ...
    ITP_DEVICE_USBHID        = (44 << ITP_DEVICE_BIT),       ///< USB HID
    ...
    ITP_DEVICE_ARMLITE       = (55 << ITP_DEVICE_BIT),       ///< ARMLITE device
    ...
    ITP_DEVICE_RS485_5       = (78 << ITP_DEVICE_BIT),       ///< RS485_5
    ITP_DEVICE_IR1           = (79 << ITP_DEVICE_BIT),       ///< IR1
    ITP_DEVICE_IR2           = (80 << ITP_DEVICE_BIT),       ///< IR2
    ITP_DEVICE_IR3           = (81 << ITP_DEVICE_BIT),       ///< IR3
    ITP_DEVICE_WIFI_NGPL     = (82 << ITP_DEVICE_BIT),       ///< WiFi SDIO NGPL
    ITP_DEVICE_USBDHID       = (83 << ITP_DEVICE_BIT),       ///< USB acts as a USB HID device
    ITP_DEVICE_WIFI_SSV      = (84 << ITP_DEVICE_BIT),       ///< WiFi SDIO SSV
	ITP_DEVICE_USBOPTION 	 = (85 << ITP_DEVICE_BIT),		 ///< USB Option for 4G modem AT command
    ITP_DEVICE_USBDNCM		 = (86 << ITP_DEVICE_BIT),		 ///< USB Network Control Model
    ITP_DEVICE_LAST
} ITPDeviceType;
...
#define ITP_IOCTL_ETHERNET_SEND_ARP   88   ///<ARP reply
...
#define ITP_IOCTL_UART_SET_GPIO       96   ///< Set Uart TX/RX GPIO
...
#define ITP_IOCTL_SET_IRTX_CARRIER    116  ///< IR TX set Carrier Freq.
#define ITP_IOCTL_SET_WRITE_PROTECT	  117  ///< SET WRITE PROTECT
#define ITP_IOCTL_RENEW_DHCP          118  ///< Ethernet open DHCP.
#define ITP_IOCTL_AT_CMD              119  ///< Process AT Cmd
#define ITP_IOCTL_WIFI_HIDDEN_SSID    121  ///< Connect to hidden SSID
#define ITP_IOCTL_ERASE               122  ///< Storage erase
#define ITP_IOCTL_STANDBY_CLOCK       123  ///< Clock standby mode
#define ITP_IOCTL_RESUME_CLOCK        124  ///< Clock resume mode
...
extern const ITPDevice   itpDeviceIr0;            ///< IR0 device
extern const ITPDevice   itpDeviceIr1;            ///< IR1 device
extern const ITPDevice   itpDeviceIr2;            ///< IR2 device
extern const ITPDevice   itpDeviceIr3;            ///< IR3 device
...
extern const ITPDevice   itpDeviceUsbHid;         ///< Usb HID device
...
extern const ITPDevice   itpDeviceWifiNgpl;       ///< NGPL WiFi device(SDIO type)
extern const ITPDevice   itpDeviceWifiSsv;        ///< SSV WiFi device(SDIO type)
extern const ITPDevice   itpDeviceUsbdHid;        ///< USB acts as a USB HID device
extern const ITPDevice   itpDeviceArmLite;        ///< Software peripheral on ARMLITE
extern const ITPDevice   itpDeviceUsbOption;      ///< USB Option for 4G modem AT command
extern const ITPDevice   itpDeviceUsbdNcm;        ///< USB Network Control Model


/** @addtogroup itp_card Card
 *  @{
 */
...
#define ITP_NAND_BLK_MODE 0x2 ///< open ITP NAND device with BLOCK mode

/** @} */                     // end of itp_nand

/** @addtogroup itp_sd SD Device
*  @{
*/
/**
* SD erase definition
...
typedef struct
{
    uint32_t    pos;     ///< The start sector to be erased
    int         count;   ///< The sector count to be erased
} ITPSdErase;

/** @} */ // end of itp_sd

/** @addtogroup itp_usbdevice Usb Device
 *  @{
 */
/**
...
/**
 * Standby mode of keypad.
 */
void itpKeypadStandby(void);

/**
 * Normal mode of keypad.
 */
void itpKeypadResume(void);

/** @} */ // end of itp_keypad

/** @addtogroup itp_usb_mouse Mouse
 *  @{
 */
...
/** @addtogroup itp_keyboard Keyboard
 *  @{
 */
...
/** @addtogroup itp_gsensor G-Sensor
 *  @{
 */
/**
...
/** @addtogroup itp_headset Headset
 *  @{
 */
/**
...
/** @addtogroup itp_amplifier Amplifier
 *  @{
 */
/**
...
/** @addtogroup itp_ethernet Ethernet
 *  @{
 */
...
typedef struct
{
    ...
    uint32_t      netmask;             ///< netmask
    char          displayName[256];    ///< display name
    unsigned char          hardwareAddress[32]; ///< hardware address
    char          name[16];            ///< short name
...
/** @addtogroup itp_ts country frequency ID
 *  @{
 */
/**
...
#define ITP_USB_FORCE_DEVICE_MODE     0x1


/** @} */ // end of itp_usb


/** @addtogroup itp_usb_option USB Option
 *  @{
 */
/**
* Mode definitions.
*/
#define ITP_USB_OPTION_IO_MODE      0x0     ///< Use I/O read/write mode
#define ITP_USB_OPTION_CMD_MODE     0x1     ///< Use command mode

/**
* Structure used to define a AT command
*/
typedef struct {
#define ITP_AT_CMD_LEN      64
#define ITP_AT_RESP_LEN     128
#define ITP_AT_ERR_LEN      32
    char        cmd_buf[ITP_AT_CMD_LEN];    ///< AT command string
    char        resp_buf[ITP_AT_RESP_LEN];  ///< the buffer to get the response string
    char        err_buf[ITP_AT_ERR_LEN];    ///< error string buffer for this AT command
    uint32_t    cmd_len;                    ///< the command string length
    uint32_t    resp_len;                   ///< the response data length

    uint32_t    timeout;                    ///< timeout value for this AT command
    uint32_t    flags;                      ///< flags for AT command
#define ITP_AT_WAIT_ACK     (0x1 << 0)      ///< Wait for "OK" or "ERROR"
#define ITP_AT_GET_RESP     (0x1 << 1)      ///< copy response string to resp_buf

    int         status;                     ///< command status
#define ITP_AT_STS_OK       (0)                 ///< status is OK
#define ITP_AT_STS_ERROR    (0x1 << 0)          ///< status is ERROR
} ITPAtCmd  __attribute__((aligned(32)));

#define ITP_AT_ERR_CMD_LEN      -1001       ///< command length error
#define ITP_AT_ERR_CMD_TIMEOUT  -1002       ///< wait ack timeout

/** @} */ // end of itp_usb_option


/** @addtogroup itp_dpu DPU
 *  @{
 */
typedef enum
{
...
typedef enum
{
    ITP_DCPS_BRFLZ_1BYTE = (0),     ///< BRFLZ compress pixel mode: 1-byte pixel
...
/** @addtogroup itp_vmem Video Memory Management
 *  @{
 * This section describes the programming interfaces of video memory (or called VRAM) management.
 *
 * # Terms and acronyms {#itp_vmem_Chapter}
 *
 * |Terms                         |Details                                                                 |
 * |------------------------------|------------------------------------------------------------------------|
 * |\b VRAM                       | Video memory. When the project is built with openrtos and run directly on the real chip, video memory is no different from system memroy. It means they are the same thing. However, if the project is built with MSVC and run on the host PC, video memory means the memory on the chip and the system memory is the memory on the host PC. At this point, the video memory and the system memory are not only physically different things, but their address space is also different. It mean that the address 0x0 on video memory is totally different from the address 0x0 on system memory. |
 *
 * # How to use this API {#HAL_I2C_Driver_Usage_Chapter}
 *
 * - Sample code:
 *   @code
 *      const size_t size = 0x1000;
 *      uint32_t vram = itpVmemAlloc(size);
 *      void*    sysram = ithMapVram(vram, size, ITH_VRAM_WRITE);
 *      memset(sysram, 0, size);
 *      ithUnmapVram(sysram, size);
 *      itpVmemFree(vram);
 *   @endcode
 */
 ...
 * @return Allocated video memory address, or 0 if an error occurred
 */
...
/** @addtogroup itp_stats Statistic
 *  @{
 */
...
/** @addtogroup itp_defer Deferred Interrupt Handling
 *  @{
 */
...
/** @addtogroup itp_event Event
 *  @{
 */

/**
 * Custom event handle implementation definition.
 */
typedef struct
{
    void *pxEvent;      ///< Native event.
} ITPEvent;
 ...
 *
 * @param The event object to destroy.
 */
 ...
 * Waits event object.
 *
 * @param event event object to wait.
 * @param bits event bits to wait.
 * @param clear clear event bits on exit or not.
 * @param waitall wait all bits or not.
 * @param ms timeout value.
 * @return bits to set, or 0 indicates timeout.
 */
