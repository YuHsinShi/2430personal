...
#ifdef CFG_BUILD_ASR
typedef enum ASR_CALLBACK_TAG{
    ASR_FAIL,
    ASR_SUCCESS_ARG,
} LeafAsrCallback;

typedef enum _LeafAsrFlowCase{
    FileAsr =0,
    FilePlayAsr,
    RecAsr,
    RecPlayAsr,
...
//ASR
#ifdef CFG_BUILD_ASR
typedef void (*AsrCallback)(int state,void *arg);
void leaf_start_asr(LeafCall *call,LeafAsrFlowCase asrcase,char *infile,char *outfile,AsrCallback func);
