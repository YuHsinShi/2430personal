...
struct hid_device {
	__u8                *dev_rdesc;      /* device report descriptor */
	unsigned               dev_rsize;       /* device report descriptor's size */
	__u8                *rdesc;          /* device report descriptor (after fixup) */
	unsigned               rsize;           /* device report descriptor's size (after fixup)  */
...
extern void hidinput_hid_event(struct hid_device *, struct hid_field *, struct hid_usage *, int32_t);
...
#define dbg_hid(format, arg ...)                     \
    do {                                    \
	    if (1)                          \
	        ithPrintf("%s: " format, __FILE__, ## arg);  \
    } while (0)
...
// copy from linux-4.13/include/uapi/linux
/*
 ...
 */
// defined in hid 1.11 section 7.1
#define HID_DT_HID			(USB_TYPE_CLASS | 0x01) // 0x21
#define HID_DT_REPORT			(USB_TYPE_CLASS | 0x02) // 0x22
#define HID_DT_PHYSICAL			(USB_TYPE_CLASS | 0x03) // 0x23

#define HID_MAX_DESCRIPTOR_SIZE		4096

static inline ssize_t iteUsbHidWrite(struct hid_device *hid, const char *buffer, size_t count)
{
	int ret = 0;
	__u8 * buf = (__u8 *)buffer;
	if (hid == NULL)
		goto out_free;
	ret = hid_hw_output_report(hid, buf, count);
	if (ret != -ENOSYS)
		goto out_free;
	
	ret = hid_hw_raw_request(hid, 0, buf, count, HID_OUTPUT_REPORT,
				HID_REQ_SET_REPORT);
