...
MS2_PUBLIC RingStream * ring_start_audiomgr(const char *file, int interval, MSSndCard *sndcard);
MS2_PUBLIC void ring_stop(RingStream *stream);
MS2_PUBLIC void ring_stop_audiomgr(RingStream *stream);
...
struct _AudioStream
{
    ...
    MSFilter    *voladj;       //for MKV record
...
typedef struct _AsrStream AsrStream;
#endif
