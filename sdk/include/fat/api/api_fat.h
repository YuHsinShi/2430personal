...
typedef struct
{
  ...
  W_CHAR  lname[F_MAXLNAME + 1];    /*  long file name   */
} F_NAME;
#endif /* elif !LFN_ENABLE */

typedef struct
{
  uint32_t  cluster;            /* which cluster is used */
  uint32_t  prevcluster;        /* previous cluster for bad block handling */
  uint32_t  sectorbegin;        /* calculated sector start */
  uint32_t  sector;             /* current sector */
  uint32_t  sectorend;          /* last saector position of the cluster */
...
typedef struct
{
  ...
  uint32_t        cluster;                  /* current file starting position */
...
typedef struct
{
  uint32_t       secnum;                /* number of sectors in this partition */
 ...
 ***************************************************************************/
 ...
 #define f_createpartition_align( driver, parnum, par, sec_align )      fm_createpartition_align( driver, parnum, par, sec_align )
 ...
 #endif
 #if FAT_ENCRYPTION
  #define f_open_enc( filename, mode, key, init_vect )                  fm_open_enc( filename, mode, key, init_vect )
 #endif
...
#endif
int fm_createpartition ( F_DRIVER * driver, int n_pars, F_PARTITION * p_pars );
int fm_createpartition_align ( F_DRIVER * driver, int32_t n_pars, F_PARTITION * p_pars, int32_t sec_align );
...
#endif
#if FAT_ENCRYPTION
FN_FILE * fm_open_enc ( const char * p_filename, const char * p_mode, const uint8_t * p_key, const uint8_t * p_init_vect );
#endif
