...
static inline int backtrace(void **bt, int depth, unsigned int lrOffset) __attribute__((always_inline)); 
static inline int backtrace(void **bt, int depth, unsigned int lrOffset)
{
#ifdef __arm__
...
extern caddr_t __heap_end__;
    ...
    while(i < depth)
    {
        ...
        if (prevFpReg > (unsigned int)&_text_end)
        {
...
static inline int backtraceFIQ(void **bt, int depth, unsigned int fiqLr) __attribute__((always_inline)); 
static inline int backtraceFIQ(void **bt, int depth, unsigned int fiqLr)
{
#ifdef __arm__
extern int _text_end;
extern caddr_t __heap_end__;
    ...
    static volatile int i  = 0;
    static volatile unsigned int fpReg, lrReg;
    int bTopFunctionCall = 1;

    asm volatile ("mov %0, fp" : "=r" (fpReg));
    asm volatile ("mov %0, lr" : "=r" (lrReg) : : "lr");
    if (depth <= 0)
        return(-1);
