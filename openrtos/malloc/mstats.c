...
<<malloc_map>> print memory statistics about memory allocation on
standard error.
...
The alternate functions <<_mallinfo_r>>, <<_malloc_stats_r>>,
<<_malloc_map_r>> and <<_mallopt_r>> are reentrant versions.
...
<<malloc_map>> does not return a result.
...
void
_DEFUN_VOID (malloc_map)
{
  //_malloc_map_r (_REENT);
}
