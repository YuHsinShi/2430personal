...
void dress_confirm(void)
{
	...
	for(i=0;i<_total;i++)
	{
		...
		//Ö»ÓÐµØÖ·Ð¡µÄ ²ÅÄÜÍùÇ°ÅÅÈë
	         if((rx_data[3]<basedata[i][1])||((rx_data[3]==basedata[i][1])&&(rx_data[4]<basedata[i][2])))
		{
			for(j=_total;j>i;j--)//ÒòÎªµØÖ·Ð¡µÄÍùÇ°²åÈë ËùÒÔËùÓÐÖµÍùºóÒÆÒ»Î»
			{				
				if(j<CONST_16)
				{
				  basedata[j][1]=basedata[j-1][1];
				  basedata[j][2]=basedata[j-1][2];
                                  basedata[j][3]=basedata[j-1][3];//ÊÒÍâ»úµÄ»úÐÍ£¬¶ÔÓ¦µÚ37×Ö½Ú
			...
			basedata[i][2]=rx_data[4];
                        basedata[i][3]=rx_data[36];//ÊÒÍâ»úµÄ»úÐÍ£¬¶ÔÓ¦µÚ37×Ö½Ú
                        
                        option_backcome_txflag=mark_deal(option_backcome_txflag,i);

                        ////////////////////////////////////////
                        //ÏÈ½«Öµ±£´æÏÂÀ´£¬ÔÙ×öÄ¬ÈÏÖµÅÐ¶Ï
                        for(j=0;j<10;j++)
                        {
                        ...
                        /////////////////////////////  
                        //Èç¹û´ÓÊÒÄÚ»ú·¢À´µÄÊý¾ÝÎªÄ¬ÈÏÖµ£¬ÇÒeepromÓÐÏµÍ³µØÖ·ÓëÖ®¶ÔÓ¦µÄÊý¾Ý Ôò´ÓeepromµÄÊý¾Ý
                        //Èç¹û²»ÊÇ£¬Ôò±£´æ´ÓÊÒÄÚ»ú·¢¹ýÀ´µÄÊý¾Ý£¬
                        //ÕâÐ©Êý¾ÝÏÈÓÃÒ»¸öµØ·½ÁÙÊ±±£´æ£¬È»ºóÔÙ°´µØÖ·Ë³Ðò´æÈëoption_dataÀïÃæ
                        for(k=0;k<CONST_16;k++)
                        {
                          if((rx_data[3]==option_data[k][11])&&(rx_data[4]==option_data[k][12]))
                          {
                            ...
                            option_backup[i][10]=option_data[k][10];//´íÎó´úÂë
                            ///////////////////
                             //ÐÂÐ­ÒéÖÐÓÃµ½µÄ»úÄÜÉè¶¨6-14
                            for(j=0;j<13;j++)
                              {
        ...
        else if(rx_data[0]==0x41)
        {
          ...
          if((rx_data[1]==0x00)&&(rx_data[2]==0x1e)&&(rx_data[9]==0x01))
          {
               if((rx_data[5]!=0xff)&&(rx_data[6]!=0xff))
               {
                ...
                if((master_flag)&&(rx_data[4]==0x02))//ÎÕÊÖ½áÊøºó£¬¶¼Îª×ÓÒ£¿Ø ×Ô¼º±äÎªÖ÷
                {   
                ...
                {
                  if((rx_data[5]==0xff)&&(rx_data[6]==0xff))//×ÔÉíÎª×Ó£¬ÊÕµ½Ç×µÄ¹ã²¥£¬ÎªÁË±ÜÃâ¶Ô·½Ò²ÊÇ×Ó£¬ËùÒÔ·¢ËÍ»ù±¾¸ñÊ½
                  {
                    //////
                    if((initialize_flag==0)&&(line_control_tx))//×ÔÉí¹ã²¥Ã»·¢³öÇ°£¬ÖØÐÂ½øÈëµÚÒ»²½
                    {
                    initialize_10ms=0;
                    initialize_cnt=3;
                    initialize_step=0;
                ...
                else if((master_flag==0)&&(rx_data[4]==0x01))//¶¼ÎªÇ×Ò£¿Ø
                {
                  if((rx_data[5]==0xff)&&(rx_data[6]==0xff))//ÔÚ¹ã²¥Ê±ÊÕµ½£¬½Ð±ðÈË¸ÄÎª×Ó
                  {
                    line_dresschange_request_tx=1;   
                    if(initialize_flag)g25_line_control_tx_flag=1;
                  }
                  else  //ÎÕÊÖ½áÊøºóÊÕµ½£¬×Ô¼º¸ÄÎª×Ó
                  {
                    master_flag=1;
                    master_flag_write=1;
                    line_control_tx=1;
                ...
                line_control_double=1;//ÓÐÇ××ÓÒ£¿ØÆ÷   
          ...
          else if((rx_data[1]==0x00)&&(rx_data[2]==0x18)&&(rx_data[9]==0xb1))
          {
              ...
              //////////////////////////////////////////////
                if((master_flag)&&(rx_data[4]==0x02))//¶¼Îª×ÓÒ£¿Ø
                {
                    master_flag=0;
                    master_flag_write=1;           
                }
          ...
          else if((rx_data[1]==0x00)&&((rx_data[2]==0x20)||(rx_data[2]==0x1A))&&(rx_data[9]==0xa1))//¼æÈÝ½Ã¹¤
          {
         ...
         //2.5GÇ××ÓÏß¿ØÆ÷À©ÕÅ¸ñÊ½
          else if((rx_data[2]==0x30)&&(rx_data[6]==line_control_dress)&&(rx_data[9]==0xbe))
         {
               g25_extenddata_line_rx_deal();            
         }
          else if((rx_data[1]==0xf1)&&(rx_data[2]==0x0c))//Ò£¿ØÆ÷µØÖ·±ä¸üÒªÇó
          { 
            line_dresschange_request_tx=0;//ÊÕµ½¶Ô·½µÄµØÖ·±ä¸üÒªÇóºó£¬×ÔÉíÒª·¢µÄ±ä¸üÒªÇóÒªÇå0
          ...
          else if((rx_data[1]==0xe2)&&(rx_data[2]==0x0a))//Ò£¿ØÆ÷µØÖ·±ä¸üÍê³É
          {
          ...
          else if((rx_data[1]==0xf2)&&(rx_data[2]==0x0a))//Ò£¿ØÆ÷µØÖ·±ä¸üÈ·ÈÏ
          {

          }
         
        }
        //½ÓÊÕ×Ô¼ºµÄÊý¾Ý½øÐÐÐ£Ñé
...
void system_on_deal(void)
{
  ...
  set_status=0;
  pre_set_status=0;
  //å¦‚æžœæ²¡æœ‰æŒ‰ä¸‹modeé”® å¼€æœºé»˜è®¤æ˜¯åˆ¶å†·
  if(system_mode==0)
  {
    system_mode=0x02;
    system_mode_write=1;
...
void rx_data_system_mode(void)
{
  ...
  //Ä£Ê½
         ...
         if(deal_data[2]&0x80)
         {
           //i=(deal_data[2]>>2)&0x1f;//¼¯ÖÐ¿ØÖÆÆ÷
           if(deal_data[2]&0x7c)i=(deal_data[2]>>2)&0x1f;//¼¯ÖÐ¿ØÖÆÆ÷
           center_control_rx_flag=1;
...
void rx_data_system_tryrun(void)
{
   ...
   //ÊÔÔË×ª ¹Ø»úÊ±½Óµ½Ë®Ä£¿é·¢À´µÄÐÅºÅ ¿ªÊ¼½øÈëÊÔÔË×ª
...
{
           pointcheck1_data[14]=deal_data[0];//Í£»úÀíÓÉ
           basedata[ee_nuber][0]=deal_data[1];//Í¬Ò»ÀäÃ½ÏµÍ³ÊÒÄÚ»úÌ¨Êý
           basedata[ee_nuber][3]=deal_data[2];//ÊÒÍâ»ú»úÖÖ´úÂë
}
/********************************************************************************
********************************************************************************/
              ...
              for(j=0;j<9;j++)
               {
                ...
                option_new_backup[ee_nuber][3]&=0x7f;  //optionÇå³ý
...
void g25_basedata_rx_deal(void)
{
  ...
  basedata[ee_nuber][41]=rx_data[24];//2.5G ¼¯¿Ø 
  ...
  if((rx_data[17]&0x60)==0x40)//×ÔÇå½àÖÕÖ¹
  {
    self_clean_flag_b&=((0x0001<<ee_nuber)^0xffff); 
    self_clean_flag_b_write=1;
  }
  
    if((rx_data[23]&0x03)==0x02)//Ç¿Á¦ÖÕÖ¹
  {
    power_wind_check_flag_b&=((0x0001<<ee_nuber)^0xffff); 
    power_wind_check_flag_b_write=1;
  }
  
  if((rx_data[23]&0x0c)==0x08)//¸ßÎÂÉ±¾úÖÕÖ¹
  {
...
void point1_data_rx_deal(void)
{
  ...
  if(system_mode==0)pointcheck1_data[0]=0;
    ...
    pointcheck1_data[0]= tempset;//Éè¶¨ÎÂ¶È b 1

  pointcheck1_data[1]= rx_data[10];//»Ø·ç¿ÕÆøÎÂ¶È b 2
  pointcheck1_data[2]= rx_data[11];//³ö·ç¿ÕÆøÎÂ¶È b 3
  pointcheck1_data[3]= rx_data[12];//Òº¹ÜÎÂ¶È  b 4
  pointcheck1_data[4]= rx_data[13];//Ô¶³Ì´«¸ÐÆ÷ÎÂ¶È  b 5
  pointcheck1_data[5]= rx_data[14];//»·¾³ÎÂ¶È b 6
  pointcheck1_data[6]= rx_data[15];//Æø¹ÜÎÂ¶È b 7
  pointcheck1_data[7]= rx_data[16];//Õô·¢ÎÂ¶È b 8
  pointcheck1_data[8]= rx_data[17];//ÀäÄýÎÂ¶È b 9
  pointcheck1_data[9]= rx_data[18];//ÅÅÆøÎÂ¶È b A
  pointcheck1_data[10]=temp_value; //temp_value;//Ïß¿ØÆ÷´«¸ÐÆ÷ÎÂ¶È b b
  pointcheck1_data[11]=rx_data[33];//Êª¶È
  pointcheck1_data[12]= rx_data[36];//ÊÒÄÚÐ¾Æ¬IO×´Ì¬ C1
  pointcheck1_data[13]= rx_data[37];//ÊÒÍâÐ¾Æ¬I/O×´Ì¬ C2
  //pointcheck1_data[14]= rx_data[41];//Í£»úÀíÓÉ  d 1
  pointcheck1_data[15]= rx_data[28];//±¨¾¯·¢Éú´ÎÊý E 1
  pointcheck1_data[16]= rx_data[29];//Ë²Í£´ÎÊý E 2
  pointcheck1_data[17]= rx_data[30];//Í£Ïß¿ØÆ÷ÓëË®Ä£¿éÍ¨ÐÅ´íÎó´ÎÊý E 3
  pointcheck1_data[18]= rx_data[31];//±äÆµÆ÷¹ÊÕÏ´ÎÊý E 4
  //pointcheck1_data[19]= rx_data[28];//×Ô¶¯µ¼·ç°å×´Ì¬F1
  pointcheck1_data[20]= rx_data[19];//ÅÅÆøÑ¹Á¦ H1
  pointcheck1_data[21]= rx_data[20];//ÎüÆøÑ¹Á¦ H2
  pointcheck1_data[22]= rx_data[21];//ÒªÇóÆµÂÊ H3
  pointcheck1_data[23]= rx_data[22];//Êµ¼ÊÔË×ªÆµÂÊ H4
  pointcheck1_data[24]= rx_data[32];//ÊÒÄÚ»úÈÝÁ¿ J1
  //pointcheck1_data[25]= rx_data[21];//ÊÒÍâ»úÖÖ´úÂë J 2
  pointcheck1_data[26]= rx_data[3];//ÀäÃ½ÏµÍ³£¨ÏÔÊ¾·½Ê½1£© J 3
  pointcheck1_data[27]= rx_data[3];//ÀäÃ½ÏµÍ³£¨ÏÔÊ¾·½Ê½2£© J 4
  pointcheck1_data[28]= rx_data[23];//ÊÒÄÚÅòÕÍ·§¿ª¶È L 1
  pointcheck1_data[29]= rx_data[24];//ÊÒÍâÅòÕÍ·§1¿ª¶È L 2
  pointcheck1_data[30]= rx_data[25];//ÊÒÍâÅòÕÍ·§2¿ª¶È L 2
  pointcheck1_data[31]= rx_data[26];//ÊÒÍâÅòÕÍ·§B¿ª¶È L 2
  pointcheck1_data[32]= rx_data[27];//Ñ¹»úµçÁ÷ P1 
  pointcheck1_data[33]= rx_data[38];//Ñ¹»úÔË×ªÊ±¼ä¸ßÎ» P2
  pointcheck1_data[34]= rx_data[40];//ÈË¸Ð´«¸ÐÆ÷·´Ó¦ÂÊq1
  pointcheck1_data[35]= rx_data[41];//µØ°åÎÂ¶È´«¸ÐÆ÷·´Ó¦ÂÊq2 
...
{
    pointcheck2_data[0]=rx_data[10];//»Ø·ç¿ÕÆøÎÂ¶È q1
    pointcheck2_data[1]=rx_data[11];//³ö·ç¿ÕÆøÎÂ¶È q2
    pointcheck2_data[2]=rx_data[12];//¶³½áÎÂ¶È q3
    pointcheck2_data[3]=rx_data[13];//ÍâÆøÎÂ¶È q4
    pointcheck2_data[4]=rx_data[14];//ÊÒÄÚÈÈ½»»»Æ÷Æø¹ÜÎÂ¶È q5
    pointcheck2_data[5]=rx_data[15];//ÖÆÈÈÕô·¢ÎÂ¶È q6
    pointcheck2_data[6]=rx_data[16];//ÀäÄýÎÂ¶È q7
    pointcheck2_data[7]=rx_data[17];//Ñ¹Ëõ»ú¶¥²¿ÎÂ¶È  q8
    pointcheck2_data[8]=rx_data[18];//ÅÅÆø£¨¸ßÑ¹£©Ñ¹Á¦ q9
    pointcheck2_data[9]=rx_data[19];//ÎüÆø£¨µÍÑ¹£©Ñ¹Á¦ qa
    pointcheck2_data[10]=rx_data[20];//ÒªÇóÆµÂÊ qb
    pointcheck2_data[11]=rx_data[21];//ÔË×ªÊµ¼ÊÆµÂÊ qc
    pointcheck2_data[12]=rx_data[22];//ÊÒÄÚ»úÅòÕÍ·§¿ª¶È qd
    pointcheck2_data[13]=rx_data[23];//ÊÒÍâ»úÅòÕÍ·§¿ª¶È1 qe
    pointcheck2_data[14]=rx_data[24];//Ñ¹Ëõ»ú£©µçÁ÷ qf
...
void line_base_data_rx_deal(void)
{
  ...
  {
    line_protocol=0;//Ïß¿ØÆ÷Îª1G
  }
  if((rx_data[16]&0x03)==0)
  {
    line_protocol&=0xfd;//Ïß¿ØÆ÷Îª2G
  }
