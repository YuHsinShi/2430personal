...
void tx_ack_next_deal(void)
{
  ...
  {
        nuber=get_nuber();
        if((server_set_status==0)&&(initialize_flag)&&((inroom_2g_protocol&(0x0001<<nuber))==0))option_data[nuber][3]&=0x7f; //Èç¹ûÃ»ÓĞ2GÊÒÄÚ»ú ÔòÇå0
  ...
  {
       nuber=get_nuber(); 
       if(((server_set_status==0)||(server_set_status==0x93))&&(initialize_flag)&&((inroom_2g5_protocol&(0x0001<<nuber))==0))option_data[nuber][3]&=0x7f;//Èç¹ûÃ»ÓĞ2.5GÊÒÄÚ»ú ÔòÇå0
...
unsigned char tx_data_17(void)
{
  unsigned char tt_data;
   ...
   //18×Ö½Ú
  //¶ÔÓ¦ÊÒÄÚ»úÓĞµ¼·ç°å(1G 38×Ö½ÚÓë2G 29×Ö½Ú bit3Îª1)Ê±£¬²ÅÄÜÉè¶¨ 
   tt_data=0;
   if(basedata[tt_nuber][7]&0x08)
   {      
      if(wind_board_set==0)//µ¼·ç°å·Ö±ğÉè¶¨Ê±£¬ÏÂÃæµÄÉè¶¨²ÅÓĞĞ§ Í³Ò»Éè¶¨Ê±£¬ÏÂÃæµÄÉè¶¨Îª0
      {
...
void g25_basedata_tx_deal(void)
{
   unsigned char i;
      ...
      tt_nuber=tt_nuber_dress();
       ...
       //0-ÎŞĞ§¡¢1-·ç´µÈË¡¢2-·ç±ÜÈË¡¢3-ÎŞÈË¡¢4-·ç´µÈËºÍ·¿¼äÎŞÈËÔËĞĞÄ£Ê½Í¬Ê±¡¢5-·ç±ÜÈËºÍ·¿¼äÎŞÈËÔËĞĞÄ£Ê½Í¬Ê±       
       i=0;
       if(human_used)
       {
        switch(human_flag)
...
unsigned char tx_line_16(void)
{
  unsigned char tt_data;
   ...
   //}
      ...
      tt_data|=0x80;//Ä¬ÈÏÓĞ0.5´ú  line_g25_flag
      tt_data|=0x01;
...
void a3d_wind_tx_deal(void)
{
   unsigned char i,j,k,i1,j1,k1;
     ...
     // else tx_data[10]&=0xfd;
      ...
      tx_data[22]=basedata[0][19];//¶ÔÓ¦2GĞ­ÒéÊÒÄÚ»úÖÁÏß¿ØÆ÷µÚ32×Ö½Ú
      tx_data[23]=0;//±¸ÓÃ
...
//  option_restore_tx_check();
  ...
  //ä»åˆ«çš„çº¿æ§å™¨å‘æ¥çš„å‘½ä»¤ï¼Œå½“æœ¬èº«ä¸ºäº²æ—¶ï¼Œåªç»™å®¤å†…æœºå‘é€åŸºæœ¬æ ¼å¼ï¼Œå½“ä¸ºå­æ—¶ï¼Œå…ˆç»™äº²çº¿æ§å™¨å‘é€åŸºæœ¬æ¨¡å¼
  if(line_change_flag)
  {
    line_change_flag=0;
    tx_systemon_flag=1;
