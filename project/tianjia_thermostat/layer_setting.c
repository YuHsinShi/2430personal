...
#endif
#define SETTING_TEXT_NUM 27

static ITUText* settingScreenLightText;
static ITUText* settingVolText;
static ITUText* settingScreenSaverText;
static ITUText* settingSecText;
static ITUText* settingText[SETTING_TEXT_NUM] = {0};
...
static ITUIcon* settingScreenModeIcon;
...
static ITUSprite* settingPageSprite;
...
void ChtNameSetting(void);
void EngNameSetting(void);
...
bool SettingOnEnter(ITUWidget* widget, char* param)
{
	...
	if (!settinLightProgressBar)
	{
		...
		for (i = 0; i < SETTING_TEXT_NUM; i++)
		{
			sprintf(tmp, "settingText%d", i);
			settingText[i] = ituSceneFindWidget(&theScene, tmp);
			assert(settingText[i]);
		}

		ituRadioBoxSetChecked(settingChtRadioBox, true);

		settingScreenModeIcon = ituSceneFindWidget(&theScene, "settingScreenModeIcon");
		assert(settingScreenModeIcon);
	}
...
bool settingChtRadioBoxOnPress(ITUWidget* widget, char* param)
{
...
bool settingEngRadioBoxOnPress(ITUWidget* widget, char* param)
{
...
void ChtNameSetting(void)
{
	...
	for (i = 0; i < SETTING_TEXT_NUM; i++)
	{
		ituTextSetString(settingText[i], StringGetSettingChtName(i));
		if (i == 20 )
			ituTextSetFontSize(settingText[i], 27);
		else if (i == 21)
			ituTextSetFontSize(settingText[i], 35);
	}

	ituWidgetSetPosition(settingScreenModeIcon, 260, 93);
	ituWidgetSetPosition(settingText[4], 318, 91);
...
void EngNameSetting(void)
{
	...
	for (i = 0; i < SETTING_TEXT_NUM; i++)
	{
		ituTextSetString(settingText[i], StringGetSettingEngName(i));

		if (i == 20)
			ituTextSetFontSize(settingText[i], 22);
		else if (i == 21)
			ituTextSetFontSize(settingText[i], 27);
	}

	ituWidgetSetPosition(settingScreenModeIcon, 190, 93);
	ituWidgetSetPosition(settingText[4], 248, 91);
}