	...
	for (i = 0; i < 4; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
			...
			if (language == LANG_CHT)
			{
				...
				ituTextSetFontSize(scenesLtAnText[i][j], 36);
				ituTextSetFontSize(scenesRtAnText[i][j], 36);
			}
			else
			{
				ituTextSetString(scenesLtAnText[i][j], StringGetScenesEngName(scenes_order[i]));
				ituTextSetString(scenesRtAnText[i][j], StringGetScenesEngName(scenes_order[i]));
				ituTextSetFontSize(scenesLtAnText[i][j], 33);
				ituTextSetFontSize(scenesRtAnText[i][j], 33);
			}
		}	
	}
...
bool ScenesRtAn2OnStopped(ITUWidget* widget, char* param)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (language == LANG_CHT)
		{
			...
			ituTextSetFontSize(scenesFocusAnText[i], 36);
		}
		else
		{
			ituTextSetString(scenesFocusAnText[i], StringGetScenesEngName(scenes_order[2]));
			ituTextSetFontSize(scenesFocusAnText[i], 33);
		}
