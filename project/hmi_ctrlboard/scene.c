...
static void CheckExternal(void)
{
    ...
    if (ret)
    {
        ...
        if (ev.type >= EXTERNAL_TAP_POWER && ev.type <= EXTERNAL_TAP_WIND_AUTO)
            AirConditionerProcessEvent(&ev);
        else if (ev.type >= EXTERNAL_TAP_HOME_PAGE_UP && ev.type <= EXTERNAL_TAP_MULTIMEDIA)
            MainMenuProcessEvent(&ev);
    }
}
