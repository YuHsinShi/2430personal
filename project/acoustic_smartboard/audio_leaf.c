...
void AudioInit(void)
{
	...
	call = leaf_init();
    leaf_set_play_level(call,50);
    leaf_set_ring_level(call,50);
    leaf_set_rec_level(call,80);
#ifdef CFG_BUILD_ASR
    AsrStartOnBackGround(1);
    BroadcastTip("B:/media/tips/start.mp3",false);
#endif
...
int AudioPlayMusic(char* filename, AudioPlayCallback func)
{
    ...
    //printf("muted=%d\n", muted);
    smtkAudioMgrSetVolume(theConfig.audiolevel*5);
...
void AudioPlayKeySound(void)
{
    ...
    leaf_start_sound_play(call,filepath,Normalplay,NULL);
}
...
void AsrStartOnBackGround(bool yesno){
    if(yesno)
        leaf_start_asr(call,RecAsr,NULL,NULL,fAsrCallback);
    else
        leaf_stop_asr(call);
}
