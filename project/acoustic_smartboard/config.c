...
void ConfigInit(void)
{
...
#ifdef CFG_BUILD_ASR
    theConfig.play_lev = iniparser_getint(cfgIni, "sound:play_lev", 15);
    theConfig.ring_lev = iniparser_getint(cfgIni, "sound:ring_lev", 15);
    theConfig.rec_lev = iniparser_getint(cfgIni, "sound:rec_lev", 15);
#endif
	//temperature
	...
	theConfig.set_temperature = iniparser_getint(cfgIni, "temperature:set_temperature", 16);
	theConfig.air_mode = iniparser_getint(cfgIni, "temperature:air_mode", 0);
...
static void ConfigSavePublic(void)
{
	...
	sprintf(buf, "%d", theConfig.set_temperature);
	iniparser_set(cfgIni, "temperature:set_temperature", buf);

	sprintf(buf, "%d", theConfig.air_mode);
	iniparser_set(cfgIni, "temperature:air_mode", buf);
