...
static void LoadScene(void)
{
    ...
    #ifdef CFG_LCD_MULTIPLE
    {
        ...
        sprintf(filepath, CFG_PRIVATE_DRIVE ":/itu/%ux%u/smart_thermostat.itu", ithLcdGetWidth(), ithLcdGetHeight());
        ituSceneLoadFileCore(&theScene, filepath);
    }
    #else
    ituSceneLoadFileCore(&theScene, CFG_PRIVATE_DRIVE ":/smart_thermostat.itu");
...
#define ncp18_offset_file "ncp_18_offset"

static bool g_ncp18_offset_setting = false;

void ncp18_offset_setting()
{
    bool found = false;
    int i = 0, timeout = 10;
    ITPDriveStatus* driveStatusTable = NULL;
    ITPDriveStatus* driveStatus = NULL;
    ITPDriveStatus* driveStatus_valid = NULL;
    char USB_offset_path[256] = {0};
    uint8_t offset_tmp[32] = {0};
    FILE *f = NULL;
    struct stat sb = {0};
    float offset_before_updating = 0;
    ...
    printf("ncp18 offset before updating: %f\n", offset_before_updating);

    if (found)
    {
        sprintf(USB_offset_path, "%s%s", driveStatus_valid->name, ncp18_offset_file);

        f = fopen(USB_offset_path, "rb");
        if (!f)
        {
            printf("USB %s file is not found\n", ncp18_offset_file);
        }
        else
        {
            fstat(fileno(f), &sb);
            printf("USB %s size: %d byte\n", ncp18_offset_file, (int)sb.st_size);

            if (fread(offset_tmp, 1, 31, f) != (int)sb.st_size)
                printf("USB %s fread fail\n", ncp18_offset_file);
            else
                theConfig.ncp18_offset = (float)atof(offset_tmp);
    ...
    {
        printf("USB disk is not found\n");
    }

    printf("ncp18 offset after updating: %f\n", theConfig.ncp18_offset);

    if (offset_before_updating != theConfig.ncp18_offset)
        ConfigSave();
}
...
int SceneRun(void)
{
    ...
    for (;;)
    {
        ...
        #ifdef CFG_NCP18_ENABLE
                        if (!g_ncp18_offset_setting)
                        {
                            ncp18_offset_setting();
                            g_ncp18_offset_setting = true;
                        }
        #endif
        ...
        if (!ScreenIsOff())
        {
                ...
                ScreenSaverCheck())
            {
                ...
                if (theConfig.screensaver_type == SCREENSAVER_CLOCK && !ScreenClockIsOff())
                {
                    // enter clock mode and freq down.
                    ScreenClockOff();
                }
