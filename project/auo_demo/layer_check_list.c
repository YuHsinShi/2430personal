...
bool CheckListOnEnter(ITUWidget* widget, char* param)
{
    ...
    CheckListInit();
    ituListBoxReload((ITUListBox*)checkListScrollIconListBox);
    ituListBoxReload((ITUListBox*)checkListScrollListBox);
    return true;
}
...
bool CheckListScrollIconListBoxReflesh(ITUWidget* widget, char* param)
{
	ITUListBox* listbox = checkListScrollIconListBox;
	int count;
	
	count = ituScrollListBoxGetItemCount(listbox);
	listbox->pageCount = checkListCount ? (checkListCount + count - 1) / count : 1;

	if (listbox->pageCount < listbox->pageIndex)
		(listbox->pageIndex)--;
	if (listbox->pageIndex == 0)
		listbox->pageIndex = 1;

	ituListBoxGoto(listbox, listbox->pageIndex);
	return true;
}

bool CheckListScrollListBoxReflesh(ITUWidget* widget, char* param)
{
	ITUListBox* listbox = checkListScrollListBox;
	int count;
