...
static ITUWaveBackground* wbg;

static bool OnChanged(ITUWidget* widget, char* param)
{
    int w = ituWidgetGetWidth(wbg);
    int i = atoi(param);
    //printf("OnChanged: value=%s\n", param);

    wbg->offsetx = w * i / 100;

    //if (wbg->offsetx > w * (wbg->xstep - 1) / wbg->xstep)
    //    wbg->offsetx = w;
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU WaveBackground Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/wavebackground.itu");

    wbg = (ITUWaveBackground*)ituSceneFindWidget(&scene, "WaveBackground2");
    assert(wbg);
