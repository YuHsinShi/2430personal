...
static ITUIcon* srcIcon;
static ITUIcon* destIcon;

static void IconDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    int destx, desty;
    ...
    scaleX = (float)rect->width / srcIcon->surf->width;
    scaleY = (float)rect->height / srcIcon->surf->height;

    ituWidgetSetClipping(widget, dest, x, y, &prevClip);

    ituTransform(dest, destx, desty, rect->width, rect->height, srcIcon->surf, 0, 0, srcIcon->surf->width, srcIcon->surf->height,
        srcIcon->surf->width / 2, srcIcon->surf->height / 2, scaleX, scaleY, 0.0f, ITU_TILE_REFLECT, true, true, desta);
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU Mrror Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/mirror.itu");
    ...
    srcIcon = (ITUIcon*)ituSceneFindWidget(&scene, "Icon2");
	destIcon = (ITUIcon*)ituSceneFindWidget(&scene, "Icon3");
	ituWidgetSetDraw(destIcon, IconDraw);
