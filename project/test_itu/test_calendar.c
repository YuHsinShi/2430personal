...
static ITUColor focusColor;

static void UnSelect(int arg)
{
    ITUText* text = (ITUText*)arg;
    if (!ituWidgetGetCustomData(text))
    {
        ituTextSetBackColor(text, 0, 0, 0, 0);
    }
    else
    {
        ituTextSetBackColor(text, focusColor.alpha, focusColor.red, focusColor.green, focusColor.blue);
    }
}
...
static bool OnSelect(ITUWidget* widget, char* param)
{
    ...
    ITUText* text = (ITUText*)ituListBoxGetFocusItem(listbox);
    char* s = ituTextGetString(text);
    if (s && strlen(s) > 0)
        printf("selected day: %s\n", s);

    ituListBoxOnLoadPage(listbox, 1);
    focusColor = listbox->focusColor;
    ituTextSetBackColor(text, 255, 255, 0, 0);
    ituSceneExecuteCommand(&scene, 6, UnSelect, (int)text);
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU Calendar Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/calendar.itu");
