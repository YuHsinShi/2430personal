...
void ConfigInit(void)
{
    // network
    theConfig.dhcp = 0;
    strncpy(theConfig.ipaddr, "192.168.190.61", sizeof (theConfig.ipaddr) - 1);
    strncpy(theConfig.netmask, "255.255.255.0", sizeof (theConfig.netmask) - 1);
    strncpy(theConfig.gw, "192.168.190.1", sizeof (theConfig.gw) - 1);
    //strncpy(theConfig.dns, "192.168.1.1", sizeof (theConfig.dns) - 1);

    // wifi mode
    theConfig.wifi_mode = WIFI_CLIENT;
	// wifi switch on/off
	theConfig.wifi_on_off= 1;
    // SSID
    snprintf(theConfig.ssid, 64, "TOTOLINK");
    // Password
    snprintf(theConfig.password, 256, "12345678");
    // Security mode
