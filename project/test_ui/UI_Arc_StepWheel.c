...
bool SettingOnEnter(ITUWidget* widget, char* param)
{
	...
	if (!StepWheel)
	{
		...
		h = ituWidgetGetHeight(stepWheelChild[0]);
		w = ituWidgetGetWidth(stepWheelChild[0]);
...
bool SettingOnTimer(ITUWidget* widget, char* param)
{
	...
	if (ituWidgetUpdate(stepWheelChild[StepWheel->focusIndex], ITU_EVENT_LAYOUT, 0, 0, 0)) {
		for (i = 0; i<10; i++) {
			resize_w = w * stepWheelChild[i]->fontHeight / focusSize;
			resize_h = h * stepWheelChild[i]->fontHeight / focusSize;
			ituWidgetSetDimension(settingLangBackground[i], resize_w, resize_h);
			ituWidgetSetPosition(settingLangBackground[i],
				(ituWidgetGetX(stepWheelChild[i]) + w - resize_w) / 2,
...
int SDL_main(int argc, char *argv[])
{
	...
	ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/UI_Arc_StepWheel.itu");
