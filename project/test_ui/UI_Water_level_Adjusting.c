...
static ITUText *mainIceText = 0;
static ITUText *mainColdText = 0;
static ITUContainer *mainIceContainer = 0;
static ITUContainer *mainColdContainer = 0;
static ITUTrackBar *mainColdTrackBar = 0;
static ITUTrackBar *mainIceTrackBar = 0;
...
bool MainOnEnter(ITUWidget* widget, char* param)
{
	...
	if (!mainIceText)
	{
		mainIceText = ituSceneFindWidget(&scene, "mainIceText");
		assert(mainIceText);

		mainColdText = ituSceneFindWidget(&scene, "mainColdText");
		assert(mainColdText);

		mainIceContainer = ituSceneFindWidget(&scene, "mainIceContainer");
		assert(mainIceContainer);

		mainColdContainer = ituSceneFindWidget(&scene, "mainColdContainer");
		assert(mainColdContainer);

		mainColdTrackBar = ituSceneFindWidget(&scene, "mainColdTrackBar");
		assert(mainColdTrackBar);

		mainIceTrackBar = ituSceneFindWidget(&scene, "mainIceTrackBar");
		assert(mainIceTrackBar);
	}

	ituTextSetString(mainIceText, "90");
	ituTextSetString(mainColdText, "90");
    return true;
}

bool MainIceTrackBarOnChanged(ITUWidget* widget, char* param)
{
	int y;
	int value = atoi(param);
	ituTextSetString(mainIceText, param);
	y = 420 - value * 360 / 100;
	ituWidgetSetPosition(mainIceContainer, 624, y);
	return true;
}

bool MainColdTrackBarOnChanged(ITUWidget* widget, char* param)
{
	int y;
	int value = atoi(param);
	ituTextSetString(mainColdText, param);
	y = 420 - value * 360 / 100;
	ituWidgetSetPosition(mainColdContainer, 112, y);
	return true;
}
...
ITUActionFunction actionFunctions[] =
{
	...
	"MainIceTrackBarOnChanged", MainIceTrackBarOnChanged,
	"MainColdTrackBarOnChanged", MainColdTrackBarOnChanged,
	NULL, NULL
};
...
int SDL_main(int argc, char *argv[])
{
	...
	ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/UI_Water_level_Adjusting.itu");
