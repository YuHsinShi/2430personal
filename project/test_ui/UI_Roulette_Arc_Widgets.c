...
static ITUCoverFlow *settingLangCoverFlow = 0;
static ITUBackground *settingLangBackground[10] = { 0 };
static ITUContainer *settingLangCFContainer[10] = { 0 };
static ITUBackground *settingLangAllBackground = 0;

static ITUWheel *settingWheel = 0;
static ITUWidget* settingWheelChild[10] = { 0 };
static ITUBackground *settingLangBackgroundW[10] = { 0 };
static ITUBackground *settingLangAllBackgroundW = 0;

static ITUStepWheel *StepWheel = 0;
static ITUText* stepWheelChild[10] = { 0 };
static ITUBackground *settingLangBackgroundSW[10] = { 0 };
static int focusSize, h_SW, w_SW;

bool SettingOnEnter(ITUWidget* widget, char* param)
{
	if (!settingLangCoverFlow)
	{
		int i;
		settingLangCoverFlow = ituSceneFindWidget(&scene, "settingLangCoverFlow");
		assert(settingLangCoverFlow);

		settingWheel = ituSceneFindWidget(&scene, "settingWheel");
		assert(settingWheel);

		StepWheel = ituSceneFindWidget(&scene, "StepWheel");
		assert(StepWheel);

		settingLangAllBackground = ituSceneFindWidget(&scene, "settingLangAllBackground");
		assert(settingLangAllBackground);

		settingLangAllBackgroundW = ituSceneFindWidget(&scene, "settingLangAllBackgroundW");
		assert(settingLangAllBackgroundW);
		...
		for (i = 0; i < 10; i++){
			...
			settingLangCFContainer[i] = ituSceneFindWidget(&scene, widgetName);
			assert(settingLangCFContainer[i]);

			sprintf(widgetName, "settingLangBackground%dW", i);
			settingLangBackgroundW[i] = ituSceneFindWidget(&scene, widgetName);
			assert(settingLangBackgroundW[i]);

			stepWheelChild[i] = (ITUText*)itcTreeGetChildAt(StepWheel, i);
			assert(stepWheelChild[i]);

			sprintf(widgetName, "settingLangBackground%dSW", i);
			settingLangBackgroundSW[i] = ituSceneFindWidget(&scene, widgetName);
			assert(settingLangBackgroundSW[i]);
		}
		h = ituWidgetGetHeight(settingLangCFContainer[0]);
		center = h * ((ituWidgetGetHeight(settingLangAllBackground) / h) / 2);

		h_SW = ituWidgetGetHeight(stepWheelChild[0]);
		w_SW = ituWidgetGetWidth(stepWheelChild[0]);

		focusSize = stepWheelChild[StepWheel->focusIndex]->fontHeight;
	}
	return true;
}
...
{
	bool updated = false;
	int i, resize_w, resize_h;
	if (ituWidgetUpdate(settingLangCFContainer[0], ITU_EVENT_LAYOUT, 0, 0, 0)) {
		for (i = 0; i<10; i++) {
			// calculate arc height
			int y = ituWidgetGetY(settingLangCFContainer[i]);
	...
	if (ituWidgetUpdate(settingWheelChild[0], ITU_EVENT_LAYOUT, 0, 0, 0)) {
		for (i = 0; i<10; i++) {
			...
			arc_height = (arc_table[abs(theta)] * h) >> 15;
			if (arc_height) {
				ituWidgetSetVisible(settingLangBackgroundW[i], true);
				ituWidgetSetHeight(settingLangBackgroundW[i], arc_height);
				if (theta>0)
					ituWidgetSetY(settingLangBackgroundW[i], y);
				else
					ituWidgetSetY(settingLangBackgroundW[i], y + h - arc_height);
			}
			else
				ituWidgetSetVisible(settingLangBackgroundW[i], false);
		}
		updated = true;
	}

	if (ituWidgetUpdate(stepWheelChild[StepWheel->focusIndex], ITU_EVENT_LAYOUT, 0, 0, 0)) {
		for (i = 0; i<10; i++) {
			resize_w = w_SW * stepWheelChild[i]->fontHeight / focusSize;
			resize_h = h_SW * stepWheelChild[i]->fontHeight / focusSize;
			ituWidgetSetDimension(settingLangBackgroundSW[i], resize_w, resize_h);
			ituWidgetSetPosition(settingLangBackgroundSW[i],
				(ituWidgetGetX(stepWheelChild[i]) + w_SW - resize_w) / 2,
				ituWidgetGetY(stepWheelChild[i]));
		}
		updated = true;
	}
	return updated;
...
int SDL_main(int argc, char *argv[])
{
	...
	ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/UI_Roulette_Arc_Widgets.itu");
