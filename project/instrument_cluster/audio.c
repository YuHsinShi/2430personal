...
//*leaf api*//
void LeafAudioPlay(char *file,LeafPlaySoundCase playcase,SoundPlayCallback func)
{
    #ifdef __OPENRTOS__
    leaf_start_sound_play(call,file,playcase,func);
    #endif
}

void LeafAudioStop(void)
{
    #ifdef __OPENRTOS__
...
void LeafMixSoundStart(char *file,double MixLevel,int Repeat){
    #ifdef __OPENRTOS__
    leaf_mix_sound(call,file,MixLevel,Repeat);
    #endif
}

void LeafMixSoundStop(void){
    #ifdef __OPENRTOS__
    leaf_mix_sound(call,NULL,0,0);
    #endif
}