...
bool StorageCheckSpace(char *DrivePath, uint64_t checkSize)
{	
    bool result = true;
	struct statvfs info;
	if (statvfs(DrivePath, &info) == 0)
	{
		uint64_t avail = (uint64_t)info.f_bfree * info.f_bsize /1024; //KB
        printf("%s avail space:%lu KB\n", DrivePath, avail);
		if(avail < checkSize)
        {
            result = false;
