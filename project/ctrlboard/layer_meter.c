...
static void MeterGraphBackgroundDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    for (i = 0; i < rect->width; ++i)
    {
        ...
        if (ir0 < ir)
        {
			diffy = rect->height / 2 - ir;
			color.red = 255 - (diffy / 7 * 5); //the red of line will reduce 5 for each 7  pixcel hight
			ituColorFill(dest, destx + i, desty + diffy, 1, ir - ir0, &color);
        }
        else if (ir0 > ir)
        {
			diffy = rect->height / 2 - ir0;
			color.red = 255 - (diffy / 7 * 5);
			ituColorFill(dest, destx + i, desty + diffy, 1, ir0 - ir, &color);
        }
        else
        {
			diffy = rect->height / 2 - ir0;
			color.red = 255 - (diffy / 7 * 5);
			ituColorFill(dest, destx + i, desty + diffy, 1, 1, &color);
        }
        ir0 = ir;
    }
