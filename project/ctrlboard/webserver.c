	       ...
	       const char *data, uint64_t off, size_t size)
{
    ...
    if (!request->filebuf)
    {
        ...
        if (itcFileStreamOpen(&fileStream, CFG_UPGRADE_NAND_DRIVE ":/" CFG_UPGRADE_FILENAME, true) == 0)
        {
            printf("open package file %s to write\n", CFG_UPGRADE_NAND_DRIVE ":/" CFG_UPGRADE_FILENAME);
            request->filebuf = (uint8_t*)&fileStream;
        }
        else
        {
            printf("try to open %s file to write fail:0x%X\n", CFG_UPGRADE_NAND_DRIVE ":/" CFG_UPGRADE_FILENAME, errno);
            return MHD_NO;
        }
    }
    off = itcStreamWrite(&fileStream, data, size);
    if (off != size)
    {
        ...
        if (0 == strcmp (method, MHD_HTTP_METHOD_GET))
        {
            ...
            *ptr = NULL;                  /* reset when done */
            ...
            if (strcmp(val, "upgrade") == 0)
            {
                ...
                // upgrade firmware
            #ifdef CFG_UPGRADE_FROM_NAND
                if (itcFileStreamOpen(&fileStream, CFG_UPGRADE_NAND_DRIVE ":/" CFG_UPGRADE_FILENAME, false) == 0)
                {
                    if (ugCheckCrc(&fileStream.stream, NULL))
                    {
                        puts("Upgrade failed.");
                    ...
                    {
                        UpgradeSetStream(&fileStream.stream);
                ...
                {
                    printf("try to open %s file to read fail:0x%X\n", CFG_UPGRADE_NAND_DRIVE ":/" CFG_UPGRADE_FILENAME, errno);
                }
            #else
