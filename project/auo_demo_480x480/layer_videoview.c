...
bool VideoViewRandomButtonOnPress(ITUWidget* widget, char* param)
{
...
bool VideoViewOnEnter(ITUWidget* widget, char* param)
{
    ...
    if (!videoPlayerStorageSprite)
    {
		...
		videoViewVolProgressBar = ituSceneFindWidget(&theScene, "videoViewVolProgressBar");
		assert(videoViewVolProgressBar);
	...
	ituProgressBarSetValue(videoViewVolProgressBar, vol);
    ituTextSetString(videoViewTimeText, NULL);
